# BONEAMANITA 3.3.2 - "THE THERMAL COUPLER"
...
class PhysicsEngine:
    def analyze(self, text):
        ...
        # [THE PARADOX ENGINE]
        # Voltage now calculates Thermal Tension (Fire + Ice interaction)
...
class FrequencyModulator:
    ...
    def tune_in(self, phys, vector, is_rag_slop):
        ...
        # [JESTER: HIGH VOLTAGE / PARADOX]
        if phys['beta_friction'] > 2.0:
...
class BoneAmanita:
    ...
    def process(self, text):
        ...
        if m['physics']['narrative_drag'] > 3.0 and not lichen_msg and m['physics']['beta_friction'] < 2.0:
            # If high drag but NOT high voltage (Paradox), cost ATP.
            # (Note: High Voltage protects against Drag Cost now)
            self.atp -= 2
    ...
    def _render(self, m, lichen_msg, strat, reason, title, ops, station, starve_msg, ghost_msg, clean_text, did_surgery):
        ...
        # Calculate Voltage for display purposes logic if needed, but beta is already in p
        # Let's wrap Beta Friction to show the heat
        beta_c = Prisma.wrap(p['beta_friction'], 2.0, invert=True)

        print(f"\n{Prisma.GRY}--------------------------------------------------{Prisma.RST}")
        print(f"{Prisma.WHT}[BONEAMANITA v3.3.2]{Prisma.RST} {Prisma.CYN}{title}{Prisma.RST}")

        drag_c = Prisma.wrap(p['narrative_drag'], 3.0)
        atp_c = Prisma.wrap(self.atp, 10, invert=True)

        print(f"ATP: {atp_c} | DRAG: {drag_c} | VOLT: {beta_c} | PROTOCOL: {strat}")
        ...
        if mems: print(f"{Prisma.GRY}MEMORIES: {', '.join(mems)}{Prisma.RST}")

        print(f"{Prisma.GRY}--------------------------------------------------{Prisma.RST}\n")

if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v3.3.2 ONLINE (THERMAL COUPLER).{Prisma.RST}")
    while True:
