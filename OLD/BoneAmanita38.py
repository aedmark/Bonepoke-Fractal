# BONEAMANITA 3.8 - "DELOREAN EDITION"
...
class TheOracle:
    @staticmethod
    def cast_bones(phys, atp, charge):
        omens = []
        drag = phys["narrative_drag"]
        volt = phys["voltage"]
        ent = phys["vector"]["ENT"]

        # 1. THE STARVATION OMEN
        if atp < 15 and charge < 5.0:
            omens.append(f"{Prisma.RED}🔮 OMEN: FAMINE IMMINENT. The pantry is bare.{Prisma.RST}")

        # 2. THE GRAVITY OMEN (approaching 8.0)
        if drag > 6.5:
            omens.append(f"{Prisma.YEL}🔮 OMEN: EVENT HORIZON DETECTED. Narrative is too heavy.{Prisma.RST}")

        # 3. THE STORM OMEN (approaching ±8.0)
        if volt > 6.5:
            omens.append(f"{Prisma.CYN}🔮 OMEN: STATIC BUILDUP. Lightning strike probable.{Prisma.RST}")
        elif volt < -6.5:
             omens.append(f"{Prisma.MAG}🔮 OMEN: TOXICITY RISING. System turning septic.{Prisma.RST}")

        # 4. THE VOID OMEN (High Entropy, Low Texture)
        if ent > 0.8 and phys["vector"]["TEX"] < 0.2:
             omens.append(f"{Prisma.BLU}🔮 OMEN: VACUUM LEAK. Tether required.{Prisma.RST}")
...
class BoneAmanita:
    ...
    def pollinate(self, vector, current_words):
        ...
        for word, tick in self.mem.artifacts.items():
            ...
            if is_abstract and mem_is_heavy:
                candidates.append((word, "GROUNDING SPORE", tick))
            elif is_concrete and mem_is_abstract:
                candidates.append((word, "ELEVATING SPORE", tick))
            elif not is_abstract and not is_concrete:
                # Wild spores are lower priority
                candidates.append((word, "WILD SPORE", tick - 100))

        if candidates:
            # SORT BY RECENCY (TICK) - The freshest memory wins
    ...
    def process(self, text):
        ...
        omens = TheOracle.cast_bones(m["physics"], self.atp, self.battery.current_charge)
        ...
        self._render(m, lichen_msg, strat, reason, title, ops, station_data, starve_msg, None, clean_text, len(ops)>0, battery_log, lazarus_msg, spore_msg, ta_vel, omens)
        self.mem.save(current_atp=self.atp, current_charge=self.battery.current_charge)

    def _render(self, m, lichen_msg, strat, reason, title, ops, station, starve_msg, ghost_msg, clean_text, did_surgery, battery_log, lazarus_msg, spore_msg=None, ta_val=0.0, omens=[]):
        p = m["physics"]
        ...
        ta_symbol = "►" # Stable
        if ta_val > 2.0: ta_symbol = "▲" # Accelerating
        elif ta_val < -2.0: ta_symbol = "▼" # Decelerating

        print(f" {Prisma.WHT}ATP:{Prisma.RST} [{atp_bar}] {batt_bar}  {Prisma.WHT}DRAG:{Prisma.RST} {drag_c}  {Prisma.WHT}Ta({ta_symbol}):{Prisma.RST} {ta_c}  {Prisma.WHT}VOLT:{Prisma.RST} {volt_c}  {Prisma.WHT}SIG:{Prisma.RST} {sig}")
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v3.8 ONLINE.{Prisma.RST}")
    try:
        while True:
