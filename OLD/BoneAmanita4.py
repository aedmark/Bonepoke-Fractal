...
class PhysicsEngine:
    def analyze(self, text):
        ...
        counts["toxin"] = len(matches) # Explicit count
        ...
        return {
            "physics": {
                ...
                "rhythm_variance": round(rhythm_variance, 2), # NEW METRIC
...
class DeepStorage:
    ...
    def recall_spores(self, current_words):
        """Finds words in the graph strongly connected to current input"""
        candidates = []
        for w in current_words:
            if w in self.graph:
                # Find strongest edge from this word
                edges = self.graph[w]["edges"]
                if edges:
                    best_match = max(edges, key=edges.get)
                    if best_match not in current_words:
                        candidates.append((best_match, edges[best_match]))
        
        # Sort by connection strength
        candidates.sort(key=lambda x: x[1], reverse=True)
        return candidates[:1] # Return top match
        ...
        if os.path.exists(path):
            try:
                with open(path, "r") as f:
                    ...
                    else: memories = {} # Legacy support dropped for purity
...
class FrequencyModulator:
    ...
    def tune_in(self, phys, vector, is_rag_slop, atp):
        ...
        # CLARENCE (The Butcher) - Bids on Drag & violations
        if phys.get("case_violation"): signals["CLARENCE"] += 15.0 # Immediate veto
        if drag > BoneConfig.CLARENCE_TRIGGER: signals["CLARENCE"] += (drag * 2)
        if is_rag_slop: signals["CLARENCE"] += 5.0

        # ELOISE (The Grounder) - Bids on Entropy & ECP
        if phys.get("ecp_violation"): signals["ELOISE"] += 15.0 # Immediate veto
        if vector["ENT"] > 0.6: signals["ELOISE"] += (vector["ENT"] * 10)
        ...
        # JESTER (The Paradox) - Bids on High Voltage
        if volt > 7.0: signals["JESTER"] += (volt - 5.0) * 3
...
class TheOracle:
    @staticmethod
    def triage(phys, atp, charge):
        # Returns: (Priority_Score, Message, Color)
        omens = []
        drag = phys["narrative_drag"]
        volt = phys["voltage"]
        ent = phys["vector"]["ENT"]

        # 1. FAMINE (Highest Priority if immediate)
        if atp < 10 and charge < 2.0:
            omens.append((100, "FAMINE IMMINENT. FEED OR DIE.", Prisma.RED))
        elif atp < 15 and charge < 5.0:
            omens.append((50, "Resources Low. Conserve energy.", Prisma.YEL))

        # 2. EVENT HORIZON (Gravity)
        if drag > 7.5:
             omens.append((90, "GRAVITATIONAL COLLAPSE. CUT ADVERBS NOW.", Prisma.RED))
        elif drag > 6.0:
        ...
        # HUMILITY CHECK: Only bother the user if priority > 40
        if score > 40:
...
class DreamEngine:
    """
    v4.0: Generates hallucinations from the Memory Graph when Boredom is high.
    """
    PROMPTS = [
    ...
    def hallucinate(self, graph):
        ...
        node_a = random.choice(keys)
        node_b = random.choice(keys)
        
        if node_a == node_b: return None
        
        template = random.choice(self.PROMPTS)
        return template.format(A=node_a.upper(), B=node_b.upper())
...
class BoneAmanita:
    ...
    def pollinate(self, vector, current_words):
        ...
        is_abstract = vector['ENT'] > 0.6
        is_concrete = vector['TEX'] > 0.6
        
        # If we are balanced, we don't need spores
        if not is_abstract and not is_concrete: return None

        candidates = []
        current_tick = self.tick_count

        for word, tick in self.mem.artifacts.items():
            if word in current_words: continue
            
            # DECAY CALCULATION:
            # Effective Tick = Real Tick - (Times Used * 50)
            # This makes used memories "feel" older to the sorter
            usage = self.mem.usage_map.get(word, 0)
            effective_tick = tick - (usage * 50) 
            
            # Filter out "ancient" history to prevent zombie spores
            if (current_tick - effective_tick) > 200: continue 

            mem_is_heavy = word in TheLexicon.HEAVY_MATTER
            mem_is_abstract = word in TheLexicon.ABSTRACTS

            if is_abstract and mem_is_heavy:
                candidates.append((word, "GROUNDING SPORE", effective_tick))
            elif is_concrete and mem_is_abstract:
                candidates.append((word, "ELEVATING SPORE", effective_tick))

        if candidates:
            # Sort by EFFECTIVE tick (Recency - Decay)
            candidates.sort(key=lambda x: x[2], reverse=True)
            target, st, _ = candidates[0]
            
            # Increment usage to decay it for next time
            self.mem.usage_map[target] = self.mem.usage_map.get(target, 0) + 1
    ...
    def process(self, text):
        ...
        if self.coma_turns > 0:
            self.coma_turns -= 1
            print(f"\n{Prisma.GRY}[ COMA STATE ]{'='*30}{Prisma.RST}")
            print(f"{Prisma.GRY}You are in biological shutdown. Stamina regenerating... ({self.coma_turns} turns remain){Prisma.RST}")
        ...
        self.stamina = min(BoneConfig.MAX_STAMINA, self.stamina + sugar - stamina_cost)

        # 5. HEALTH IMPACT & COMA
        health_impact = 0
        ...
        omens = TheOracle.cast_bones(m["physics"], self.stamina, self.battery.current_charge)
        spore_msg = self.pollinate(m["physics"]["vector"], m["clean_words"])

        self._render(m, lichen_msg, strat, reason, title, ops, station_data, None, None, clean_text, len(ops)>0, battery_log, None, spore_msg, ta_vel, omens, dream_msg)
        self.mem.save(health=self.health, stamina=self.stamina)
    ...
    def _render(self, m, lichen_msg, strat, reason, title, ops, station, starve_msg, ghost_msg, clean_text, did_surgery, battery_log, lazarus_msg, spore_msg=None, ta_val=0.0, omens=[], dream_msg=None):
        ...
        # --- 2. THE FLIGHT DECK ---
        # Added 'strat' to the header so you can see the Archetype Strategy
        print(f"\n{Prisma.GRY}[ {Prisma.WHT}{title}{Prisma.GRY} | {Prisma.CYN}{strat}{Prisma.GRY} ]{'='*35}{Prisma.RST}")
        
        # [NOTE] Ensure self.atp and self.battery match your class variables
        # If you renamed them to self.health/stamina, swap these back.
        hp_p = int((self.atp / 50.0) * 10) # Assuming Max ATP is 50
        ...
        print(f" {Prisma.WHT}ATP:{Prisma.RST} [{hp_bar}] {batt_bar}")
        print(f" {Prisma.WHT}DRAG:{Prisma.RST} {drag_c}  {Prisma.WHT}Ta:{Prisma.RST} {ta_display}  {Prisma.WHT}VOLT:{Prisma.RST} {volt_c}  {Prisma.WHT}SIG:{Prisma.RST} {sig}")
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v4.0 'XENOMORPH' ONLINE.{Prisma.RST}")
    print(f"{Prisma.GRY}System: Circadian Rhythms Active. Dream Engine Idle.{Prisma.RST}")
    try:
        while True:
