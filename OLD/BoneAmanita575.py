# BONEAMANITA 5.7.5 - "WHOSE LINE IS IT ANYWAY?"
...
class PhysicsEngine:
    ...
    def analyze(self, text):
        ...
        # CATEGORY SCANS
        for w in clean_words:
            # 1. CHECK THE LEXICON (The Standard Physics)
            ...
            # 2. CHECK THE PROPS (The Scene Director)
            # Props grant Mass (Heavy) but flavor the Voltage later.
            if w in BoneConfig.PROPS["LAB"]:
                counts["heavy"] += 1
                counts["kinetic"] += 1 # Lab props are active

            if w in BoneConfig.PROPS["COURTYARD"]:
                counts["heavy"] += 1
                counts["aerobic"] += 1 # Courtyard props are breathable

            if w in BoneConfig.PROPS["DREAM"]:
                counts["abstract"] += 1
                counts["photo"] += 1 # Dreams are visual abstractions
...
class HubThermostat:
    ...
    def check_temperature(self, clean_words):
        ...
        # 3. UPDATE HISTORY (The Sliding Window)
        ...
        # 4. CHECK FREQUENCY (The Ban Hammer)
        ...
        for w in clean_words:
            if len(w) > 3 and w not in TheLexicon.SOLVENTS:
                ...
                if count >= 5:
                    ...
                    self.history = [x for x in self.history if x != w]
                    overheated.append(w)
...
class BoneAmanita:
    ...
    def _render(
        self, m, lichen_msg, strat, reason, title, ops, station, clean_text, did_surgery, battery_log, lazarus_msg, spore_msg, ta_val, omens, dream_msg, mode, cosmic_msg):
        p = m["physics"]
        if mode == "COURTYARD":
