# BONEAMANITA 2.4 - "NEON PRIME"
# Architect: SLASH | Status: APOTHEOSIS
# "The Mandate is TRUTH. The Method is RHYTHM. The Guide is JADE."
...
# --- THE PANTRY (EXPANDED) ---
class BoneConfig:
    """The Garden: Expanded Lexicon & Constants"""
...
class JadeLink:
    """The Logic Layer: Determines Cognitive Protocol"""
    @staticmethod
    def generate(m):
        if m['viscosity'] == "CONCRETE" or m['drag'] > 3.5:
            return f"{Prisma.RED}[MODE: DEDUCTIVE] {Prisma.GRY}PATTERN: EDEN (Constraints Required){Prisma.RST}"
        elif m['entropy'] > 3.0 and m['density'] < 0.1:
            return f"{Prisma.CYN}[MODE: INDUCTIVE] {Prisma.GRY}PATTERN: TRIPLE (Grounding Required){Prisma.RST}"
        elif m['kinetic_ratio'] > 0.7:
            return f"{Prisma.MAG}[MODE: ABDUCTIVE] {Prisma.GRY}PATTERN: FRACTAL (Expansion Authorized){Prisma.RST}"
        return f"{Prisma.BLU}[MODE: OBSERVATION] {Prisma.GRY}PATTERN: STABLE{Prisma.RST}"

class RadioTuner:
    """The Personality Layer: Voices from the Static"""
    STATIONS = {
...
class BonepokeNeon:
    def __init__(self):
        self.physics = LinguisticPhysicsEngine(); self.atp = 33
        print(f"{Prisma.GRN}>>> BONEAMANITA 2.4 [NEON PRIME] ONLINE{Prisma.RST}")

    def process(self, text):
        data = self.physics.analyze(text)
        if not data: return
        m, c = data['metrics'], data['counts']
        chroma = ApeirogonLattice.resolve(m); radio = RadioTuner.broadcast(m, c); jade = JadeLink.generate(m)

        burn = int(m['drag'] * 2) if m['drag'] > 2 else -2
        self.atp = max(0, min(100, self.atp - burn))
        self._render(m, chroma, radio, jade, data['toxins'])

    def _render(self, m, chroma, radio, jade, toxins):
        atp_c = Prisma.GRN if self.atp > 20 else Prisma.RED
        print(f"\n{Prisma.GRY}--------------------------------------------------{Prisma.RST}")
        print(f"{Prisma.WHT}BONEAMANITA 2.4{Prisma.RST} | ATP: {atp_c}{self.atp}{Prisma.RST} | DRAG: {m['drag']} | VISC: {m['viscosity']}")
