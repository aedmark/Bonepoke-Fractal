# BONEAMANITA 4.4.2 - "THE JESTER'S PATCH"
...
class MycelialNetwork:
    ...
    def bury(self, clean_words, tick):
        ...
        if len(self.graph) > BoneConfig.MAX_MEMORY_CAPACITY:
            self.cannibalize() # Replaced internal _prune with public cannibalize

    def cannibalize(self, preserve_current=None):
        """
        Sacrifice memory to survive, but preserve current context.
        NOISY CANNIBALISM (Jester's fix).
        """
        if preserve_current:
        ...
        candidates = [(k, v) for k, v in self.graph.items() if k not in protected]

        if not candidates: return None # Nothing to eat

        # Sort by (age, weakness)
        candidates.sort(key=lambda x: (x[1]["last_tick"], -len(x[1]["edges"])))

        victim, data = candidates[0]
        ...
        # Return the bone for the logs
        return f"MEMORY SACRIFICED: '{victim}' (Connections: {len(data['edges'])})"
...
class BoneAmanita:
    def __init__(self):
        ...
        # INHERITANCE PARADOX FIX
        parent_hp = self.mem.session_health
        if parent_hp is not None:
            # Jester's Logic: Trauma decays
            trauma = (BoneConfig.MAX_HEALTH - parent_hp) / BoneConfig.MAX_HEALTH
            inheritance_penalty = trauma * 20 # Max 20 HP penalty
            self.health = min(BoneConfig.MAX_HEALTH, BoneConfig.MAX_HEALTH - inheritance_penalty)
            if trauma > 0.3:
                print(f"{Prisma.CYN}[GENETICS]: Ancestral trauma ({int(trauma*100)}%) carried forward.{Prisma.RST}")
        else:
    ...
    def reinforce_salvage_words(self, clean_words, phys):
        ...
        if phys.get("symbolic_state") != "SALVAGE": return
        ...
        for word in heavy_words[:3]: TheLexicon.teach(word, "heavy")
        for word in kinetic_words[:2]: TheLexicon.teach(word, "kinetic")
    ...
    def process(self, text):
        ...
        if self.stamina < 20 and self.battery.current_charge > 0.5:
            ...
            if drained > 0:
                ...
                battery_log.append(f"{Prisma.YEL}⚡ EMERGENCY POWER (-{round(drained,1)} Chg -> +{round(recovered,1)} STM){Prisma.RST}")
        ...
        lazarus_msg = f"{Prisma.RED}💀 CRITICAL CONDITION: {autopsy_msg}{Prisma.RST}" if is_dead else None
        ...
        cannibal_msg = None
        if len(self.mem.graph) >= BoneConfig.MAX_MEMORY_CAPACITY:
            cannibal_msg = self.mem.cannibalize(preserve_current=m["clean_words"])
        ...
        # Add cannibal msg to battery log for visibility
    ...
    def _render(self, m, lichen_msg, strat, reason, title, ops, station, clean_text, did_surgery, battery_log, lazarus_msg, spore_msg, ta_val, omens, dream_msg, mode):
        ...
        for b in battery_log:
            if "EMERGENCY" in b or "TOXIN" in b or "NEURO" in b or "SACRIFICED" in b: print(f"    {b}")
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v4.4.2 (THE JESTER'S PATCH){Prisma.RST}")
