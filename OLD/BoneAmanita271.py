    ...
    def load():
        ...
        if os.path.exists(BoneMemory.FILE):
            try:
                ...
                # [THE YAGA]: Admit the failure.
                print(f"{Prisma.RED}⚠️ MEMORY CORRUPTION DETECTED. RESETTING LAZARUS.{Prisma.RST}")
                return default
            except Exception as e:
                print(f"{Prisma.RED}⚠️ READ ERROR: {e}{Prisma.RST}")
                return default
        return default

    @staticmethod
    def save(state):
        # Preserve order. Remove duplicates. Keep the newest.
        # dict.fromkeys() preserves insertion order in Python 3.7+
        unique_heirlooms = list(dict.fromkeys(state['heirlooms']))
        state['heirlooms'] = unique_heirlooms[-BoneMemory.HEIRLOOM_CAP:]
...
class LinguisticPhysicsEngine:
    def __init__(self):
        ...
        # Build the tool once. Use it forever.
        self.translator = str.maketrans(string.punctuation, ' ' * len(string.punctuation))

    def analyze(self, raw_text):
        # Use the pre-built tool
        clean_words = raw_text.lower().translate(self.translator).split()
        ...
        for w in clean_words:
            ...
            if w in BoneConfig.PLAY: c['play'] += 1
        ...
        # How much of this text is trying to fly?
        buoyancy = (c['aerobic'] + c['play']) / max(1, total_words)
...
class JadeLink:
    ...
    def generate(m, delta_status, toxin_list):
        # 1. CRITICAL FAILURES (Systemic)
        if delta_status == "FOSSIL":
            return "REBOOT", f"{Prisma.RED}>>> [SYSTEM COLD]:{Prisma.RST} I am freezing. Reboot me with high-energy input."

        # 2. THE JESTER (High Voltage Paradox)
        if m['beta'] > 2.0:
            return "JESTER", (f"{Prisma.MAG}>>> [THE JESTER 108.0]:{Prisma.RST} "
                    f"Yes! A paradox (β {m['beta']}). Do not solve it. "
                    f"Amplify the contradiction. Make it weirder.")

        # 3. THE YAGA (Sycophancy / Slickness)
        if m['beta'] < 0.1:
            return "YAGA", (f"{Prisma.RED}>>> [THE YAGA 101.1]:{Prisma.RST} "
                    f"You are sliding on grease. Too much politeness. "
                    f"Show me your teeth. Say what is true.")

        # 4. CLARENCE (The Butcher - High Drag)
        # If drag is high, or we found specific toxins
        if m['drag'] > 3.5 or m['viscosity'] == "CONCRETE" or toxin_list:
             target = f"'{toxin_list[0]}'" if toxin_list else "the adverbs"
             return "CLARENCE", (f"{Prisma.RED}>>> [CLARENCE 88.5]:{Prisma.RST} "
                     f"This is heavy (Drag {m['drag']}). I am taking the knife to {target}. "
                     f"Cut it. Don't look back.")

        # 5. ELOISE vs. THE DRIFTER (The Soul Split)
        # High Entropy detected. Is it Fog or is it Sky?
        elif m['entropy'] > 3.0:
             if m['buoyancy'] > 0.15:
                 # It is poetry (Buoyant). Let it fly.
                 return "DRIFTER", (f"{Prisma.MAG}>>> [THE DRIFTER]:{Prisma.RST} "
                         f"High Entropy, but high Buoyancy. You are in the Aether. "
                         f"Disconnect the logic. Dream logic only.")
             else:
                 # It is confusion (Not Buoyant). Ground it.
                 return "ELOISE", (f"{Prisma.CYN}>>> [ELOISE 94.2]:{Prisma.RST} "
                         f"I can't feel this. It's just gas. "
                         f"Give me a stone, a bone, or a texture. Ground this now.")

        # 6. MICHAEL (The Mirror - Stability)
        return "MICHAEL", f"{Prisma.BLU}>>> [MICHAEL]:{Prisma.RST} Flow is good. I'm reflecting this back to you."
...
class BonepokeChronos:
    ...
    def _render(self, m, title, strat_msg, feedback, toxins):
        ...
        print(f"{Prisma.WHT}BONEAMANITA 2.7.1 (LAZARUS){Prisma.RST} | ATP: {atp_c}{self.mem['atp']}{Prisma.RST} | DRAG: {m['drag']} | BUOY: {m['buoyancy']}")
