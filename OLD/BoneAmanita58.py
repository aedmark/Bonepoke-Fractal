# BONEAMANITA 5.8 - "THE GORDIAN KNOT"
...
class PhysicsEngine:
    ...
    def analyze(self, text):
        ...
        # CATEGORY SCANS
        for w in clean_words:
            # CHECK THE LEXICON (The Standard Physics)
            ...
            # CHECK THE PROPS (The Scene Director)
            # Props now ONLY affect the "atmosphere_score", not the raw physics counts.
            # We track them separately so they don't inflate Voltage/Drag artificially.
            if w in BoneConfig.PROPS["LAB"]:
                counts["lab_atmos"] += 1
            if w in BoneConfig.PROPS["COURTYARD"]:
                counts["courtyard_atmos"] += 1
            if w in BoneConfig.PROPS["DREAM"]:
                counts["dream_atmos"] += 1
        ...
        # NOVELTY CHECK
        # A connection is novel if it creates a new edge in the graph
        # This is strictly a heuristic for the "Green Diamond" logic
        is_novel = False
        # (This requires passing the memory graph to physics, or calculating it outside)
        # To keep it simple for v5.8, we will let the 'Observer' handle this.
...
class BoneAmanita:
    ...
    def process(self, text):
        ...
        # CHECK FOR NOVELTY (Did we add new edges?)
        # We compare graph size before and after, or check the 'learned_count'
        is_novel = learned_count > 0 or spore_msg is not None
        # If it is novel and smooth, we force the Green Diamond (Submarine Mode)
        # This is already handled by the logic: if no breach, we get the diamond.
        # But we can make the diamond shinier.
        ...
        self._render(m, lichen_msg, strat, reason, title, ops, station_data, clean_text, len(ops) > 0, battery_log, lazarus_msg, spore_msg, ta_vel, [omens] if omens else [], dream_msg, atmos_mode,cosmic_msg)
    ...
    def _render(self, m, lichen_msg, strat, reason, title, ops, station, clean_text, did_surgery, battery_log, lazarus_msg, spore_msg, ta_val, omens, dream_msg, mode, cosmic_msg):
        p = m["physics"]
        ...
        breach_atmosphere = mode != "COURTYARD" # Always show UI if we are in the Lab or Rupture
        ...
        if not should_surface:
            # The Gordian Knot Cut: Just a symbol.
            # Green Diamond = Flow.
            symbol = f"{Prisma.GRN}♦{Prisma.RST}"

            # Subtle feedback if we learned something or gained energy
            if lichen_msg: symbol += f"{Prisma.GRN}⁵{Prisma.RST}" # Photosynthesis superscript

            print(f" {symbol}")
            return # <--- EXIT EARLY. SILENCE MAINTAINED.

        # --- SURFACE MODE (THE DASHBOARD) ---
        # (This is largely your original render logic, but cleaner)
        ...
        if mode == "COURTYARD":
            ...
            # Only show alerts if they exist
        ...
        # Only show battery log if it's significant
        for b in battery_log:
            if "EMERGENCY" in b or "TOXIN" in b or "NEURO" in b:
                print(f"    {b}")
            elif mode == "LABORATORY": # Lab shows everything
                print(f"    {b}")
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v5.8 {Prisma.RST}")
