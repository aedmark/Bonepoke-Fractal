# ---------------------------------------------------------------------------
# BONEAMANITA 0.9 - "The Helium Protocol"
...
class TheLexicon:
    """
    THE PANTRY [v0.9.0 PATCHED]
    Now includes Aerobic Matter and Whimsy.
    """
    # 1. PHYSICS MATTER (Combined Heavy & Light)
...
# --- COMPONENT 5: THE SIGNATURE ENGINE ---------------------------
...
class SignatureEngine:
    ...
    def get_pressure_settings(self, archetype_name):
        # 1. ESTABLISH BASELINE PHYSICS
        # Default: "The Gym Coach" (Standard strictness)
        defaults = {
        ...
        # 2. DEFINITION OF ALL PRESSURE OVERRIDES (v0.8.2 + v0.9.0)
        overrides = {
            # --- THE LIGHT ARCHETYPES (v0.9.0) ---
            "THE BARD": {
            ...
            # --- THE HEAVY/SURVIVAL ARCHETYPES (v0.8.2) ---
            "THE PALADIN": {
    ...
    def calculate_dimensions(self, token_data, phys_metrics, stipe_data):
        ...
        # --- 1. CALCULATE BUOYANCY (The Helium Protocol) ---
        ...
        # --- 2. VELOCITY (Adjusted for Play) ---
        # Play verbs count as movement now
        velocity = phys_metrics.get('kinetic_ratio', 0.0)
        if play_count > 0: velocity = min(1.0, velocity + 0.2)

        # --- 3. STRUCTURE (The Drag Offset) ---
        drag = phys_metrics.get('narrative_drag', 2.0)

        # THE PATCH: If Buoyancy is high, we forgive Drag.
        # "The balloon floated lazily over the horizon" is slow (High Drag)
        # but buoyant, so we don't punish it.
        ...
        # --- 5. TEXTURE (Includes Aerobic Matter) ---
        # Uses the parent Lexicon's UNIVERSALS which now includes balloons/feathers
        ...
        # --- 6. TEMPERATURE (The Warmth Index) ---
        temp_sum = 0
        for w in clean_words:
        ...
        return {
            ...
            "BUOYANCY": round(buoyancy, 2) # Exported for Dashboard
        }
    ...
    def detect_slurry(self, sig):
        ...
        """
        # 1. The "Competence" Trap (Mid-tier stats are suspicious)
        is_mid_vel = 0.3 <= sig['VEL'] <= 0.6
        is_mid_str = 0.4 <= sig['STR'] <= 0.8

        # 2. The Void (Lack of Sensory Detail)
        is_low_tex = sig['TEX'] < 0.2

        # 3. THE V0.9.0 PATCH: The Spark Check
...
class BonepokeCore:
    ...
    def process(self, text, parent_id=None):
        ...
        # LOGIC: If we are in a "Light" Archetype or High Buoyancy,
        # Michael speaks instead of Clarence.
        ...
        # 1. PACING CHECK (Michael vs Clarence)
        if phys['narrative_drag'] > 3.0 or loop_count > 2:
            if is_light_mode:
                # Michael handles the critique gently
                msg = self.cortex.synthesize_voice("MICHAEL", token_data, metrics, loop_count)
...
if __name__ == "__main__":
    ...
    print("--- BONEAMANITA v0.9: THE HELIUM PROTOCOL---")
