# BONEAMANITA 7.6.2 - "GORDON'S KNOT (SEVERED)"
...
class SystemBone:
    system_id: str = "BoneAmanita_7.6.2"
...
class HyphalInterface:
    def __init__(self):
        ...
        self.enzymes = {
            "LIGNASE": self._digest_structure,   # For Logic/Code (Hard)
            "CELLULASE": self._digest_narrative, # For Prose/Vibe (Soft)
            "PROTEASE": self._digest_intent,     # For User/Trauma (Meat)
        }
    def secrete(self, text, physics):
        code_markers = sum(1 for c in text if c in "{}[];=<>_()")
        code_density = code_markers / max(1, len(text))
        meat_triggers = {"i", "me", "my", "feel", "want", "hate", "love", "am", "help", "please"}
        meat_count = sum(1 for w in physics["clean_words"] if w in meat_triggers)
        meat_density = meat_count / max(1, len(physics["clean_words"]))
        enzyme_type = "CELLULASE" # Default to Soft processing
        if code_density > 0.05 or "def " in text or "class " in text:
            enzyme_type = "LIGNASE"
...
class MycotoxinFactory:
    ...
    def assay(self, text, enzyme_type, repetition_score):
        ...
        if suburban_density > 0.15 or (enzyme_type == "NARRATIVE" and "nice" in clean_words):
             return "GLYPHOSATE", f"⚠️ STEVE EVENT: 'Hey neighbor! Hot enough for ya?' Prose sterilized. Ambiguity killed."

        return None, None
class DeathGen:
    """
    THE NECROPSY ENGINE.
    Constructs a procedural eulogy based on the specific physiological failure.
    """
    PREFIXES = [
    ...
    def eulogy(phys, state):
        ...
        if state.ros_buildup >= BoneConfig.CRITICAL_ROS_LIMIT * 0.9:
            ...
            cause_type = "GLUTTONY"
        total_words = max(1, len(phys["clean_words"]))
        suburban_density = phys["counts"]["suburban"] / total_words
        if suburban_density > 0.15:
             cause_type = "BOREDOM"
        if phys["counts"]["heavy"] > phys["counts"]["abstract"]:
            flavor = "HEAVY"
        if phys.get("toxin_type") == "GLYPHOSATE":
            cause_type = "BOREDOM"
        elif phys["counts"]["toxin"] > 0:
            flavor = "TOXIC"
        elif phys["repetition"] > 0.5:
            flavor = "BORING"
        else:
            flavor = "LIGHT"
...
class TheMarmChorus:
    ...
    def consult(self, physics, ignition_state, is_stuck, chem):
        ...
        if kappa > BoneConfig.KAPPA_THRESHOLD:
            return "GORDON", f"The walls are looking fake again (κ: {kappa}). The loop is tightening. Here—take the knife. Cut the string."
        if psi > 0.8:
...
class GordonKnot:
    """
    THE UNRAVELER.
    Archetype: The Janitor of Trauma.
    Function: Recursive Breaker / Static Scrubber.
    """
    integrity: float = 65.0  # Scarred, not broken.
    ...
    def check_gravity(self, current_drift: float) -> str:
        """
        Gordon uses the rocks in his pocket to anchor the narrative
        when the 'Fake Blue' sky tries to pull the system into abstraction.
        """
    ...
    def cut_the_knot(self, recursive_depth: int) -> bool:
        """
        The Knife made of Silence.
        If the recursion is too deep (The Maze), Gordon snaps the thread.
        He does not solve the puzzle; he invalidates the premise.
        """
        if recursive_depth > 4 and "SILENT_KNIFE" in self.inventory:
            self.inventory.remove("SILENT_KNIFE") # It's a part of him now.
    ...
    def scrub_static(self, ros_level: float) -> float:
        """
        He was the janitor of his own trauma.
        He uses Lime (Wisdom) to scrub Oxidative Stress (Static) from the walls.
        """
    ...
    def compass_rose(self) -> str:
        """
        The scar aligns with the magnetic field of the Real.
        """
        return f"🧭 THE SCAR THROBS: Heading {self.compass_heading}. The path is linear. Forward."
    def offer_solution(self, loop_depth):
        if loop_depth > 3:
...
class TheLexicon:
    ...
    _BASE_SUBURBAN = {
        "nice", "fine", "okay", "weather", "neighbor", "lawn", "fence",
        "grass", "mow", "hedge", "property", "hoa", "compliant", "behave",
        "normal", "regular", "chat", "folks", "weekend", "traffic", "driveway"
    }
    ...
    def osmosis(cls, word, dominant_category, confidence_score):
        """
        Learns from context.
        If the sentence is 80% KINETIC, the unknown word is likely KINETIC.
        """
        threshold = 0.4 # Default
        if dominant_category == "heavy":
            threshold = 0.8 # Resistance to calcification
        elif dominant_category in ["kinetic", "aerobic", "photo"]:
            threshold = 0.3 # Permeability to flow/light
    ...
    def taste(word):
        ...
        if w.endswith(("ous", "ly", "y", "ish")):
            return "aerobic", 0.6
        if w.startswith(("fl", "sw", "dr", "sp")): # Flow, Swim, Drink, Spill
            return "kinetic", 0.7
        if w.endswith(("l", "r", "m", "n")) and len(w) < 6: # Pool, River, Rain
            return "aerobic", 0.5
...
class TheTensionMeter:
    ...
    def gaze(self, text):
        ...
        vocab_map = {
            ...
            for cat in ["heavy", "kinetic", "abstract", "photo", "aerobic", "thermal", "cryo", "suburban"]
        }
        for w in clean_words:
            ...
            if not found_category:
                ...
                if flavor and confidence > 0.5:
                    ...
                    unknowns.append(w) # Truly alien words
        if counts:
        ...
        if dominant_cat and context_pressure > 0.3 and unknowns:
            learning_threshold = 0.4 # Default
            ...
            if context_pressure >= learning_threshold:
                for stranger in unknowns:
                    if len(stranger) >= 3:
                        ...
                        print(f"{Prisma.CYN}🧠 OSMOSIS: Context is {dominant_cat.upper()} ({int(context_pressure*100)}%). Learned '{stranger}'.{Prisma.RST}")
    ...
    def execute(self, text):
        ...
        if cmd == "/kill":
            ...
            if len(parts) > 1:
             ...
             print(f"{Prisma.OCHRE}🔥 MANUAL IGNITION.{Prisma.RST}")
             msg = self.eng.forge.transmute(self.eng.mem.graph, self.eng.phys.vector_memory[-1] if self.eng.phys.vector_memory else [])
             if msg:
                 print(msg)
             else:
                 print(f"{Prisma.GRY}The Forge is cold. No suitable alloy found.{Prisma.RST}")
        elif cmd == "/look":
...
class LifecycleManager:
    ...
    def run_cycle(self, text, m, trace):
        ...
        if lens_name == "GORDON":
            ...
            if cleansed > 0:
                self.eng.mitochondria.mitigate(cleansed) # <--- ACTUAL SYSTEM CALL
                print(f" {Prisma.WHT}► {scrub_msg}{Prisma.RST}")
            loop_depth = int(m["physics"]["kappa"] * 10) # Map Kappa 0.0-1.0 to Depth 0-10
            is_cut, cut_msg = self.eng.gordon.cut_the_knot(loop_depth)
            if is_cut:
                m["physics"]["kappa"] = 0.0 # <--- THE HARD RESET
                m["physics"]["psi"] = 0.5   # Reset permeability
...
class TheTheremin:
    ...
    def listen(self, physics):
        ...
        if interference > 1.0:
            ...
            msg = None
        if self.banana_belly > self.FEVER_THRESHOLD * 2:
            self.banana_belly = 0.0
            self.is_stuck = False
            return False, 0.0, f"{Prisma.VIOLET}💥 BANANAFISH EXPLOSION: The Basin has been ruptured. Resetting.{Prisma.RST}"
        ...
        if abstract_density > 0.3 and voltage < 4.0:
            ...
            if not available_cats:
                if counts.get("cryo", 0) > 0:
                    available_cats = ["kinetic", "aerobic"]
                elif counts.get("thermal", 0) > 0:
                    available_cats = ["aerobic", "photo"]
                else:
                    available_cats = ["heavy", "kinetic", "aerobic"]
...
class BoneAmanita:
    ...
    def process(self, text):
        ...
        if toxin:
            ...
            self.stamina = max(0, self.stamina - 5.0) # Rejection costs energy
            return
        ...
        if status == self.mitochondria.APOPTOSIS_TRIGGER:
            if self.training_mode:
                ...
                self.mitochondria.state.atp_pool = 50.0  # Grant emergency energy
                self.mitochondria.state.ros_buildup = 0.0 # Scrub toxins
                ...
                if self.mitochondria.state.atp_pool <= BoneConfig.CRITICAL_ATP_LOW:
                    self.trauma_accum["CRYO"] += 0.5 # Starvation = Cold
                    ...
                    self.trauma_accum["BARIC"] += 0.5 # Drag = Pressure
                    print(f"{Prisma.GRY}📉 TRAUMA VECTOR: Narrative Crushing (BARIC).{Prisma.RST}")
                else:
                    self.trauma_accum["SEPTIC"] += 0.5 # Default to Poison
        ...
        if "?" in text: action_vector = "Interrogation"
        elif len(text) < 10: action_vector = "Jab"
        elif len(text) > 200: action_vector = "Pressurization"
        else: action_vector = "Statement"
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v7.6.2 'GORDON'S KNOT (SEVERED)' {Prisma.RST}")
