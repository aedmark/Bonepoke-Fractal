# BONEAMANITA 3.4 - "UNFINISHED BUSINESS"
...
class PhysicsEngine:
    def analyze(self, text):
        ...
        # [THE PARADOX ENGINE] <-- INDENT THIS BLOCK
        thermal_tension = min(counts['thermal'], counts['cryo']) * 5.0

        # Calculate Voltage (Raw Intensity)
        voltage = (counts['kinetic'] * 0.5) + (counts['heavy'] * 0.2) + (toxin_score * -1.0) + thermal_tension
...
class DeepStorage:
    def __init__(self, seed_file=None):
        ...
        # Generate a unique Session ID (Timestamp + Short Hash)
        self.session_id = f"session_{int(time.time())}"
        self.filename = f"memories/{self.session_id}.json"
        
        self.artifacts = {} # Fresh start
        
        # [CONTEXT SEEDING]
        # Only load if explicitly told to
        if seed_file:
    ...
    def save(self):
        """Archives the current session to a unique file."""
        try:
        ...
        if os.path.exists(path):
            try:
                with open(path, 'r') as f:
                    data = json.load(f)
                    self.artifacts.update(data) # Merge, don't overwrite
                print(f"{Prisma.CYN}[MEMORY]: Ingested {len(data)} artifacts from '{target_file}'.{Prisma.RST}")
            except (json.JSONDecodeError, IOError):
                print(f"{Prisma.RED}[MEMORY]: Corrupt seed file.{Prisma.RST}")
        else:
            print(f"{Prisma.RED}[MEMORY]: Seed file '{target_file}' not found.{Prisma.RST}")
...
class FrequencyModulator:
    ...
    def tune_in(self, phys, vector, is_rag_slop, atp):
        ...
        if loudest_key == 'CLARENCE': 
            ...
            if atp < 15: msg = "STARVATION PROTOCOL. Drag is fatal. Cut immediately."
            else: msg = "Drag is critical. Cut the fat."
        ...
        elif loudest_key == 'JESTER': msg = "High Voltage detected. The paradox is holding."
    ...
    def architect(self, metrics, station, is_bored):
        ...
        if station:
            ...
            if station['name'] == 'JESTER':
                return "JESTER", "Paradox sustained. Reality is bending.", "THE FROZEN FIRE"

        # THE APEIROGON
        title = self._resolve_title(vec)
        primary_dim = sorted(vec.items(), key=lambda x: x[1], reverse=True)[0][0]

        if primary_dim == 'VEL': directive = "Maintain velocity. Do not stop."
        elif primary_dim == 'ENT': directive = "High concept detected. Explore the abstraction."
        elif primary_dim == 'TEX': directive = "Heavy matter. Feel the weight."
        elif primary_dim == 'TMP': directive = "Critical Voltage. Harness the reaction."
        else: directive = "Structure is dominant. Reinforce the frame."
...
class BoneAmanita:
    ...
    def _invoke_ghost(self, current_phys):
        ...
        if last_strat == 'CLARENCE':
            if delta_drag < -0.5: msg = f"{Prisma.GRN}👻 THE BUTCHER NODS: Drag reduced.{Prisma.RST}"
            elif delta_drag > 0: msg = f"{Prisma.RED}👻 THE BUTCHER SIGHS: Drag increased.{Prisma.RST}"
        elif last_strat == 'ELOISE':
            if delta_ent < -0.1: msg = f"{Prisma.CYN}👻 ELOISE SMILES: Entropy grounded.{Prisma.RST}"
        elif last_strat == 'MICHAEL':
            if current_phys['is_whimsical']: msg = f"{Prisma.GRN}👻 THE VIBE CONTINUES.{Prisma.RST}"
    ...
    def process(self, text):
        ...
        self.last_context = {'physics': m['physics'], 'strategy': strat}

        # METABOLIC COST
        if m['physics']['narrative_drag'] > 3.0 and not lichen_msg and m['physics']['beta_friction'] < 2.0:
            self.atp -= 2
    ...
    def _render(self, m, lichen_msg, strat, reason, title, ops, station, starve_msg, ghost_msg, clean_text, did_surgery):
        ...
        atp_bar = f"{atp_color}{'|' * atp_p}{Prisma.GRY}{'.' * (10 - atp_p)}{Prisma.RST}"
        ...
        volt_c = f"{Prisma.MAG}{volt_val}{Prisma.RST}" if volt_val > 5.0 else f"{Prisma.CYN}{volt_val}{Prisma.RST}"
        ...
        print(f" {Prisma.WHT}ATP:{Prisma.RST} [{atp_bar}]   {Prisma.WHT}DRAG:{Prisma.RST} {drag_c}   {Prisma.WHT}VOLT:{Prisma.RST} {volt_c}   {Prisma.WHT}β:{Prisma.RST} {beta_c}   {Prisma.WHT}SIG:{Prisma.RST} {sig_col}{sig_name}{Prisma.RST}")
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v3.4 ONLINE.{Prisma.RST}")
