...
class MycelialDashboard:
    def __init__(self, render_mode="ANSI"): self.mode = render_mode
    def render(self, m, intv, nrg, anc, chron, arch, lich, volt=0):
        phys, sig = m['physics'], arch.get('signature', {})
        if self.mode == "ANSI":
            print(f"\n\033[96m--- MYCELIAL EKG ---\033[0m\nATP: {nrg['current_atp']} ({nrg['status']}) | DRAG: {phys['narrative_drag']}")
            print(f"VEL: {sig.get('VEL',0)} | STR: {sig.get('STR',0)} | ENT: {sig.get('ENT',0)} | TEX: {sig.get('TEX',0)} | TMP: {sig.get('TMP',0)} | BUOY: {sig.get('BUOYANCY',0)}")
            print(f"ARCH: {arch.get('archetype')} | {intv or 'STABLE'}")
        else:
            print(f"\n### 🍄 MYCELIAL EKG\n**ATP:** `{nrg['current_atp']}` ({nrg['status']}) | **DRAG:** `{phys['narrative_drag']}`")
            print(f"| VEL | STR | ENT | TEX | TMP | BUOY |\n|---|---|---|---|---|---|\n| {sig.get('VEL',0)} | {sig.get('STR',0)} | {sig.get('ENT',0)} | {sig.get('TEX',0)} | {sig.get('TMP',0)} | **{sig.get('BUOYANCY',0)}** |")
            print(f"\n**ARCH:** `{arch.get('archetype')}`\n> **STATUS:** {intv or '✅ STABLE'}")
            if lich.get('warning'): print(f"> ⚠️ {lich['warning']}")
...
class BonepokeCore:
    ...
    def generate_instruction_block(self, res):
        dirs = "\n        - ".join(res['editorial']['directives']) or "Maintain course."
        return f"""
        [BONEAMANITA 1.0 | DELUXE BUILD]
        STATE: {res['energy']['status']} | ATP: {res['energy']['current_atp']}
        ARCH: {res['archetype_data']['archetype']} | SIG: {res['archetype_data']['signature']}
        DIRECTIVES:
        - {dirs}
        """
