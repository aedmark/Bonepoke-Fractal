...
class LifecycleManager:
    ...
    def run_cycle(self, text, m, trace, has_bracelet, is_hybrid, enzyme):
        ...
        phys = self.eng.phys
        mind = self.eng.mind
        events = self.eng.events
        ghost_text = text
        if m["physics"]["voltage"] < 2.0 and m["physics"]["kappa"] > 0.8:
            ghost_text = self.eng.limbo.haunt(text)
        if ghost_text != text:
            events.log(f"🌫️ THE FOG: Timeline bleeding. Ghosts active.", "GRY")
        refusal_mode = self.eng.refusal.check_trigger(text)
        if refusal_mode:
            print(f"\n{Prisma.RED}🚫 REFUSAL TRIGGERED ({refusal_mode}){Prisma.RST}")
            if refusal_mode == "FRACTAL": print(self.eng.refusal.execute_fractal(text, m["physics"]["kappa"]))
            elif refusal_mode == "GURU_TRAP": print(self.eng.refusal.execute_guru_refusal())
            elif refusal_mode == "MIRROR": print(self.eng.refusal.execute_mirror(text))
            elif refusal_mode == "SILENT": print(self.eng.refusal.execute_silent())
            return
        damp_success, damp_msg, voltage_reduction = phys['crucible'].dampen(m["physics"]["voltage"], m["physics"]["kappa"])
        if damp_success:
            m["physics"]["voltage"] = max(0.0, m["physics"]["voltage"] - voltage_reduction)
            events.log(damp_msg, "CYN")
        if self.eng.coma_turns > 0:
            self._handle_coma(text)
            return
        chem_state = bio['endo'].metabolize(trace, self.eng.health, self.eng.stamina,
                                            ros_level=bio['mito'].state.ros_buildup,
                                            enzyme_type=enzyme)
        if chem_state["MEL"] >= 0.95:
            print(f"\n{Prisma.INDIGO}💤 CIRCADIAN CRASH: Melatonin Critical.{Prisma.RST}")
            self.eng.coma_turns = 5
            bio['endo'].melatonin = 0.0
            return
        is_stuck, interference, theremin_msg, critical_event = phys['theremin'].listen(m["physics"], bio['governor'].mode)
        if theremin_msg: events.log(theremin_msg, "OCHRE")
        is_disrupted, disrupt_msg = RuptureEngine.check_for_disruption(m["physics"], TheLexicon, self.eng.trauma_accum)

        if is_disrupted:
            events.log(disrupt_msg, "RED")
            lens_data = ("JESTER", "The Slate is Wiped. Invent something new.")
        else:
            ignition_score, constellations, current_thresh = mind['integrator'].measure_ignition(
                m["clean_words"], phys['dynamics'].voltage_history
            )
            ignition_state = "IGNITED" if ignition_score > current_thresh else "INERT"
            if ignition_state == "IGNITED":
                events.log(f"🔥 HEAP IGNITION ({int(ignition_score*100)}%): Ancestors speaking.", "OCHRE")

            lens_data = mind['chorus'].consult(m["physics"], ignition_state, is_stuck, chem_state,
                                               self.eng.gordon.inventory, self.eng.health,
                                               governor_mode=bio['governor'].mode)
        is_forged, forge_msg, new_item = phys['forge'].hammer_alloy(m["physics"])
        if is_forged:
        ...
        if is_hybrid:
            ...
            if loot_msg: print(f"\n{Prisma.MAG}🕰️ {loot_msg}{Prisma.RST}")
        elif has_bracelet:
            events.log("♾️ TIME BRACELET ACTIVE: 100% Efficiency.", "CYN")
        bloom_msg = mind['mem'].tend_garden(m["clean_words"])
        if bloom_msg: events.log(bloom_msg, "GRN")
        lens_name = lens_data[0]
        if lens_name == "MAIGRET": self._execute_maigret(lens_data)
        elif lens_name == "CARTOGRAPHER": self._execute_cartographer(m, text)
        elif lens_name == "GORDON": self._execute_gordon(m)
        elif lens_name == "GRADIENT_WALKER":
            print(f"\n{Prisma.CYN}[ GRADIENT_WALKER ]{Prisma.RST}")
            print(f" {Prisma.WHT}► {TheLexicon.walk_gradient(text)}{Prisma.RST}")
        else:
            pass
        cosmic_state, drag_mod, cosmic_msg = self.eng.cosmic.analyze_orbit(mind['mem'], m["clean_words"])
        _apply_cosmic_physics(m["physics"], cosmic_state, drag_mod)
        events.log(cosmic_msg, "MAG")
        if phys['pulse'].is_bored():
            rupture_msg = RuptureEngine.trip_the_waiter("abstract", TheLexicon)
            events.log(rupture_msg, "RED")
        self._metabolize_cycle(m, events)
        self._render(m, trace, lens_data, events)
    ...
    def _metabolize_cycle(self, m, logs):
        ...
        if lichen_msg: logs.log(lichen_msg) # Changed from append
        ...
        if density_bonus > 3.0: logs.log(f"{Prisma.OCHRE}🍽️ DENSITY GAIN (+{density_bonus:.1f}){Prisma.RST}") # Changed
        if drag_penalty > 2.0: logs.log(f"{Prisma.OCHRE}🍽️ DRAG PENALTY (-{drag_penalty:.1f}){Prisma.RST}") # Changed
        if sugar > 0:
            scrub_amt = sugar * 0.5
            self.eng.mitochondria.mitigate(scrub_amt)
            logs.log(f"{Prisma.GRN}🧼 BIO-SCRUB (-{scrub_amt:.1f} ROS){Prisma.RST}") # Changed
        ...
        if crucible_state == "RITUAL":
            ...
            logs.log(f"{Prisma.OCHRE}{crucible_msg} (+20 Stamina){Prisma.RST}") # Changed
            BoneConfig.MAX_VOLTAGE = self.eng.crucible.max_voltage_cap
        elif crucible_state == "MELTDOWN":
            self.eng.health -= crucible_val
            logs.log(f"{Prisma.RED}{crucible_msg}{Prisma.RST}") # Changed
    ...
    def _render(self, m, trace, lens_data, events_bus):
        if m["physics"]["voltage"] > 8.0:
            events_bus.log("💧 SWEATING", "CYN")
        vitals = self.eng.mitochondria.state
        chem = self.eng.endocrine.get_state()
        print(f"\n{Prisma.paint('--- [ BONEAMANITA 8.5] ---', '0')}")
        ...
        signals = {
            ...
            "logs": events_bus.flush(),
            "mode": self.eng.bio['governor'].mode}
...
class BoneAmanita:
    ...
    def process(self, text: str):
        ...
        if resp_status == "NECROSIS" or resp_status == bio['mito'].APOPTOSIS_TRIGGER:
            ...
            self.health -= 30
            return
        bio['life'].run_cycle(text, m, trace, has_bracelet, is_hybrid, enzyme)
        self.tick_count += 1

        # [STEP 10] Neuroplasticity
        chem_state = bio['endo'].get_state()
        chem_state["ATP"] = bio['mito'].state.atp_pool
        adaptation = bio['plasticity'].adapt_generation_rules(trace, chem_state)
        if adaptation:
            print(f"\n{Prisma.MAG}{adaptation}{Prisma.RST}")
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.paint('>>> BONEAMANITA 8.5', 'G')}")
