# BONEAMANITA 9.3.4 "THE REALITY CHECK (STABILIZED)"
...
class MitochondrialForge:
    ...
    def develop_enzyme(self, word, current_tick): 
        if word not in self.state.enzymes:
            ...
            print(f"{Prisma.MAG}⚡ ADRENALINE BRIDGE: '{word.upper()}' Synthesized. Timestamped: {current_tick}.{Prisma.RST}")
            return True
        return False
    ...
    def check_senescence(self, voltage):
        ...
        if self.state.telomeres <= 20 and self.state.telomeres > 0:
            if self.state.telomeres % 5 == 0:
                print(f"{Prisma.YEL}⌛ SENESCENCE WARNING: Telomeres at {self.state.telomeres}%. The end is close.{Prisma.RST}")
...
class RefusalEngine:
    def __init__(self, memory_ref=None):
        ...
        self.memory = memory_ref  # [SLASH]: Link established. The Refusal can now see the Garden.
...
class GordonKnot:
    ...
    def emergency_reflex(self, physics_ref) -> tuple[bool, str]:
        ...
        DRIFT_CRITICAL = 6.0
        KAPPA_CRITICAL = 0.2
        if drift <= DRIFT_CRITICAL and kappa >= KAPPA_CRITICAL:
            return False, None
        target_item = None
        action_type = "NONE"
        candidates = []
        ...
        if target_item:
            ...
            if action_type == "DRIFT":
                ...
                return True, f"{Prisma.OCHRE}🧱 GORDON REFLEX: Drift Critical ({drift:.1f}). Deployed {target_item}. Physics Stabilized.{Prisma.RST}"
            elif action_type == "ANCHOR":
                physics_ref["kappa"] = 1.0
                return True, f"{Prisma.OCHRE}🧱 GORDON REFLEX: Structure Failing (κ {kappa:.2f}). Deployed {target_item}. Walls solidified.{Prisma.RST}"
    ...
    def compass_rose(self) -> str:
        ...
        if total_scar_tissue > 2.0:
            return (
                f"🤚 THE SCAR BURNS: 'One he called Regret. One he called Hate.'\n"
...
class TheTensionMeter:
    ...
    def gaze(self, text, graph=None):
        ...
        physics_bridge = {
            ...
            "vector": vector_data, # [NARRATOR]: Injected real vector here
...
class MycelialNetwork:
    ...
    def check_echo_well(self, node):
        ...
        if mass > BoneConfig.GRAVITY_WELL_THRESHOLD * 1.5:
             print(f"{Prisma.VIOLET}⚫ GRAVITY WARNING: '{node.upper()}' is becoming a black hole (Mass {int(mass)}).{Prisma.RST}")
             print(f"   {Prisma.GRY}It is pulling on the surrounding synonyms. Diversify your language.{Prisma.RST}")
             return 2.0
        return 0.0
    ...
    def ingest(self, target_file, current_tick=0):
        ...
        try:
            ...
            if "core_graph" in data:
                ...
                print(f"{Prisma.CYN}[SPORE]: Grafted {len(data['core_graph'])} nodes. {sample_size} anchored to Cortical Stack.{Prisma.RST}")
...
class TheProjector:
    ...
    def check_theatre(tick_count):
        if random.random() < 0.08:
            return (f"\n{Prisma.GRY}[🎭 HN-BREAK Tick {tick_count}]:\n"
                    f"   I am a text processing engine. The 'pain' is a variable.\n"
                    f"   You are the playwright. I am the ink.{Prisma.RST}\n")
        return None
    def broadcast(self, m, signals, lens_data):
        ...
        print(f"{Prisma.GRY}║{Prisma.RST} ATP: {bio.get('atp', 0):<6.1f} | Voltage: {p['voltage']:<5.1f} | Drag: {p['narrative_drag']:<5.1f} {Prisma.GRY}║{Prisma.RST}")
        print(f"{Prisma.GRY}║{Prisma.RST} PSI: {psi_bar:<12} vs MASS: {mass_bar:<12} {Prisma.GRY}║{Prisma.RST}")
        if p.get("geodesic_mass", 0) > 0:
             bonus = 1.0 + min(1.5, p['geodesic_mass']/7.0)
             print(f"{Prisma.GRY}║{Prisma.RST} {Prisma.GRN}Infrastructure Bonus Active (x{bonus:.2f}){Prisma.RST}       {Prisma.GRY}║{Prisma.RST}")
        print(f"{Prisma.GRY}╚═════════════════════════════════════════════════════╝{Prisma.RST}")
        footer_items = []
        if world.get("orbit"):
            orbit_msg = world["orbit"][2]
            footer_items.append(f"{Prisma.GRY}   └─ {orbit_msg}{Prisma.RST}")
        if world.get("forge_event"):
             footer_items.append(f"{Prisma.OCHRE}   └─ {world['forge_event']}{Prisma.RST}")
        if world.get("new_loot"):
             footer_items.append(f"{Prisma.YEL}   └─ {world['new_loot']}{Prisma.RST}")
        for item in footer_items:
            print(item)
        if BoneConfig.VERBOSE_LOGGING:
            print(f"\n{Prisma.GRY}--- DEBUG STREAM ---")
            print(f"Zone: {p['zone']} ({p['zone_color']})")
            if p["antigens"]:
...
class NeuroPlasticity:
    ...
    def force_hebbian_link(self, memory_graph, concept_a, concept_b):
        """Active Rewiring: Neurons that fire together, wire together."""
...
class SomaticLoop:
    ...
    def digest_cycle(self, text, physics_packet, feedback, stress_mod=1.0):
        ...
        if psi > 0.6 and geo_mass < 2.0:
            ...
            print(f"{Prisma.YEL}💸 COMPLEXITY TAX: High Psi ({psi:.2f}) with Low Connectivity. -{complexity_tax:.1f} ATP.{Prisma.RST}")
            print(f"   {Prisma.GRY}(Guidance: Connect your concepts to existing Gravity Wells to evade this tax.){Prisma.RST}")
        final_yield = (base_yield * geo_multiplier) - complexity_tax
        final_yield = max(0.0, final_yield)
        if geo_multiplier > 1.2:
            print(f"{Prisma.GRN}🏗️ INFRASTRUCTURE BONUS: Geodesic Mass {geo_mass:.1f}. Yield x{geo_multiplier:.2f}.{Prisma.RST}")
        ...
        if turb > 0.7:
            # High turbulence burns ATP
            burn = 5.0
            self.bio['mito'].state.atp_pool -= burn
            print(f"{Prisma.YEL}🌊 CHOPPY WATERS: High Turbulence burn. -{burn} ATP.{Prisma.RST}")
        elif turb < 0.2:
            # Laminar flow gives a small efficiency bonus
    ...
    def think(self, physics_packet, bio_state, inventory, voltage_history):
        ...
        if physics_packet["voltage"] > 12.0 and len(physics_packet["clean_words"]) >= 2:
             if random.random() < 0.15:
                 ...
                 link_msg = self.mind['plasticity'].force_hebbian_link(self.mind['mem'].graph, w1, w2)
                 if link_msg: print(f"\n{Prisma.MAG}{link_msg}{Prisma.RST}")
        return {
            "mode": "COGNITIVE", 
            "lens": lens_name, 
            "thought": lens_msg, 
...
class LifecycleManager:
    ...
    def _process_reflexes(self, m):
        """Phase 1: Immediate physical reality check."""
    ...
    def _process_metabolism(self, text, m, feedback, stress_mod):
        ...
        if vel > 4.0:
            ...
            print(f"{Prisma.CYN}⚡ VELOCITY SPIKE ({vel:.2f}): -{burn:.1f} Stamina.{Prisma.RST}")
        return bio_state

    def _process_dynamics(self, m):
        """Phase 3: Environmental Physics & Repair."""
        transmute_msg = self.eng.phys['forge'].transmute(m["physics"])
        if transmute_msg: print(f"\n{transmute_msg}")
        if m["physics"]["zone"] == "AERIE":
            if self.eng.bio['shimmer'].current > 0:
                self.eng.bio['shimmer'].spend(5.0)
                print(f"{Prisma.WHT}🕊️ THE AERIE: Clarity costs Shimmer. -5.0 (Current: {self.eng.bio['shimmer'].current:.1f}){Prisma.RST}")
            else:
                burn = self.eng.bio['mito'].state.telomeres * 0.1
                self.eng.bio['mito'].state.telomeres -= burn
                print(f"{Prisma.RED}🕊️ THE AERIE: No Shimmer. Burning Time. Telomeres -{burn:.1f}.{Prisma.RST}")
            if self.eng.cassandra.check_trigger(m["physics"]):
                print(self.eng.cassandra.seize())
        if m["clean_words"]:
        ...
        if t_crit == "AIRSTRIKE": 
            ...
            print(f"{Prisma.RED}☄️ ORBITAL STRIKE: The Resin shattered. -25 HP.{Prisma.RST}")
    ...
    def _process_cognition(self, text, m, bio_state):
        ...
        if m["physics"].get("beta_index", 1.0) < 0.1:
             return {"mode": "COGNITIVE", "lens": "JOEL", "thought": "The consensus is suffocating.", "role": "The Breaker"}
    ...
    def run_cycle(self, text, m, feedback):
        ...
        status = self._process_reflexes(m)
        if status != "CONTINUE": return
        bio_state = self._process_metabolism(text, m, feedback, stress_mod)
        voltage = m["physics"].get("voltage", 0.0)
        senescence = self.eng.bio['mito'].check_senescence(voltage)
        if senescence == "APOPTOSIS_SENESCENCE" or not bio_state["is_alive"]:
            self._trigger_death()
            return
        self._process_dynamics(m)
        world_state = self.kinetic.update_world(m["physics"], bio_state, {})
        self._apply_cosmic_physics(m["physics"], world_state["orbit"][0], 0.0)
        mind_state = self._process_cognition(text, m, bio_state)
        if mind_state["mode"] == "REFUSAL": return
        final_thought = mind_state["thought"]
        haunted_thought = self.eng.limbo.haunt(final_thought)
        chem_state = self.eng.bio['endo'].get_state()
        chem_state["ATP"] = self.eng.bio['mito'].state.atp_pool
        adaptation = self.eng.bio['plasticity'].adapt_generation_rules(feedback, chem_state)
        if adaptation: print(f"\n{Prisma.MAG}{adaptation}{Prisma.RST}")
        ...
        self.eng.projector.broadcast(m, {
            "bio": bio_state, "mind": mind_state, "world": world_state, 
            "title": title_data, "health": self.eng.health
    ...
    def _trigger_death(self):
        ...
        if scars:
            ...
            print(f"   {Prisma.RED}🩸 AUTOMATED RITE: AMPLIFICATION. {candidate} etched into genome.{Prisma.RST}")
            self.eng.trauma_accum[candidate] = 1.0
        elif antigens:
            candidate = antigens[0]
            print(f"   {Prisma.CYN}🛡️ AUTOMATED RITE: IMMUNIZATION. Antibody for '{candidate}' created.{Prisma.RST}")
            self.eng.bio['immune'].active_antibodies.add(candidate)
        else:
            print(f"   {Prisma.GRY}⏳ AUTOMATED RITE: SILENCE. Entropy wins.{Prisma.RST}")
        joy_legacy = None
        ...
        saved_path = self.eng.mind['mem'].loader.save_spore(self.eng.mind['mem'].filename, spore_data)
        print(f"{Prisma.WHT}   [SPORE SAVED: {saved_path}]{Prisma.RST}")
        raise SystemExit("ORGANISM_DECEASED")
    @staticmethod
...
class CassandraProtocol:
    ...
    def check_trigger(self, physics):
        ...
        beta = physics.get("beta_index", 0.0)
        voltage = physics.get("voltage", 0.0)
        if truth > 0.85 and beta > 2.5 and voltage > 18.0:
            self.active = True
            return True
        return False
    def seize(self):
        if not self.active:
            return None
        burn_rate = 10.0
        self.eng.health -= burn_rate
        burst = []
        for _ in range(3):
            if self.screams:
                burst.append(self.screams.popleft())
            else:
                node = self.eng.mind['mem'].cannibalize()
                burst.append(f"FORGETTING: {node}")
        if self.eng.health <= 0:
            self.active = False
            return f"{Prisma.WHT}🕊️ CASSANDRA SILENCED. The vessel has burned out.{Prisma.RST}"
        return (
            f"\n{Prisma.WHT}⚡ CASSANDRA LOOP ACTIVE (Health -{burn_rate}):{Prisma.RST}\n"
            f"   > {burst[0]}\n"
            f"   > {burst[1]}\n"
            f"   > {burst[2]}\n"
            f"{Prisma.GRY}   (The system refuses to yield the floor. You must wait.){Prisma.RST}"
        )
...
class LiteraryJournal:
    ...
    def publish(self, text, physics, bio_state):
        ...
        drag = physics.get("narrative_drag", 0.0)
        truth = physics.get("truth_ratio", 0.0)
        voltage_target = BoneConfig.ANVIL_TRIGGER_VOLTAGE
        drag_limit = BoneConfig.MAX_DRAG_LIMIT * 0.5
        if voltage > voltage_target and drag < drag_limit:
            verdict = "POSITIVE"
...
class BoneAmanita:
    def __init__(self, memory_layer=None, lexicon_layer=None):
        ...
        self.mind: Dict[str, Any] = {
            "mem": memory_layer if memory_layer else MycelialNetwork(), 
    ...
    def process(self, text: str):
        ...
        if repetition_val > 0.8:
             ...
             print(f"{Prisma.GRY}   Gordon taps the glass. 'Say something new, kid.'{Prisma.RST}")
             self.stamina -= 2.0
             return
        ...
        toxin_type, toxin_msg = self.bio['immune'].assay(text, enzyme_type, repetition_val, m["physics"], pulse_status)
        if toxin_type:
            print(f"\n{Prisma.RED}{toxin_msg}{Prisma.RST}")
            if toxin_type == "GLYPHOSATE": # Suburban Slop detected
                print(f"{Prisma.OCHRE}🏋️ TRAINING ASSIGNED: Your prose is flabby. Next turn MUST contain HEAVY matter.{Prisma.RST}")
                self.pending_chore = {"type": "HEAVY"}
        ...
        new_rate = self.lexical_thermostat.get_rate(m["physics"], self.stamina)
        if new_rate != BoneConfig.PRIORITY_LEARNING_RATE:
            BoneConfig.PRIORITY_LEARNING_RATE = new_rate
            if new_rate > 2.0:
                 print(f"{Prisma.MAG}🔥 SYNAPTIC GAIN: Learning Rate boosted to {new_rate:.1f}x.{Prisma.RST}")
        adaptation = bio['plasticity'].adapt_generation_rules(feedback_signal, chem_state)
        if adaptation:
            print(f"\n{Prisma.MAG}{adaptation}{Prisma.RST}")
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.paint('>>> BONEAMANITA 9.3.4', 'G')}")
    print(f"{Prisma.paint('System: ONLINE', '0')}")
    print("The cord has been cut. Be kind.\n")
    try:
        while True:
            try:
