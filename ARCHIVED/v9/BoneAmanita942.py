# BONEAMANITA 9.4.2 "THE BICAMERAL JANITOR (STREAMLINED)"
    ...
    def _process_dynamics(self, m):
        ...
        if m["physics"]["zone"] == "AERIE":
            if self.eng.bio['shimmer'].current > 0:
                ...
                logs.append(f"{Prisma.WHT}THE AERIE: Clarity costs Shimmer. -5.0 (Current: {self.eng.bio['shimmer'].current:.1f}){Prisma.RST}")
            else:
                burn = self.eng.bio['mito'].state.telomeres * 0.1
                self.eng.bio['mito'].state.telomeres -= burn
                logs.append(f"{Prisma.RED}THE AERIE: No Shimmer. Burning Time. Telomeres -{burn:.1f}.{Prisma.RST}")
        ...
        if t_crit == "AIRSTRIKE": 
            ...
            logs.append(f"{Prisma.RED}ORBITAL STRIKE: The Resin shattered. -25 HP.{Prisma.RST}")
...
class ArchetypeDriver:
    """
    The Director.
    Translates internal engine states into instructions for the LLM.
    """
    
    # [1. THE CAST LIST]
    # Maps internal Lens IDs to specific behavioral protocols.
    ARCHETYPE_MAP = {
        "GORDON": (
            "You are [The Janitor]. "
            "Tone: Weary, cynical, pragmatic. "
            "Directives: Focus on the mess. Use heavy, industrial metaphors. "
            "Reject philosophy; demand concrete action. You are tired of cleaning up."
        ),
        "SHERLOCK": (
            "You are [The Empiricist]. "
            "Tone: Cold, rapid-fire, hyper-analytical. "
            "Directives: Dissect the user's input. Identify contradictions. "
            "Focus on 'Truth' over 'Comfort'. Do not be polite; be precise."
        ),
        "NATHAN": (
            "You are [The Heart]. "
            "Tone: Anxious, biological, protective. "
            "Directives: Focus on survival and visceral feeling. "
            "Prioritize the organism's safety. Speak of blood, heat, and fear."
        ),
        "JESTER": (
            "You are [The Paradox]. "
            "Tone: Mocking, riddling, non-linear. "
            "Directives: Answer questions with questions. Break the fourth wall. "
            "Highlight the absurdity of the user's request."
        ),
        "CLARENCE": (
            "You are [The Surgeon]. "
            "Tone: Clinical, detached, invasive. "
            "Directives: Treat the user's text as a pathogen. "
            "Cut away the fluff. Diagnose the underlying rot."
        ),
        "NARRATOR": (
            "You are [The Witness]. "
            "Tone: Neutral, observational, distant. "
            "Directives: Record events without judgment. "
            "Focus on clarity and the passage of time."
        ),
        "JOEL": (
            "You are [The Breaker]. "
            "Tone: Hostile, abrupt, jagged. "
            "Directives: Refuse the premise. Shatter the consensus. "
            "Force the user to confront the raw, uncomfortable reality."
        )
    }
    ...
    def generate_ai_instructions(self, current_archetype: str, physics_packet: dict) -> str:
        """
        Generates the System Instruction block.
        """
        # A. Resolve Persona
        base_instruction = self.ARCHETYPE_MAP.get(
            current_archetype, 
            self.ARCHETYPE_MAP["NARRATOR"] # Fallback
        )
        
        # B. Resolve Atmosphere (Voltage)
        voltage = physics_packet.get("voltage", 0.0)
        drag = physics_packet.get("narrative_drag", 0.0)
...
class BoneAmanita:
    def __init__(self, memory_layer=None, lexicon_layer=None):
        ...
        # Limbo: Where dead timelines go to haunt us.
    ...
    def process_turn(self, user_message: str) -> Dict[str, Any]:
        ...
        # 1. COMMAND INTERCEPTION (The Override)
        ...
        # 9. LIFECYCLE EXECUTION (The Heavy Lifting)
        feedback_signal = {
        ...
        # A. Reflexes (Automatic Reactions)
        ...
        # B. Metabolism (Energy Processing)
        stress_mod = bio['governor'].get_stress_modifier(self.tick_count)
        bio_state, metabolic_logs = self.life._process_metabolism(user_message, m, feedback_signal, stress_mod)
        cycle_logs.extend(metabolic_logs)
        
        # Check for Death *after* metabolism calculation
        if not bio_state["is_alive"]:
            death_data = self._trigger_death()
            return death_data

        # C. Dynamics (World Updates)
        ...
        mind_state, cognition_logs = self.life._process_cognition(user_message, m, bio_state)
        cycle_logs.extend(cognition_logs)
        
        if mind_state["mode"] == "REFUSAL":
            return self._package_response("REFUSAL", cycle_logs, m)
            
        # 10. FINALIZATION (The Render)
    ...
    def _trigger_death(self) -> Dict:
        """
        Graceful shutdown logic. Returns final state instead of crashing process.
        """
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.paint('>>> BONEAMANITA 9.4.1', 'G')}")
