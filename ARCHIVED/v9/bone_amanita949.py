# BONEAMANITA 9.4.9 "THE CURTAIN CALL"
...
class BoneAmanita:
    def __init__(self, memory_layer=None, lexicon_layer=None):
        # [SYSTEM INITIALIZATION - Same as before]
        ...
        # 5. THE LOOPS (Dependency Injected)
    ...
    def process_turn(self, user_message: str) -> Dict[str, Any]:
        """
        The Main Loop. Refactored for Linear Clarity (The Pinker Standard).
        Flow: Input -> Command Check -> Physics -> Environment -> Biology -> Cognition -> Output.
        """
        self.events.flush() # Clear buffer
        
        # --- 1. COMMAND INTERCEPTION ---
        if user_message.strip().startswith("/"):
            self.cmd.execute(user_message)
            return self._quick_response("COMMAND", [e['text'] for e in self.events.flush()])

        # --- 2. PHYSICS CALCULATION (The Tension) ---
        ...
        # 2a. Navigation & Anomaly Check
        logs = []
        current_manifold = self.navigator.locate(phys)
        if self.navigator.check_anomaly(user_message):
            logs.append(f"{Prisma.VIOLET}ANOMALY: Timeline Divergence.{Prisma.RST}")
            phys["voltage"] = 99.9; phys["narrative_drag"] = 0.0
        else:
            logs.append(f"{Prisma.GRY}[NAV]: {current_manifold} | Shimmer: {self.shimmer_state.current:.1f}{Prisma.RST}")
        self.shimmer_state.recharge(0.5)

        # 2b. Memory Encoding
        if self.mind['mem'].encode(context["clean_words"], phys, self.bio['governor'].mode):
            self.events.log(f"{Prisma.MAG}ENGRAM FORMED.{Prisma.RST}", "MEM")
        
        # 2c. Safety Overrides (Fever/Governor)
        phys["psi"] = self.bio['governor'].psi_mod
        if self.bio['governor'].mode == "FORGE": phys["narrative_drag"] = 0.0
        if self.safety.fever_dream_active:
            phys["voltage"] = 99.9; phys["narrative_drag"] = 0.0; phys["kappa"] = 0.0

        # --- 3. ENVIRONMENTAL CHECKS (The Gatekeepers) ---
        # 3a. Preserves
        active_preserves, preserve_logs = self.preserves.check_preserves(phys)
        logs.extend([f"{Prisma.GRN}{l}{Prisma.RST}" for l in preserve_logs])
        
        # 3b. Tangibility Gate (Unless in a Preserve)
        if not active_preserves:
            passed, customs_msg = self.phys['gate'].weigh(phys, self.stamina)
            if not passed:
                logs.append(f"\n{customs_msg}")
                self.stamina = max(0.0, self.stamina - 2.0)
                if self.stamina < 10.0: logs.append(f"\n{self.limbo.trigger_stasis_failure(user_message)}")
                return self._package_turn("REFUSAL", logs, context)
            if customs_msg: logs.append(f"\n{customs_msg}")

        # 3c. Repetition & Toxin
        phys["repetition"] = self.phys['pulse'].check_pulse(context["clean_words"])
        toxin_type, toxin_msg = self.bio['immune'].assay(user_message, "NARRATIVE", phys["repetition"], phys, self.phys['pulse'].get_status())
        if toxin_type:
            logs.append(f"\n{Prisma.RED}{toxin_msg}{Prisma.RST}")
            if toxin_type in ["AMANITIN", "CYANIDE_POWDER"]:
                self.health -= 20.0; self.mind['mem'].cannibalize()
                return self._package_turn("TOXICITY", logs, context)

        # --- 4. BIOLOGICAL METABOLISM (The Flesh) ---
        # 4a. Feedback loop for hormones
        feedback = {
        ...
        # 4b. Run the Somatic Loop
        stress_mod = self.bio['governor'].get_stress_modifier(self.tick_count)
        
        # Schur Note: We fired the LifecycleManager. We call the stomach directly.
        bio_result = self.soma.digest_cycle(user_message, phys, feedback, stress_mod, self.tick_count)
        logs.extend(bio_result["logs"])
        
        if not bio_result["is_alive"]:
            return self._trigger_death(phys)
            
        # 4c. Update physical stats based on activity
        vel = self.phys['dynamics'].get_velocity()
        if vel > 4.0:
            burn = vel * 1.5; self.stamina -= burn
            logs.append(f"VELOCITY SPIKE ({vel:.2f}): -{burn:.1f} Stamina.")

        # --- 5. WORLD DYNAMICS (The Environment) ---
        # 5a. Pareidolia & Parasites
        is_face, p_msg = self.check_pareidolia(context["clean_words"]) # Static method call
        if is_face: logs.append(f"\n{p_msg}"); phys["psi"] = min(1.0, phys["psi"] + 0.3)
        
        parasite_active, p_log = self.bio['parasite'].infect(phys, self.stamina)
        if parasite_active: logs.append(f"\n{p_log}")
        
        # 5b. The Theremin & Crucible
        is_stuck, _, t_msg, t_crit = self.phys['theremin'].listen(phys, self.bio['governor'].mode)
        if t_msg: logs.append(f"\n{t_msg}")
        ...
        c_state, c_val, c_msg = self.phys['crucible'].audit_fire(phys)
        if c_msg: logs.append(f"\n{c_msg}")
        if c_state == "MELTDOWN": self.health -= c_val

        # 5c. Gordon's Reflexes (The Janitor)
        drift_delta, grav_msg = self.gordon.check_gravity(phys["narrative_drag"], phys["psi"])
        if drift_delta != phys["narrative_drag"]: phys["narrative_drag"] = drift_delta; logs.append(f"\n{grav_msg}")
        
        is_pain, pain_msg = self.gordon.flinch(context["clean_words"])
        if is_pain: return self._package_turn("PAIN_INTERRUPT", logs + [f"\n{pain_msg}"], context)
        
        g_active, g_log = self.gordon.emergency_reflex(phys)
        if g_active: logs.append(f"\n{g_log}")

        # --- 6. COGNITION (The Mind) ---
        # 6a. Rupture & Perfection Checks
        rupture, r_msg = RuptureEngine.check_for_disruption(phys, self.mind['lex'], self.trauma_accum)
        if rupture: logs.append(f"\n{r_msg}"); phys["voltage"] += 5.0; phys["beta_index"] = 0.0

        is_perfect, s_msg, _ = RuptureEngine.audit_perfection(phys, self.mind['lex'])
        if is_perfect: logs.append(f"\n{s_msg}"); self.health -= 15.0; phys["narrative_drag"] = 0.0

        # 6b. The Noetic Loop (Thinking)
        mind_state = self.noetic.think(phys, bio_result, self.gordon.inventory, self.phys['dynamics'].voltage_history)
        
        if mind_state["mode"] == "REFUSAL":
            # Expand refusal details
            trig = mind_state.get("trigger")
            if trig == "GURU_TRAP": logs.append(f"\n{self.refusal.execute_guru_refusal()}")
            elif trig == "SILENT": logs.append(f"\n{self.refusal.execute_silent()}")
            elif trig == "MIRROR": logs.append(f"\n{self.refusal.execute_mirror(user_message)}")
            elif trig == "FRACTAL": logs.append(f"\n{self.refusal.execute_fractal(user_message)}")
            else: logs.append(f"\n{self.refusal.manifest_glitch(user_message, self.mind['mem'])}")
            return self._package_turn("REFUSAL", logs, context)

        # --- 7. FINAL RENDER & METRICS ---
        self.tick_count += 1
        
        # Calculate Title/Archetype for UI
    ...
    def _quick_response(self, type_str, logs):
        return {"type": type_str, "ui": "\n".join(logs), "logs": logs, "metrics": self._get_metrics()}
    ...
    def _phase_simulation_loop(self, user_message, m, cycle_logs):
        ...
        Updated to call loops DIRECTLY, bypassing LifecycleManager.
        """
        phys = m["physics"]
        
        # 1. Feedback Construction
        feedback_signal = {
            "STATIC": max(phys["repetition"], phys["counts"]["antigen"] * 0.2),
            "INTEGRITY": phys["truth_ratio"],
            "FORCE": min(1.0, phys["voltage"] / 10.0), 
        ...
        # 3. Reflexes (Directly via Gordon/Lexicon)
        # We moved the 'ReflexSystem' logic here for transparency.
        if self.tick_count % 10 == 0:
            self.mind['lex'].atrophy(self.tick_count, max_age=max(15, 30 - int(phys.get("voltage", 0.0))))
        ...
        is_pain, pain_msg = self.gordon.flinch(m["clean_words"])
        if is_pain: return self._package_response("PAIN_INTERRUPT", cycle_logs + [f"\n{pain_msg}"], m)
        
        g_active, g_log = self.gordon.emergency_reflex(phys)
        if g_active: cycle_logs.append(f"\n{g_log}")

        # 4. Metabolism (Direct call to SomaticLoop)
        stress_mod = self.bio['governor'].get_stress_modifier(self.tick_count)
        
        # THE FIX: Calling digest_cycle directly with the right args
        ...
        # 5. Dynamics (Direct calls)
        # Note: We manually handle the "Cost of Clarity" here or inside SomaticLoop.
        # Let's keep the Velocity Burn here as it relates to the whole body moving.
        vel = self.phys['dynamics'].get_velocity()
        if vel > 4.0:
            burn = vel * 1.5
            self.stamina -= burn
        ...
        # Environment Logic (extracted from EnvironmentalSystem)
        transmute_msg = self.phys['forge'].transmute(phys)
        if transmute_msg: cycle_logs.append(f"\n{transmute_msg}")
        
        is_pareidolia, p_msg = self.check_pareidolia(m["clean_words"])
        if is_pareidolia:
            cycle_logs.append(f"\n{p_msg}")
            phys["psi"] = min(1.0, phys["psi"] + 0.3)
        ...
        # 6. Cognition (Direct call to NoeticLoop)
        ...
        if mind_state["mode"] == "REFUSAL":
            # [Keep your existing refusal handling logic here]
            return self._package_response("REFUSAL", cycle_logs, m)

        # 7. World Update (Direct call to KineticLoop)
        world_state = self.kinetic.update_world(phys, bio_result, mind_state)
        self._apply_cosmic_physics(phys, world_state["orbit"][0], 0.0)

        return self._finalize_turn(m, bio_result, mind_state, world_state, cycle_logs)
...
class SessionGuardian:
    ...
    def __enter__(self):
        print(f"{Prisma.paint('>>> BONEAMANITA 9.4.9', 'G')}")
