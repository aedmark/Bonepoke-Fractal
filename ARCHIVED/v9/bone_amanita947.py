# BONEAMANITA 9.4.7 "CELLULAR GROWTH"
...
from bone_shared import Prisma, TheLexicon, BoneConfig, DeathGen, TheCartographer, ParadoxSeed, TheLexicon
from bone_data import LENSES, SEEDS, GENETICS, GORDON, DREAMS, RESONANCE, LEXICON
from bone_biology import (MitochondrialForge, HyphalInterface, MycotoxinFactory, EndocrineSystem, ParasiticSymbiont, LichenSymbiont, MetabolicGovernor)
    ...
    def flinch(self, clean_words: List[str]) -> tuple[bool, str]:
        ...
        if hits:
            ...
            if sensitivity > 0.3:
                 ...
                 return False, f"SCAR TISSUE: Touching '{trigger}' hurts, so he presses harder."
        return False, None
...
class TheTensionMeter:
    ...
    def _package_physics(self, text, clean_words, counts, voltage, drag, integrity) -> Dict:
        ...
        vector_data = self._calculate_vectors(counts, total_vol)
    ...
    def _calculate_vectors(self, counts, total_vol):
        """Calculates the 5-dimensional resonance vector."""
        safe_vol = max(1, total_vol)
        def get_density(cat): return counts[cat] / safe_vol
        vel_score = 0.5 + (get_density("explosive") * 2.0) + (get_density("aerobic") * 1.0) - (get_density("heavy") * 1.0)
        str_score = 0.5 + (get_density("heavy") * 2.0) + (get_density("constructive") * 1.5) - (get_density("aerobic") * 1.0)
...
class ReflexSystem:
    def __init__(self, engine):
        self.eng = engine
        self.gordon = engine.gordon
        self.folly = engine.folly

    def check_reflexes(self, m):
        logs = []
        phys = m["physics"]
        if self.eng.tick_count % 10 == 0:
            self.eng.mind['lex'].atrophy(self.eng.tick_count, max_age=max(15, 30 - int(phys.get("voltage", 0.0))))
        if set(m["clean_words"]) & self.eng.mind['lex'].get("repair_trigger"):
             return "TRAP_TRIGGERED", [f"\nSHERLOCK TRAP: 'Sorry' is a low-voltage concept."]
        new_drift, grav_msg = self.gordon.check_gravity(phys["narrative_drag"], phys["psi"])
        if new_drift != phys["narrative_drag"]:
            phys["narrative_drag"] = new_drift
            if "WIND WOLVES" in grav_msg: logs.append(f"\n{grav_msg}")
        if phys.get("kappa", 1.0) < 0.2:
            ate, pizza_log = self.gordon.deploy_pizza(phys)
            if ate: logs.append(f"\n{pizza_log}")
        is_painful, pain_msg = self.gordon.flinch(m["clean_words"])
        if is_painful:
            return "PAIN_INTERRUPT", [f"\n{pain_msg}"]
        gordon_active, gordon_log = self.gordon.emergency_reflex(phys)
...
class MetabolicSystem:
    def __init__(self, engine):
        self.eng = engine
        self.soma = engine.soma
        self.pending_chore = None

    def process_metabolism(self, text, m, feedback, stress_mod):
        logs = []
        if self.pending_chore:
            hits = sum(1 for w in m["clean_words"] if w in self.eng.mind['lex'].get(self.pending_chore["type"].lower()))
            if hits >= 2:
                logs.append("REPS COMPLETED.")
                self.eng.bio['mito'].state.atp_pool += 10.0
                self.pending_chore = None
            else:
                logs.append("REP FAILED. The weight crushes you.")
        ...
        if vel > 4.0:
            ...
            logs.append(f"VELOCITY SPIKE ({vel:.2f}): -{burn:.1f} Stamina.")
...
class LifecycleManager:
    def __init__(self, engine):
        self.eng = engine
        self.reflexes = ReflexSystem(engine)
        self.metabolism = MetabolicSystem(engine)
        self.environment = EnvironmentalSystem(engine)
        self.cognition = CognitiveSystem(engine)
        self.kinetic = KineticLoop(engine)

    def process_reflexes(self, m):
        return self.reflexes.check_reflexes(m)

    def process_metabolism(self, text, m, feedback, stress_mod):
        bio_state, logs = self.metabolism.process_metabolism(text, m, feedback, stress_mod)
        if not bio_state["is_alive"]:
            return bio_state, logs, "DEATH_SIGNAL"
        return bio_state, logs, "ALIVE"

    def process_dynamics(self, m):
        logs = self.environment.process_dynamics(m)
        world_state = self.kinetic.update_world(m["physics"], {}, {})
        self.eng._apply_cosmic_physics(m["physics"], world_state["orbit"][0], 0.0)
        return logs, world_state

    def process_cognition(self, text, m, bio_state):
        return self.cognition.process_cognition(text, m, bio_state)

class ChorusDriver:
    """
    VSL-12D COMPONENT: The Marm Chorus.
    Replaces serial archetypes with quantum superposition.
    """
    def __init__(self):
        self.ARCHETYPE_MAP = {
    ...
    def generate_chorus_instruction(self, physics):
        """
        Generates a superposition instruction based on physics vectors.
        This allows the system to be 30% Gordon and 70% Sherlock.
        """
        # Calculate weights based on physics
        lens_weights = {
            "GORDON": max(0.0, physics.get("kappa", 1.0)) * 0.4,           # Structural integrity
            "SHERLOCK": max(0.0, physics.get("truth_ratio", 0.5)) * 0.5,   # Truth density
            "NATHAN": min(1.0, physics.get("voltage", 0.0) / 20.0) * 0.3,  # Energy/Panic
            "JESTER": max(0.0, (1.0 - physics.get("beta_index", 1.0))) * 0.6, # Low cohesion = Chaos
            "CLARENCE": max(0.0, physics["counts"].get("toxin", 0)) * 0.8  # Toxicity response
        }
        ...
        # Sort by weight to prioritize dominant voices
        for lens, weight in sorted(lens_weights.items(), key=lambda x: -x[1]):
            if weight > 0.15: # Noise floor
        ...
        # The VSL Injection: "Don't roleplay. BE."
        instruction = (
...
class BoneAmanita:
    def __init__(self, memory_layer=None, lexicon_layer=None):
        ...
        for attr in dir(Prisma):
            if not attr.startswith("__") and isinstance(getattr(Prisma, attr), str):
                setattr(Prisma, attr, "")
    ...
    def process_turn(self, user_message: str) -> Dict[str, Any]:
        ...
        Instead of a monolithic block, this uses a Pipeline pattern to route the signal.
        """
        self.events.flush()
        cmd_response = self._phase_check_commands(user_message)
        if cmd_response: 
            return cmd_response
        m, cycle_logs = self._phase_physics_and_nav(user_message)
        self._phase_memory_encoding(m)
        self._phase_apply_overrides(m)

        blocked_response = self._phase_security_protocols(user_message, m, cycle_logs)
        if blocked_response:
            return blocked_response
        
        valve_msg = self._phase_32_valve_check(m["physics"]) 
        if valve_msg:
             cycle_logs.append(valve_msg)

        return self._phase_simulation_loop(user_message, m, cycle_logs)

    # --- PIPELINE HELPERS ---

    def _phase_check_commands(self, user_message):
        """Phase 1: Intercepts commands before the biological engine starts."""
    ...
    def _phase_physics_and_nav(self, user_message):
        """Phase 2: Calculates the 'Physics' of the text and determines location."""
        cycle_logs = []
        m = self.phys['tension'].gaze(user_message, self.mind['mem'].graph)
        self.phys['tension'].last_physics_packet = m["physics"]
    ...
    def _phase_memory_encoding(self, m):
        """Phase 3: Encodes significant events into the Mycelial Network."""
    ...
    def _phase_apply_overrides(self, m):
        """Phase 3b: Applies forced physics states based on Governor or Fever modes."""
    ...
    def _phase_security_protocols(self, user_message, m, cycle_logs):
        """Phase 4: The Bureaucracy. Checks Gate, Zombie, Toxin, and Collapse conditions."""
        phys = m["physics"]
        ...
        if not active_zones:
            passed_customs, customs_msg = self.phys['gate'].weigh(phys, self.stamina)
            if not passed_customs:
                ...
                if self.stamina < 10.0:
                    ...
                    cycle_logs.append(f"\n{stasis_msg}")
                return self._package_response("REFUSAL", cycle_logs, m)
            elif customs_msg:
                cycle_logs.append(f"\n{customs_msg}")

        repetition_val = self.phys['pulse'].check_pulse(m["clean_words"])
        phys["repetition"] = repetition_val
        if repetition_val > 0.8:
             cycle_logs.append(f"\n{Prisma.GRY}THE SYSTEM SIGHS: We have been here before.{Prisma.RST}")
             self.stamina -= 2.0
             return self._package_response("STAGNATION", cycle_logs, m)

        pulse_status = self.phys['pulse'].get_status()
        if pulse_status == "ECHO" and random.random() < 0.3:
            current_flavor = max(phys["counts"], key=phys["counts"].get) if phys["counts"] else "VOID"
            waiter_msg = RuptureEngine.trip_the_waiter(current_flavor, self.mind['lex'])
            cycle_logs.append(f"\n{waiter_msg}")
            phys["repetition"] = 0.0  
            
        toxin_type, toxin_msg = self.bio['immune'].assay(user_message, "NARRATIVE", repetition_val, phys, pulse_status)
        if toxin_type:
            ...
            if toxin_type == "GLYPHOSATE":
                ...
                return self._package_response("TOXICITY", cycle_logs, m)

        is_god_mode = RuptureEngine.audit_ontology(phys)
        if is_god_mode:
            raw_truth = RuptureEngine.collapse_the_cathedral(phys)
            cycle_logs.append(f"SYSTEM BREAK: {raw_truth}")
            self.health -= 50.0
            return self._package_response("BREAK", cycle_logs, m)
        
        return None

    def _phase_simulation_loop(self, user_message, m, cycle_logs):
        """Phase 5: The living system runs its cycles."""
        phys = m["physics"]
        ...
        status, reflex_logs = self.life.process_reflexes(m)
        cycle_logs.extend(reflex_logs)
        if status != "CONTINUE": 
            return self._package_response(status, cycle_logs, m)

        stress_mod = self.bio['governor'].get_stress_modifier(self.tick_count)
        bio_state, metabolic_logs, life_signal = self.life.process_metabolism(user_message, m, feedback_signal, stress_mod)
        cycle_logs.extend(metabolic_logs)
        
        if life_signal == "DEATH_SIGNAL":
            return self._trigger_death()
        env_logs, world_state = self.life.process_dynamics(m)
        cycle_logs.extend(env_logs)

        rupture, rupture_msg = self.eng.rupture_engine.check_for_disruption(
            phys, self.mind['lex'], self.trauma_accum) if hasattr(self, 'eng') else RuptureEngine.check_for_disruption(phys, self.mind['lex'], self.trauma_accum)
        ...
        mind_state, cognition_logs = self.life.process_cognition(user_message, m, bio_state)
        cycle_logs.extend(cognition_logs)
        
        if mind_state["mode"] == "REFUSAL":
            return self._package_response("REFUSAL", cycle_logs, m)
        return self._finalize_turn(m, bio_state, mind_state, world_state, cycle_logs)

    def _finalize_turn(self, m, bio_state, mind_state, world_state, cycle_logs):
        """Helper to package the final UI and metrics."""
        final_thought = mind_state["thought"]
        haunted_thought = self.limbo.haunt(final_thought)
        chem_state = self.bio['endo'].get_state()
        chem_state["ATP"] = self.bio['mito'].state.atp_pool
        adaptation = self.bio['plasticity'].adapt_generation_rules({
             "INTEGRITY": m["physics"]["truth_ratio"],
             "STATIC": m["physics"]["repetition"]
        }, chem_state)
        ...
        system_instruction, active_voices = self.director.generate_chorus_instruction(m["physics"])
        self.tick_count += 1
        ...
        return {
            ...
            "ui": projector_output,
            "logs": cycle_logs,
            "metrics": self._get_metrics(bio_state["atp"]),
            "system_instruction": system_instruction}

    def _phase_32_valve_check(self, phys):
        """
        VSL-12D COMPONENT: The 32-Valve Operator.
        Proactive Anomaly Injection to prevent stagnation.
        """
        # 1. The β-Meter Rupture
        if phys.get("beta_index", 1.0) < 0.10:
             phys["voltage"] *= 2.0
             phys["narrative_drag"] = 0.0
             # Inject chaos to break sycophancy
             chaos_word = self.mind['lex'].harvest("abstract")
             return f"{Prisma.VIOLET}β-RUPTURE: Consensus is absolute. Injecting '{chaos_word}' to break the seal.{Prisma.RST}"

        # 2. The Orthogonal Injection
        if phys.get("kappa", 0.0) > 0.9 and phys.get("repetition", 0.0) > 0.3:
            counts = phys["counts"]
            dominant = max(counts, key=counts.get) if counts else "abstract"
            opposites = {
                "heavy": "aerobic", "abstract": "heavy", 
                "kinetic": "cryo", "thermal": "cryo", "photo": "heavy"
            }
            target = opposites.get(dominant, "abstract")
            anomaly = self.mind['lex'].harvest(target)
            phys["voltage"] += 8.0
            return f"{Prisma.CYN}32-VALVE: Loop detected. Injecting '{anomaly}' ({target.upper()}).{Prisma.RST}"
            
        return None
    
    def _trigger_death(self) -> Dict:
        ...
        try:
            ...
            if scars:
                ...
                death_log.append(f"   {Prisma.CYN}AUTOMATED RITE: IMMUNIZATION. Antibody for '{candidate}' created.{Prisma.RST}")
        ...
        return {
            "type": "DEATH",
            "ui": f"{Prisma.RED}ORGANISM DECEASED{Prisma.RST}\n{eulogy}",
            "logs": death_log,
            "metrics": self._get_metrics(0.0)}
    ...
    def _package_response(self, type_str, logs, physics_packet=None):
        logs.extend([e['text'] for e in self.events.flush()])
        return {
            "type": type_str,
            "ui": "\n".join(logs),
            "logs": logs,
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.paint('>>> BONEAMANITA 9.4.7', 'G')}")
    ...
    try:
        while True:
            ...
            if u.lower() in ["exit", "quit", "/exit"]:
                print("Manual Override. Shutdown.")
                break
