# BONEAMANITA 9.5.6 "The Twelfth Dimension (TIDY)"
...
class TherapyProtocol:
    def __init__(self):
        ...
        self.HEALING_THRESHOLD = 6

    def check_progress(self, phys, stamina, current_trauma_accum):
        healed_types = []
        voltage = phys.get("voltage", 0.0)
        truth = phys.get("truth_ratio", 0.0)
        criteria = {
            "SEPTIC": (phys["counts"]["toxin"] == 0 and phys["vector"]["TEX"] > 0.3),
            "CRYO": (stamina > 30 and phys["counts"]["photo"] > 0),
            "THERMAL": (2.0 <= voltage <= 8.0), # Controlled burn, not explosion
            "BARIC": (phys["narrative_drag"] < 2.0 and phys["vector"]["VEL"] > 0.4)
        }
        is_breakthrough = (truth > 0.85 and voltage > 10.0)
        for trauma_type, condition_met in criteria.items():
            if condition_met or is_breakthrough:
                gain = 2 if is_breakthrough and condition_met else 1
                self.streaks[trauma_type] += gain
            else:
                self.streaks[trauma_type] = max(0, self.streaks[trauma_type] - 1)
            if self.streaks[trauma_type] >= self.HEALING_THRESHOLD:
                self.streaks[trauma_type] = 2
                if current_trauma_accum[trauma_type] > 0.001:
                    heal_amount = 0.8 if is_breakthrough else 0.5
                    current_trauma_accum[trauma_type] = max(
                        0.0, current_trauma_accum[trauma_type] - heal_amount)
...
class GeodesicOrchestrator:
    ...
    def run_turn(self, user_message: str) -> Dict[str, Any]:
        self.eng.events.flush() # Clear buffer

        # --- NODE 1: OBSERVE (Physics) ---
        gaze_result = self.eng.phys.tension.gaze(user_message, self.eng.mind.mem.graph)
        ...
        toxin_type, toxin_msg = self.eng.bio.immune.assay(
            user_message, "NARRATIVE", repetition_val, physics, pulse_status)
        if toxin_type:
            logs.append(f"{Prisma.RED}{toxin_msg}{Prisma.RST}")
            if toxin_type in ["AMANITIN", "CYANIDE_POWDER"]:
                self.eng.health -= 20.0
                scar_log = self.eng.gordon.learn_scar(clean_words, 20.0)
                if scar_log: logs.append(scar_log)
                return self.eng._package_turn("TOXICITY", logs, {"physics": physics})
        semantic_refusal = self.vsl_semantic.audit(user_message)
        if semantic_refusal:
            return self.eng._package_turn("REFUSAL", logs + [semantic_refusal], {"physics": physics})

        # --- NODE 4: VSL AUDIT ---
        # C. Entropy Vent
        vent_msg = self.vsl_vent.check(physics)
        if vent_msg:
            return self.eng._package_turn("REFUSAL", logs + [vent_msg], {"physics": physics})

        is_ruptured, rupture_msg = RuptureEngine.check_for_disruption(physics, self.eng.lex, self.eng.trauma_accum)
        if is_ruptured:
            return self.eng._package_turn("RUPTURE", logs + [rupture_msg], {"physics": physics})

        hubris_hit, hubris_msg, _ = RuptureEngine.audit_perfection(physics, self.eng.lex)
        if hubris_hit:
            self.eng.health -= 15.0
            logs.append(hubris_msg)

        # D. 32-Valve Rupture
        rupture = self.vsl_32v.audit(physics)
        if rupture:
            logs.append(rupture["log"])
        ...
        bio_result = self.eng.soma.digest_cycle(
            user_message, physics, bio_feedback, stress_mod, self.eng.tick_count
        )
        critical_bio_logs = [l for l in bio_result["logs"] if "CRITICAL" in l or "TAX" in l or "Poison" in l]
        logs.extend(critical_bio_logs)
        ...
        if is_cracked:
            logs.append(f"{Prisma.YEL}🏺 KINTSUGI ACTIVATED: The vessel is cracking (Stamina Low).{Prisma.RST}")
        ...
        if self.eng.kintsugi.active_koan:
            ...
            if repair_result and repair_result["success"]:
                ...
                logs.append(f"   {Prisma.GRY}{repair_result['detail']}{Prisma.RST}")
        ...
        if healed_list:
            joined_heals = ", ".join(healed_list)
            logs.append(f"{Prisma.GRN}❤️ THERAPY STREAK: Consistency detected. Healing [{joined_heals}].{Prisma.RST}")
        ...
        if mind_state["mode"] == "REFUSAL":
            return self.eng._package_turn("REFUSAL", logs + [mind_state.get("trigger", "REFUSAL")], {"physics": physics})
...
class SessionGuardian:
    ...
    def __enter__(self):
        print(f"{Prisma.paint('>>> BONEAMANITA 9.5.6', 'G')}")
