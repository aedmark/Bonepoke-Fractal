...
class GordonKnot:
    ...
    def get_item_data(self, item_name):
        return self.ITEM_REGISTRY.get(item_name, {
            "description": "Unknown Artifact", "function": "NONE", "usage_msg": "It does nothing."
        })
    ...
    def _effect_bureaucratic_anchor(self, p, data, item_name):
        # The Red Stapler increases Consensus (Beta) but adds Drag
    ...
    def _effect_luminescence(self, p, data, item_name):
        # Fireflies add 'photo' count silently
        # This triggers Photosynthesis in the LichenSymbiont!
        p["counts"]["photo"] = p["counts"].get("photo", 0) + 2
        return None # Silent buff, or: f"{Prisma.CYN}..blink..{Prisma.RST}"

    def _effect_grounding_gear(self, p, data, item_name):
        # Lead Boots prevent you from entering the VOID or AERIE
    ...
    def _effect_safety_scissors(self, p, data, item_name):
        # Cuts 'suburban' words (adjectives/filler)
        suburban = p["counts"].get("suburban", 0)
        if suburban > 2:
            p["counts"]["suburban"] = 0
    ...
    def check_gravity(self, current_drift: float, psi: float) -> tuple[str | Any, str] | tuple[float, str] | tuple[
        float, None]:
        for item in self.inventory:
            data = self.get_item_data(item)
    ...
    def emergency_reflex(self, physics_ref) -> tuple[bool, str] | tuple[bool, None]:
        for item in self.inventory:
            data = self.get_item_data(item)
            ...
            if trigger_key and trigger_key in self.REFLEX_MAP:
                ...
                if condition_met:
                    ...
                    if func == "ENTROPY_BUFFER": # Quantum Gum
                        self.inventory.remove(item)
    ...
    def deploy_pizza(self, physics_ref, item_name="STABILITY_PIZZA") -> tuple[bool, str]:
        ...
        if data.get("consume_on_use"):
            if item_name in self.inventory: self.inventory.remove(item_name)
    ...
    def flinch(self, clean_words: List[str]) -> tuple[bool, None, None] | tuple[bool, str, dict[str, float]] | tuple[
        bool, str, None]:
    ...
    def learn_scar(self, toxic_words: List[str], damage: float):
        if damage < 10.0 or not toxic_words: return
...
class EnvironmentalSystem:
    ...
    def process_dynamics(self, m):
        ...
        if "Shattered" in t_msg or "Melted" in t_msg:
            if phys["counts"].get("photo", 0) > 0:
                logs.append(self.eng.gordon.acquire("JAR_OF_FIREFLIES"))
            elif phys["counts"].get("heavy", 0) > 3:
                logs.append(self.eng.gordon.acquire("BROKEN_WATCH"))
...
class TurnContext:
    ...
    bio_result: Dict[str, Any] = field(default_factory=dict)
...
class TheCycleController:
    def __init__(self, engine_ref):
        self.eng = engine_ref
        self.security = TheSecurityChief(engine_ref)

    def run_turn(self, user_message: str) -> Dict[str, Any]:
        self.eng.events.flush()

        # Phase 0: Genesis - Create the Context
        ctx = TurnContext(input_text=user_message, tick=self.eng.tick_count)

        # Phase 1: Physics & Navigation (Where are we?)
        self._phase_physics_and_nav(ctx)

        # Phase 2: Memory Encoding (Remembering the Now)
        self._phase_memory_encoding(ctx)

        # Phase 3: Reality Overrides (Drugs and Dreams)
        self._phase_apply_overrides(ctx)

        # Phase 4: Security Audit (Are we allowed to be here?)
        security_pkg = self.security.audit_input(user_message, {"physics": ctx.physics, "clean_words": ctx.clean_words}, ctx.logs)
        if security_pkg:
            # Schur Note: If the bouncer kicks us out, return the security package immediately.
            return security_pkg

        # Phase 5: The Simulation Loop (The heavy lifting)
        return self._phase_simulation_loop(ctx)

    def _phase_physics_and_nav(self, ctx: TurnContext):
        # Gaze into the tension meter
        ...
        # Update engine state reference (Legacy support)
        self.eng.phys.tension.last_physics_packet = ctx.physics

        # Navigate Manifolds
        current_manifold = self.eng.navigator.locate(ctx.physics)

        if self.eng.navigator.check_anomaly(ctx.input_text):
            ctx.logs.append(f"{Prisma.VIOLET}ANOMALY DETECTED: The user is walking between timelines.{Prisma.RST}")
            ctx.physics["voltage"] = 99.9
            ctx.physics["narrative_drag"] = 0.0
            self.eng.navigator.plot_course("THE_GLITCH")
        else:
            ctx.logs.append(f"{Prisma.GRY}[NAV]: Orbiting {current_manifold} | Shimmer: {self.eng.bio.shimmer.current:.1f}{Prisma.RST}")

        self.eng.bio.shimmer.recharge(0.5)

    def _phase_memory_encoding(self, ctx: TurnContext):
        is_significant = self.eng.mind.mem.encode(
            ctx.clean_words,
            ctx.physics,
            self.eng.bio.governor.mode
        )
        if is_significant:
            self.eng.events.log(f"{Prisma.MAG}ENGRAM FORMED: Context {self.eng.bio.governor.mode} captured.{Prisma.RST}", "MEM")

        shift_msg = self.eng.bio.governor.shift(ctx.physics, self.eng.bio.plasticity.history)
        if shift_msg:
            self.eng.events.log(shift_msg, "GOV")

    def _phase_apply_overrides(self, ctx: TurnContext):
        # Apply the Governor's will
        ctx.physics["psi"] = self.eng.bio.governor.psi_mod

        # Apply Reality Warps
        if self.eng.bio.governor.mode == "FORGE":
            ctx.physics["narrative_drag"] = 0.0
        if self.eng.safety.fever_dream_active:
            ctx.physics["voltage"] = 99.9
            ctx.physics["narrative_drag"] = 0.0
            ctx.physics["kappa"] = 0.0

    def _phase_simulation_loop(self, ctx: TurnContext) -> Dict[str, Any]:
        # --- SUB-SYSTEM 1: DESIRE & FOLLY ---
        folly_clamp, folly_msg, voltage_mod, _ = self.eng.folly.audit_desire(ctx.physics, self.eng.stamina)
        if folly_clamp:
            ctx.physics["voltage"] = voltage_mod
            ctx.logs.append(f"\n{folly_msg}")

        # --- SUB-SYSTEM 2: REPAIR ---
        is_kintsugi, k_msg = self.eng.kintsugi.attempt_repair(ctx.physics, self.eng.trauma_accum)
        if is_kintsugi:
            ctx.logs.append(f"\n{k_msg}")
            ctx.physics["narrative_drag"] = 0.0

        # --- SUB-SYSTEM 3: FEEDBACK LOOP ---
        feedback_signal = {
            "STATIC": max(ctx.physics["repetition"], ctx.physics["counts"]["antigen"] * 0.2),
            "INTEGRITY": ctx.physics["truth_ratio"],
            "FORCE": min(1.0, ctx.physics["voltage"] / 10.0),
            "BETA": ctx.physics.get("beta_index", 1.0)
        }
        self.eng.safety.audit_cycle({"err": ctx.physics["repetition"]})

        # --- SUB-SYSTEM 4: GORDON (Tools & Reflexes) ---
        drift_delta, grav_msg = self.eng.gordon.check_gravity(ctx.physics["narrative_drag"], ctx.physics["psi"])
        if drift_delta != ctx.physics["narrative_drag"]:
            ctx.physics["narrative_drag"] = drift_delta
            ctx.logs.append(f"\n{grav_msg}")

        tool_logs = self.eng.gordon.audit_tools(ctx.physics)
        if tool_logs:
            ctx.logs.extend(tool_logs)
            if "pain_signal" in ctx.physics:
                self.eng.health -= ctx.physics["pain_signal"]
                del ctx.physics["pain_signal"]

        is_pain, pain_msg, panic_physics = self.eng.gordon.flinch(ctx.clean_words)
        if is_pain:
            if panic_physics:
                for k, v in panic_physics.items():
                    if k == "narrative_drag": ctx.physics["narrative_drag"] += v
                    elif k == "voltage": ctx.physics["voltage"] = v
            ctx.logs.append(f"\n{pain_msg}")

        g_active, g_log = self.eng.gordon.emergency_reflex(ctx.physics)
        if g_active: ctx.logs.append(f"\n{g_log}")

        # --- SUB-SYSTEM 5: BIOLOGY (Digestion) ---
        ...
        ctx.logs.extend(ctx.bio_result["logs"])

        # Death Check
        if not ctx.bio_result["is_alive"]:
        ...
        if vel > 4.0:
            ...
            ctx.logs.append(f"VELOCITY SPIKE ({vel:.2f}): -{burn:.1f} Stamina.")

        # --- SUB-SYSTEM 6: PHYSICS (Forge & Theremin) ---
        transmute_msg = self.eng.phys.forge.transmute(ctx.physics)
        if transmute_msg: ctx.logs.append(f"\n{transmute_msg}")

        is_pareidolia, p_msg = self.eng.check_pareidolia(ctx.clean_words)
        if is_pareidolia:
            ctx.logs.append(f"\n{p_msg}")
            ctx.physics["psi"] = min(1.0, ctx.physics["psi"] + 0.3)

        is_stuck, _, t_msg, t_crit = self.eng.phys.theremin.listen(ctx.physics, self.eng.bio.governor.mode)
        if t_msg: ctx.logs.append(f"\n{t_msg}")
        if t_crit == "AIRSTRIKE": self.eng.health -= 25.0

        # --- SUB-SYSTEM 7: RUPTURE & CASSANDRA ---
        rupture, rupture_msg = RuptureEngine.check_for_disruption(ctx.physics, self.eng.mind.lex, self.eng.trauma_accum)
        if rupture:
            ctx.logs.append(f"\n{rupture_msg}")
            ctx.physics["voltage"] += 5.0
            ctx.physics["beta_index"] = 0.0

        if self.eng.cassandra.check_trigger(ctx.physics):
            c_msg = self.eng.cassandra.seize()
            if c_msg:
                ctx.logs.append(c_msg)
                ctx.physics["voltage"] = max(5.0, ctx.physics["voltage"] - 8.0)
                ctx.logs.append(f"{Prisma.GRY}   (DISCHARGE: Voltage dropped by 8.0 to silence the voice.){Prisma.RST}")

        # --- SUB-SYSTEM 8: MIND (Cognition) ---
        ...
        if pulse_status == "ZOMBIE_KNOCK" or ctx.physics["narrative_drag"] > 5.0:
            ...
            if healed_trauma and heal_amt > 0:
                self.eng.trauma_accum[healed_trauma] = max(0.0, self.eng.trauma_accum[healed_trauma] - heal_amt)
                ctx.logs.append(f"{Prisma.GRN}   TRAUMA PROCESSED: {healed_trauma} -{heal_amt:.2f}{Prisma.RST}")
        ctx.mind_state = self.eng.noetic.think(
            ctx.physics, ctx.bio_result, self.eng.gordon.inventory,
            self.eng.phys.dynamics.voltage_history
        )
        if ctx.mind_state["mode"] == "REFUSAL":
            trig = ctx.mind_state.get("trigger")
            if trig == "GURU_TRAP": ctx.logs.append(f"\n{self.eng.refusal.execute_guru_refusal()}")
            elif trig == "SILENT": ctx.logs.append(f"\n{self.eng.refusal.execute_silent()}")
            elif trig == "MIRROR": ctx.logs.append(f"\n{self.eng.refusal.execute_mirror(ctx.input_text)}")
            elif trig == "FRACTAL": ctx.logs.append(f"\n{self.eng.refusal.execute_fractal(ctx.input_text)}")
            else: ctx.logs.append(f"\n{self.eng.refusal.manifest_glitch(ctx.input_text, self.eng.mind.mem)}")
            legacy_ctx = {"physics": ctx.physics, "clean_words": ctx.clean_words}
            return self.eng._package_turn("REFUSAL", ctx.logs, legacy_ctx)

        # --- SUB-SYSTEM 9: WORLD & KINETICS ---
        ctx.world_state = self.eng.kinetic.update_world(ctx.physics, ctx.bio_result, ctx.mind_state)
        self.eng._apply_cosmic_physics(ctx.physics, ctx.world_state["orbit"][0], 0.0)

        # Tick the Clock
        self.eng.tick_count += 1

        # --- SUB-SYSTEM 10: RENDERING ---
        needs_koan, koan_msg = self.eng.kintsugi.check_integrity(self.eng.stamina)
        if needs_koan:
            ctx.logs.append(f"\n{Prisma.CYN}THE CRACK: You are tired. The Kintsugi Protocol offers a Koan:\n   \"{koan_msg}\"{Prisma.RST}")
        title_data = self.eng.mind.wise.architect(
            {"physics": ctx.physics, "clean_words": ctx.clean_words}, # Legacy shape required by architect
            (ctx.mind_state.get("lens"), ctx.mind_state.get("thought"), ctx.mind_state.get("role")),
            self.eng.phys.pulse.is_bored()
        )
        ui_output = self.eng.projector.render(
            {"physics": ctx.physics}, # Legacy shape
            {
                "bio": ctx.bio_result,
                "mind": ctx.mind_state,
            ...
            (ctx.mind_state.get("lens"), self.eng.limbo.haunt(ctx.mind_state["thought"]))
        )
        sys_instr, _ = self.eng.director.generate_chorus_instruction(ctx.physics)
        ctx.logs.extend([e['text'] for e in self.eng.events.flush()])
        return {
            "type": "CYCLE_COMPLETE",
            "ui": ui_output,
            "logs": ctx.logs,
            "metrics": self.eng._get_metrics(ctx.bio_result["atp"]),
            "system_instruction": sys_instr
        }

class TheSecurityChief:
    def __init__(self, engine_ref):
        self.eng = engine_ref

    def audit_input(self, user_message, context, cycle_logs) -> Optional[Dict[str, Any]]:
        phys = context["physics"]
        active_zones, preserve_logs = self.eng.preserves.check_preserves(phys)
        cycle_logs.extend([f"{Prisma.GRN}{log}{Prisma.RST}" for log in preserve_logs])
        if not active_zones:
            passed, customs_msg = self.eng.phys.gate.weigh(phys, self.eng.stamina)
            if customs_msg: cycle_logs.append(f"\n{customs_msg}")
            if not passed:
                self.eng.stamina = max(0.0, self.eng.stamina - 2.0)
                if self.eng.stamina < 10.0:
                    stasis_msg = self.eng.limbo.trigger_stasis_failure(user_message)
                    cycle_logs.append(f"\n{stasis_msg}")
                return self.eng._package_turn("REFUSAL", cycle_logs, context)
        repetition_val = self.eng.phys.pulse.check_pulse(context["clean_words"])
        phys["repetition"] = repetition_val
        pulse_status = self.eng.phys.pulse.get_status()
        if repetition_val > 0.8:
            cycle_logs.append(f"\n{Prisma.GRY}THE SYSTEM SIGHS: We have been here before.{Prisma.RST}")
            self.eng.stamina -= 2.0
            return self.eng._package_turn("STAGNATION", cycle_logs, context)
        ...
        if toxin_type:
            ...
            if toxin_type == "GLYPHOSATE":
                ...
                scar_log = self.eng.gordon.learn_scar(context["clean_words"], 20.0)
                if scar_log: cycle_logs.append(scar_log)
                self.eng.mind.mem.cannibalize()
                cycle_logs.append(f"{Prisma.RED}   >> SYSTEM SHOCK: Health -20.{Prisma.RST}")
                return self.eng._package_turn("TOXICITY", cycle_logs, context)
        if RuptureEngine.audit_ontology(phys):
            ...
            self.eng.health -= 50.0
            return self.eng._package_turn("BREAK", cycle_logs, context)
        return None
...
class BoneAmanita:
    def __init__(self, memory_layer=None, lexicon_layer=None):
        ...
        # [9. THE CONTROLLER]
        self.cycle_controller = TheCycleController(self)
...
class SessionGuardian:
    ...
    def __enter__(self):
        print(f"{Prisma.paint('>>> BONEAMANITA 9.5.2', 'G')}")
