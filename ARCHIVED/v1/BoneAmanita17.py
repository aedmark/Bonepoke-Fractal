# BONEAMANITA 1.7 - "The Salvage Operation"
...
class DeepStorage:
    ...
    def cannibalize(self):
        """
        S_Salvage Protocol
        Sacrifices the oldest memory for immediate energy.
        """
        if not self.artifacts:
            return None

        # Find the oldest artifact (smallest tick value)
        target = min(self.artifacts, key=self.artifacts.get)
        del self.artifacts[target]
        return target
...
class FactStipe:
    ...
    def check_consistency(self, clean_words, current_style, metabolic_status, kinetic_ratio=0.0, tolerance_mode="STANDARD"):
        ...
        # Always return the raw voltage, even if invalid.
        if violations:
            return {
                "valid": False,
                "voltage": voltage,
                "errors": violations,
                "intervention": f"{violations[0]} Stationary paradox."
            }
...
class MetabolicReserve:
    ...
    def metabolize(self, metrics, voltage=0, deep_storage=None):
        ...
        if self.atp < 6 and voltage > 5.0 and deep_storage:
            burnt_artifact = deep_storage.cannibalize()
            if burnt_artifact:
                salvage_bonus = 10
                self.atp += salvage_bonus
                self.status = "SALVAGE"
                salvage_msg = f"🔥 SALVAGE: Memory '{burnt_artifact}' consumed for fuel. WRITE IT NOW."
        ...
        # Include salvage_msg in the return packet if it exists
        response = {"current_atp": self.atp, "delta": delta, "status": self.status}
        if salvage_msg:
            response["salvage_msg"] = salvage_msg
...
class MycelialDashboard:
    ...
    def generate_report(self, m, intv, nrg, chron, anchor_obj, arch, lich, tick_count, volt=0):
        ...
        # VSL-12D Vector String
        s = arch.get('signature', {})
        coord_str = f"[V:{s.get('VEL',0)} S:{s.get('STR',0)} E:{s.get('ENT',0)} Tx:{s.get('TEX',0)}]"

        if nrg['status'] == "FLASHPOINT":
            status_icon = "⚡"
            drag_display = "NULLIFIED"
        ...
        if self.mode == "ANSI":
            ...
            report_lines.append(f"ARCH: {arch.get('archetype')} {coord_str} | {intv or 'STABLE'}")
            ...
            report_lines.append(f"> **ID:** `{arch.get('archetype')}` `{coord_str}` // **INTENT:** {intv or 'FLOW STATE'}")
...
class BonepokeCore:
    ...
    def process(self, raw_input_text, parent_id=None):
        ...
        # 8. METABOLISM & ENERGY
        nrg = self.metabolism.metabolize(
            full_metrics,
            stipe_chk.get('voltage', 0),
            self.memory.deep_storage
        )

        # 9. CALCULATE EFFECTIVE METRICS
        ...
        is_flashpoint = (nrg['status'] == "FLASHPOINT")

        logic_fail = not stipe_chk.get('valid', True)
        hard_mode_fail = (PhysicsConstants.CURRENT_MANDATE == "TRUTH_OVER_COHESION" and stipe_chk.get('voltage', 0) >= 10.0)

        if (logic_fail or hard_mode_fail) and not is_flashpoint:
             ...
             return {
                ...
                "energy": self.metabolism.atp, # Use current ATP
        ...
        if is_flashpoint and logic_fail:
            directives.append(f"⚡ FLASHPOINT OVERRIDE: Logic Tear ignored. Voltage sustained.")
        ...
        if nrg.get('salvage_msg'):
             directives.append(nrg['salvage_msg'])

        # 12. CORTEX INTERVENTION
...
if __name__ == "__main__":
    ...
    print("\n> [SYSTEM ONLINE] BoneAmanita 1.7 is listening...")
