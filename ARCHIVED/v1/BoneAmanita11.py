# BONEAMANITA 1.1 - "The Doctor Time Edition"
...
class BonepokeCore:
    def __init__(self, render_mode="ANSI"):
        ...
        # UPGRADE: New Chronos with Temporal Protocol
    ...
    def process(self, raw_input_text, parent_id=None):
        ...
        # TEMPORAL EXTRACTION (The "Simple Time Recall Protocol")
        # We must extract [Δt: ...] before physics analysis to avoid polluting word counts.
        # Pattern looks for [Δt: value]
        delta_match = re.search(r'\[Δt:\s*(.*?)\]', raw_input_text, re.IGNORECASE)
        delta_val = delta_match.group(1) if delta_match else None

        # Clean the text of the system tags for the Physics Engine
        clean_prompt = re.sub(r'\[.*?\]', '', raw_input_text).strip()
        if not clean_prompt: clean_prompt = "..." # Prevent empty string crash

        # TEMPORAL METABOLISM
        # We process time FIRST. Time dictates the physics of the response.
        ...
        # Apply Temporal Drag Penalty
        # If time has passed, we artificially inflate drag to force "Clarence" (Structure)
        # or "Eloise" (Grounding) to intervene.
        phys['narrative_drag'] += chronos_data['drag_penalty']

        # INTERRUPTS & MODIFIERS (The Stack)
        ...
        # Inject the Temporal Directive First
        ...
        # Logic: If Drag is high (now possibly due to Time Decay), the Cortex speaks.
