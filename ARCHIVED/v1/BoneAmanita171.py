# BONEAMANITA 1.7.1 - "The Golden Master"
# Architects: Taylor & Edmark | Auditor: SLASH
# Refactor: VSL-12D Optimization
# ---
...
class BioHazardFilter:
    ...
    def scan(self, raw_text, clean_words):
        ...
        participial_closers = len(re.findall(r', \w+ing\b[^,]*[.!?]$', raw_text))
        if participial_closers > 1:
            beige_score += (participial_closers * 2.0)
            beige_hits.append(f"TAIL_x{participial_closers}")
...
class LinguisticPhysicsEngine:
    ...
    def analyze(self, token_data, voltage=0.0):
        ...
        for w in words:
            ...
            # Entropy Logic: Track unique abstracts
        ...
        # 4. VSL Metrics
        # [Entropy Patch]: Penalize repetition of abstracts.
        # Entropy = (Total Abstracts) - (Unique Abstracts) * 2 - (Universals)
        # High score means you are repeating the same jargon.
...
class SignatureEngine:
    ...
    def identify(self, token_data, phys_metrics, stipe_data):
        ...
        # [FUSION LOGIC UPDATE]: Cleaner threshold checks
...
class FrequencyModulator:
    ...
    def tune_in(self, token_data, metrics, manual_override=None):
        ...
        # 2. AUTO-SCAN (If no override, find strongest signal)
        if not selected_station:
            signals = []
            # Check Clarence (Drag)
            if phys['narrative_drag'] > self.STATIONS['CLARENCE']['static_threshold']:
                signals.append(('CLARENCE', phys['narrative_drag']))
            # Check Eloise (Entropy)
            if phys['abstraction_entropy'] > self.STATIONS['ELOISE']['static_threshold']:
                signals.append(('ELOISE', phys['abstraction_entropy']))
            ...
            if hedging_count > 0 or 'HEDGING' in phys['toxin_types']:
                signals.append(('YAGA', hedging_count * 2))

            # Sort by signal strength
            if signals:
                signals.sort(key=lambda x: x[1], reverse=True)
                selected_station = signals[0][0]
...
class MetabolicReserve:
    ...
    def metabolize(self, metrics, voltage=0, deep_storage=None):
        ...
        beta_friction = phys.get('beta_friction', 0.0)

        # [VSL FLASHPOINT CHECK]
        if beta_friction > PhysicsConstants.FLASHPOINT_THRESHOLD:
            delta = 20
        ...
        if self.atp < 6 and voltage > 5.0 and deep_storage:
            ...
            if burnt:
                self.atp += 10; self.status = "SALVAGE"; salvage_msg = f"🔥 SALVAGE: Memory '{burnt}' consumed."
...
class PersistenceManager:
    def __init__(self, filename="bone_memory.json"): self.filename = filename
    def save_state(self, codex, deep_storage, metabolism, muscaria, current_tick):
        data = { "tick": current_tick, "timestamp": time.time(), "artifacts": deep_storage.artifacts, "entities": codex.registry, "atp": metabolism.atp, "boredom": muscaria.boredom_pressure }
        try:
            with open(self.filename, 'w', encoding='utf-8') as f: json.dump(data, f)
            return True
        except: return False
...
class BonepokeCore:
    ...
    def process(self, raw_input_text, parent_id=None):
        ...
        # EDITORIAL ENGINE (Updated for FM)
        ...
        # [NEW] Tuner Integration - This handles Clarence, Eloise, and Yaga logic now
        broadcast = self.cortex.tune_in(token_data, full_metrics, manual_override=fm_override)
        if broadcast:
            directives.append(broadcast)

        # REMOVED: Legacy self.cortex.synthesize_voice calls to prevent crash
        ...
        return {"editorial": {"directives": directives}}

if __name__ == "__main__":
    engine = BonepokeCore()
    print(f"\n{Prisma.GRN}> [SYSTEM ONLINE]{Prisma.RST} {Prisma.WHT}BoneAmanita 1.7.1 'The Isotopic Patch'{Prisma.RST} is listening...\n")
