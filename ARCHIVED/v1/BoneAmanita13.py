# BONEAMANITA 1.3 - "The Butcher & The Well"
...
class TheLexicon:
    ...
    # [NEW] THE HYDRATION SUITE
    # Phrases that dilute density and allow the reader to breathe.
    SOLVENTS = {
    ...
    # These are low-calorie connectors that gamers usually delete to save space.
    # We now reward them slightly to encourage natural rhythm.
    HYDRATION_MARKERS = SOLVENTS.union({
        'that', 'this', 'there', 'here', 'which', 'who', 'when', 'where', 'why', 'how'
    })

    # [NEW] THE BUTCHER'S BLOCK
    # Words that tell instead of show.
    WEAK_INTENSIFIERS = {
        'very', 'really', 'quite', 'totally', 'utterly', 'absolutely',
        'literally', 'suddenly', 'finally', 'immediately'
    }

    # [NEW] THE COMPASS ROSE
    # Words that imply movement or transition (to break stagnation)
    TRANSITIONS = {
...
class TheWitchRing:
    def __init__(self):
        self.aphorism_streak = 0 # [NEW] Track consecutive short inputs
    ...
    def evaluate_intent(self, clean_words, metrics):
        ...
        # [MODIFIED] APHORISM CHECK
        if count < 8: # Short sentence
            if density >= 0.4:
        ...
        # Reset streak if normal sentence
...
class HydrationMonitor:
    def __init__(self):
        self.density_threshold = 0.15  # 15% Heavy Matter is the "Choking Point"
        self.critical_density = 0.25   # 25% is "Gravel"

    def measure_viscosity(self, clean_words, heavy_matter_set, solvent_set):
        total = len(clean_words)
        if total == 0: return {"density": 0, "hydration": 0, "status": "VOID"}
        ...
        # The Ratio: We need roughly 1 part Water for every 3 parts Concentrate
        balance_ratio = hydration / (density if density > 0 else 1.0)

        status = "OPTIMAL"
        if density > self.critical_density and balance_ratio < 0.2:
        ...
        return {
            ...
            "status": status,
            "penalty": 0.0 if status == "OPTIMAL" else (2.0 if status == "THICK" else 5.0)
        }
...
class LinguisticPhysicsEngine:
    ...
    def analyze(self, token_data):
        ...
        # [MODIFIED] Narrative Drag now penalizes "CONCRETE" status
        # If text is too thick (CONCRETE), it drags.
        base_drag = (total_words + (toxicity_score * 10)) / action_score
        narrative_drag = base_drag + hydro_data['penalty'] + butcher_penalty
        ...
        return {
            "physics": {
                ...
                "garnish_ratio": round(garnish_ratio, 2), # [NEW]
...
class SignatureEngine:
    ...
    def calculate_dimensions(self, token_data, phys_metrics):
        ...
        # [MODIFIED] TEXTURE CALCULATION
        raw_tex = (sum(1 for w in words if w in self.lexicon.UNIVERSALS) / max(1, len(words))) * 3.33

        # THE GOVERNOR: If status is CONCRETE (Gaming), cap Texture at 0.5.
        # Real texture requires flow. A pile of bricks is not a wall.
...
class ChronosAnchor:
    def __init__(self):
        # Regex to parse [Δt: H:MM/D] or simple minutes
        self.time_pattern = re.compile(r'(\d+):?(\d+)?')

    def metabolize_delta(self, delta_str):
        if not delta_str:
            return {"state": "UNKNOWN", "minutes": 0, "drag_penalty": 0.0, "directive": "SYSTEM: No Temporal Data."}

        minutes = 0
        s = delta_str.lower()
        nums = [int(n) for n in re.findall(r'\d+', s)]

        # LOGIC: Handle 'day', 'hour', and 'minute' explicit labels
        if 'd' in s or 'day' in s:
             minutes = 1440
        elif ('h' in s or 'hour' in s) and len(nums) == 1:
             # Case: "2 hours", "2h" -> Multiply by 60
             minutes = nums[0] * 60
        elif len(nums) == 1:
             # Case: "90" (assumed minutes)
             minutes = nums[0]
        elif len(nums) >= 2:
             # Case: "1:30" (H:MM)
             minutes = (nums[0] * 60) + nums[1]

        # LOGIC GATES (Unchanged)
        if minutes < 10:
...
class NarrativeChronometer:
    def __init__(self):
        ...
        self.MACRO_MARKERS = {
            ...
            'week', 'lifetime', 'forever', 'later', 'meanwhile', 'ago', 'cycles'
        }

    def analyze_pacing(self, clean_words):
        micro = sum(1 for w in clean_words if w in self.MICRO_MARKERS)
        macro = sum(1 for w in clean_words if w in self.MACRO_MARKERS)

        # Velocity Score: Positive = Fast (Montage), Negative = Slow (Bullet Time)
        score = (macro * 2.0) - (micro * 1.5)

        state = "REALTIME"
        if score >= 1.5: state = "MONTAGE"
        elif score <= -1.5: state = "BULLET_TIME"

        return {"state": state, "score": score, "micro": micro, "macro": macro}

    def get_modifiers(self, state):
        # MONTAGE: High tolerance for abstraction (Entropy), moderate drag forgiveness.
        if state == "MONTAGE": return {"entropy_grace": 4.0, "drag_grace": 1.0}
        # BULLET TIME: Zero tolerance for abstraction. Show me the atoms.
...
class VirtualCortex:
    ...
    def synthesize_voice(self, agent, token_data, metrics, loop_count=0):
        ...
        if agent == "CLARENCE":
         ...
         # [NEW] HYDRATION INTERRUPT
        # If the text is "CONCRETE" (Too dense/gaming), Michael steps in.
        if viscosity.get('status') == "CONCRETE" and agent == "CLARENCE":
            # We override Clarence because the user thinks they are being smart (Clarence-like).
            # We send in Michael to humble them.
...
class MycelialDashboard:
    ...
    def render(self, m, intv, nrg, chron, anchor_obj, arch, lich, volt=0):
        phys, sig = m['physics'], arch.get('signature', {})
        visc = phys.get('viscosity', {'status': 'N/A'}) # Get Viscosity
        narr = m.get('narrative', {'state': 'REALTIME'})

        status_icon = "🟢" if nrg['status'] == "STABLE" else ("🔴" if nrg['status'] == "STARVING" else "🟡")

        if self.mode == "ANSI":
            # Add Viscosity Status to the display
            print(f"\n[BONEAMANITA v1.3] {status_icon} ATP: {nrg['current_atp']} | DRAG: {phys['narrative_drag']} | VISC: {visc['status']}")
            print(f"ARCH: {arch.get('archetype')} | {intv or 'STABLE'}")
        else:
            # MARKDOWN HUD
            print(f"> **[{status_icon} SYSTEM HUD]** ATP: `{nrg['current_atp']}` | DRAG: `{phys['narrative_drag']}` | VISC: `{visc['status']}`")
            print(f"> **TIME:** `{chron.get('state', 'FLOW')}` (Real) | `{narr['state']}` (Story)")
...
class BonepokeCore:
    ...
    def process(self, raw_input_text, parent_id=None):
        ...
        # 2. PHYSICS & NARRATIVE TIME
        metrics = self.physics.analyze(t_dat)
        phys, stat = metrics['physics'], metrics['status']

        # [NARRATIVE TIME ANALYSIS]
        narr_time = self.narrative_timer.analyze_pacing(t_dat['clean_words'])
        time_mods = self.narrative_timer.get_modifiers(narr_time['state'])
        metrics['narrative'] = narr_time

        # Apply Real-Time Drag Penalty
        phys['narrative_drag'] += chronos_data['drag_penalty']
        ...
        if self.nilsson.evaluate_mantra(t_dat['clean_words'], phys['repetition_rate']):
        ...
        # [EFFECTIVE METRICS APPLICATION]
        eff_drag = phys['narrative_drag'] - time_mods['drag_grace']
        eff_entropy = phys['abstraction_entropy'] - time_mods['entropy_grace']
        ...
        if eff_entropy > 2:
             directives.append(f"GROUNDING: {self.cortex.synthesize_voice('ELOISE', t_dat, metrics)}")
        ...
        self.last_id = cur_id
        self.memory.leave_trace(clean_prompt, phys['dominant_style'])

        self.dashboard.render(metrics, musc_msg, nrg, chronos_data, self.chronos, arch_dat, lich)
