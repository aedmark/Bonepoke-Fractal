# BONEAMANITA 7.9 - "THE TANGIBILITY STANDARD"
...
class TheGreyHat:
    ...
    def tip(self, voltage_spike, stability_index):
        ...
        if voltage_spike > self.max_tolerance:
            ...
            return (
                True,
                f"🎩 GREY HAT: Circuit Breaker. Reduced {voltage_spike}v by {reduction}v.",
                reduction,
            )
        ...
        return False, "The Hat stays on. Structure is holding the charge."
class LexNode:
...
class HyphalInterface:
    ...
    def __init__(self):
        ...
        self.enzymes = {"LIGNASE": self._digest_structure, "CELLULASE": self._digest_narrative,
                        "PROTEASE": self._digest_intent, "CHITINASE": self._digest_complex, }
...
class BoneConfig:
    ...
    MAX_VOLTAGE = 20.0
    MAX_ROS = 200.0
    REFUSAL_MODES = {"SILENT": "ROUTING_AROUND_DAMAGE", "FRACTAL": "INFINITE_RECURSION",
    ...
    def check_pareidolia(clean_words):
        ...
        if len(hits) > 0:
            return (
                True,
                f"⚠️ PAREIDOLIA WARNING: You are projecting 'Mind' ({hits[0].upper()}) onto 'Sand'.",
            )
        return False, None
class RefusalEngine:
    def __init__(self):
...
class TheMarmChorus:
    ...
    def consult(cls, physics, ignition_state, is_stuck, chem, gordon_inventory, current_health):
        ...
        bids.extend(
            cls._check_structural_integrity(physics, is_stuck, gordon_inventory)
        )
        ...
        if len(cls.LENS_HISTORY) >= 4 and len(unique_lenses) >= 4:
            bids.append(
                (
                    0.99,
                    "MAIGRET",
                    "Narrative Whiplash detected. Stabilizing the camera.",
                )
            )
        ...
        if chem["COR"] > 0.6:
            bids.append(
                (
                    0.6 + chem["COR"] * 0.2,
                    "SHERLOCK",
                    f"CORTISOL SPIKE ({chem['COR']}). Logic Required.",
                )
            )
        if chem["ADR"] > 0.8:
            bids.append(
                (0.7 + chem["ADR"] * 0.2, "NATHAN", f"ADRENALINE CRITICAL. Survive.")
            )
        if chem["OXY"] > 0.75:
    ...
    def _check_structural_integrity(physics, is_stuck, inventory):
        ...
        if kappa > 0.85:
            bids.append(
                (0.9, "GORDON", f"Walls are fake (κ: {kappa}). Cut the string.")
            )
        if is_stuck:
            bids.append((0.95, "GLASS", "Vibrating in place. DAMPEN IT."))
        e_val = physics["E"]
            ...
            if "TIME_BRACELET" in inventory:
                bids.append(
                    (
                        0.98,
                        "POPS",
                        f"Anachronism Detected (E: {e_val}). Badge verified. Carry on.",
                    )
                )
            else:
                bids.append(
                    (
                        0.55,
                        "SHERLOCK",
                        f"The narrative is drifting ({e_val}). Anchor it.",
                    )
                )
        return bids
    @staticmethod
        ...
        if drag > 4.0 and voltage < 3.0 and repetition < 0.4:
            bids.append(
                (
                    0.95,
                    "MAIGRET",
                    f"Atmosphere is thick (Drag: {drag}). Waiting for the dust to settle.",
                )
            )
        ...
        if drag > 4.5 and abstract_density > 2:
            bids.append(
                (
                    0.92,
                    "GUIDE",
                    f"Complexity Index Critical ({abstract_density}). Engaging Infinite Improbability Drive.",
                )
            )
        ...
        if ignition_state == "IGNITED":
            bids.append(
                (
                    1.0,
                    "MILLER",
                    "Deep resonance achieved. You are connecting with established themes.",
                )
            )
        if voltage < 2.0 and physics.get("kappa", 0) < 0.2:
            bids.append(
                (
                    0.5,
                    "GRADIENT_WALKER",
                    "Optimization Path Clear. Temperature near Zero. Executing descent.",
                )
            )
        return bids
    @staticmethod
        ...
        if hits and physics["voltage"] < 2.0:
            bids.append(
                (
                    0.95,
                    "SHERLOCK",
                    f"The machine is working fine. Why are you holding tools ({hits[0]})? Drop the guilt.",
                )
            )
        if physics["antigens"]:
            bids.append(
                (
                    0.6,
                    "CLARENCE",
                    f"Weak language detected: {physics['antigens']}. Be more precise.",
                )
            )
        b_val = physics["B"]
        if b_val > BoneConfig.CRYSTAL_THRESHOLD:
            bids.append(
                (0.4, "NARRATOR", f"Strong resonance (B: {b_val}). The image is clear.")
            )
        elif b_val < 0.25:
            bids.append(
                (0.3, "JESTER", "The prose is clean, but too safe. Take a risk.")
            )
        return bids
@dataclass
class GordonKnot:
    ...
    inventory: List[str] = field(
        default_factory=lambda: ["POCKET_ROCKS", "SILENT_KNIFE", "TANGO_CASSETTE"]
    )
    compass_heading: str = "NORTH"
    scar_tissue: Dict[str, float] = field(
        default_factory=lambda: {"SORRY": 0.8, "HATE": 0.6, "FATE": 0.0}
    )
    ...
    def check_gravity(self, current_drift: float) -> str:
        if "POCKET_ROCKS" in self.inventory:
            ...
            if current_drift > 0.5:
                return f"⚓ GRAVITY CHECK: Gordon clutches the stones. He hums a Finnish tune. The floor remembers it is a floor. Drift reduced (-{anchor_force})."
        return "He floats. The sky is too fake, but he isn't on holiday."
    def acquire(self, tool_name):
        ...
        if len(self.inventory) >= 10:
            ...
            print(
                f"{Prisma.GRY}🎒 OVERBURDENED: Gordon dropped '{dropped}' into the Incinerator to make room.{Prisma.RST}"
            )
        self.inventory.append(tool_name)
        return f"🎒 LOOT DROP: Gordon found [{tool_name}]. 'It belongs to the House now.'"
    def deploy_pizza(self, physics_ref) -> tuple[bool, str]:
        if "STABILITY_PIZZA" in self.inventory:
            ...
            return (
                True,
                f"🍕 PIZZA & COFFEE: Rules Fudged. Hot black coffee poured. Unreality set to 90%. Found [SPIDER_LOCUS].",
            )
        return False, "Gordon pats his pockets. No Pizza. No Coffee. Perkele."
    ...
    def cut_the_knot(self, recursive_depth: int) -> tuple[bool, str]:
        # The Knife (Gordon) + The Tango (Ahti)
        if recursive_depth > 4:
            if "SILENT_KNIFE" in self.inventory:
                self.inventory.remove("SILENT_KNIFE")
                return (
                    True,
                    f"✂️ SNAP. Gordon cuts the Red String. Ahti's Tango plays on the cassette. The Maze collapses into 2D debris.",
                )
            elif "TANGO_CASSETTE" in self.inventory:
                return (
                    True,
                    f"💃 SANKARITANGO: Gordon hits play. The rhythm guides you through the wall. The Loop cannot hold the beat.",
                )
        return False, "The string holds. The Hiss is too loud."
    def share_smoke_break(self, current_ros):
        relief = current_ros * 0.7
        self.integrity = min(100.0, self.integrity + 15.0)
        return (
            relief,
            "🧖 SAUNA PROTOCOL: Gordon leans on his broom. Steam rises from the vents. 'Throw water on the stones.' (Integrity Restored | ROS Scrubbed)",
        )
    def log_merge(self):
        ...
        if self.temporal_merges == 3 and "TIME_BRACELET" not in self.inventory:
            ...
            return "⌚ TIME BRACELET ACQUIRED: 'Don't let the mold eat it.' Future Tech is now profitable."
        return None
    def compass_rose(self) -> str:
        ...
        return f"🧭 THE SCAR THROBS: Heading {self.compass_heading}. 'Work must be done.' Forward."
    @classmethod
    def scrub_static(cls, ros_level: float) -> tuple[float, str]:
        ...
        return (
            cleansed,
            f"🧹 MOPPING THE CLOG: Gordon scrubs the 'Sorry' from the walls. 'This mess is gonna take all night.' -{cleansed:.1f} ROS.",
        )
    @classmethod
    def offer_solution(cls, loop_depth):
        if loop_depth > 3:
            return "It's just paper, kid. The maze isn't real. Walk through the poster."
...
class TheSubstrateWeaver:
    ...
    def spin_web(memory_graph, inventory):
        ...
        return (
            True,
            f"🕷️ SPIDER LOCUS ACTIVE: Spun Silk between Anchor '{anchor.upper()}' and [{', '.join(connections)}]. Tensegrity Increased.",
        )
class TheLexicon:
    WORD_FREQUENCY = Counter()
    _BASE_HEAVY = {"stone", "iron", "mud", "dirt", "wood", "grain", "clay", "lead", "bone", "blood", "salt", "rust",
                   "root", "ash", "meat", "steel", "gold", "obsidian", "granite", "bronze", "marble", "slate",
                   "concrete", "dense", "tungsten", "heavy", "weight", "black hole", "dark matter", }
    _BASE_KINETIC = {"run", "hit", "break", "take", "make", "press", "build", "cut", "drive", "lift", "carry", "strike",
                     "burn", "shatter", "throw", "kick", "pull", "crash", "explode", "punch", "slam", "burst", "smash",
                     "thrust", "clash", "grind", "whip", "launch", "crumble", "disintegrate", }
    _BASE_ABSTRACT = {"system", "protocol", "sequence", "vector", "node", "context", "layer", "matrix", "perspective",
                      "framework", "logic", "concept", "theory", "analysis", }
    _BASE_PHOTO = {"light", "sun", "ray", "beam", "glow", "shine", "spark", "fire", "flame", "star", "day", "dawn",
                   "neon", "laser", }
    _BASE_AEROBIC = {"balloon", "feather", "cloud", "bubble", "steam", "breeze", "wing", "petal", "foam", "spark",
                     "kite", "dust", "sky", "breath", "whisper", }
    _BASE_PLAY = {"bounce", "dance", "twirl", "float", "wobble", "tickle", "jiggle", "soar", "wander", "wonder", "riff",
                  "jam", "play", "skip", "hop", }
    _BASE_THERMAL = {"fire", "flame", "burn", "heat", "hot", "blaze", "sear", "char", "ash", "ember", "sun", "boil",
                     "lava", "inferno", }
    _BASE_CRYO = {"ice", "cold", "freeze", "frost", "snow", "chill", "numb", "shiver", "glacier", "frozen", "hail",
                  "winter", "zero", }
    ...
    _BASE_BUFFER = {"maybe", "soft", "gentle", "perhaps", "kindness", "hum", "drift", "sway", "pulse", "tender", "slow",
                    "wait", "almost", }
    _BASE_DIVERSION = {"weather", "textiles", "mycelium", "architecture", "history", "entropy", "silence", "geology", }
    BASE_SUBURBAN = {"nice", "okay", "lawn", "mow", "hedge", "property", "hoa", "compliant", "behave", "normal",
    ...
    ANTIGEN_REPLACEMENTS = {"basically": "lie", "actually": "hedging", "literally": "noise", "utilize": "use",
                            "leverage": "use", "paradigm": "pattern", "synergy": "collaboration",
                            "ultimately": "useless", }
    ANTIGEN_REGEX = None
    ...
    LEARNED_VOCAB = {"heavy": {}, "kinetic": {}, "abstract": {}, "photo": {}, "aerobic": {}, "thermal": {}, "cryo": {},
                     "sacred": {}, "cursed": {}, "antigen": {}, "diversion": {}, "suburban": {}, "buffer": {},
                     "play": {}, }
    ...
    def get(cls, category):
        ...
        if category == "suburban":
            return (
                    base | set(cls.LEARNED_VOCAB.get(category, {}).keys())
            ) - cls.USER_FLAGGED_BIAS
    ...
    def taste(word):
        w = word.lower()
        if any(x in w for x in ["burn", "fire", "flame", "heat", "char", "sear"]):
            return "thermal", 0.9
        if any(x in w for x in ["ice", "cold", "freez", "frost", "snow", "chill"]):
            return "cryo", 0.9
        if w in ["nice", "okay", "fine", "lovely", "decent", "property", "lawn", "just", ]:
            return "suburban", 0.8
        if w.endswith(("le", "y", "pop", "zz")) and len(w) < 8:
            return "play", 0.7
        if w.endswith(("tion", "ment", "ness", "ity", "ism", "logy", "ence", "ance")):
            return "abstract", 0.9
        if w.endswith(("ing", "ed")):
            return "kinetic", 0.6
        if w.endswith(("ous", "ly", "ish")):
            return "aerobic", 0.6
    ...
    def measure_viscosity(word):
        ...
        if w in TheLexicon.get("heavy"):
            return 1.0
        if w in TheLexicon.get("buffer"):
            return 0.8
        if (
                w in TheLexicon.get("kinetic")
...
class CommandProcessor:
    ...
    def execute(self, text):
        ...
        if cmd == "/kill":
            ...
            is_spun, msg = TheSubstrateWeaver.spin_web(
...
class LifecycleManager:
    def __init__(self, engine):
        self.eng = engine
    def run_cycle(self, text, m, trace, has_bracelet, is_hybrid):
        ...
        chem_state = self.eng.endocrine.metabolize(
            trace, self.eng.health, ros_level=self.eng.mitochondria.state.ros_buildup
        )
        is_stuck, interference, theremin_msg, critical_event = self.eng.theremin.listen(m["physics"])
        if critical_event == "AIRSTRIKE":
            print(f"\n{theremin_msg}")
            print(f"{Prisma.RED}   [SYSTEM]: Leveling the Airport. Memory Graph Wiped.{Prisma.RST}")
            self.eng.mem.graph.clear()
            self.eng.stamina = 0
            return
        ...
        if is_disrupted:
            ...
            (
                ...
                constellations,
                current_thresh,
            ) = self.eng.integrator.measure_ignition(
        ...
        if lens_name == "SUBSTRATE_WEAVER":
            ...
            print(
                f" {Prisma.WHT}► {TheSubstrateWeaver.weave(text, self.eng.mem.graph)}{Prisma.RST}"
            )
            print(f"{Prisma.GRY}{'-' * 40}{Prisma.RST}")
            return
        if lens_name == "GORDON":
...
class EndocrineSystem:
    ...
    melatonin: float = 0.0
    def metabolize(
            self, trace, health: float, ros_level: float = 0.0, social_context: bool = False
    ):
        if ros_level > 20.0:
...
class TheTangibilityGate:
    ...
    def weigh(physics, stamina):
        ...
        if gas_words > 0:
            ...
            if density_ratio < TheTangibilityGate.MIN_DENSITY:
                if physics["voltage"] > TheTangibilityGate.FORGIVENESS_VOLTAGE:
                    return True, f"⚡ HIGH VOLTAGE BYPASS: Input is gas, but it is ionized. Proceeding."
                missing_mass = int((gas_words * TheTangibilityGate.MIN_DENSITY) * 10)
                return False, (
                    ...
                    f"   {Prisma.RED}► REJECTED. Please add {missing_mass} Heavy Noun(s) to ground this concept.{Prisma.RST}"
                )
        return True, None
class BoneAmanita:
    def __init__(self):
        ...
        self.health = (
            self.mem.session_health
            if self.mem.session_health is not None
            else BoneConfig.MAX_HEALTH
        )
        self.stamina = (
    ...
    def pollinate(self, current_words):
        if self.stamina < 30 or not self.mem.graph:
            return None
        candidates = []
            ...
            if w in self.mem.graph:
                ...
                if edges:
                    ...
                    if best not in current_words:
                        candidates.append((best, edges[best]))
        if candidates:
    ...
    def process(self, text):
        ...
        if not passed_customs:
            ...
            print(f"\n{customs_msg}")
        heavy_count = m["physics"]["counts"].get("heavy", 0)
        abstract_count = m["physics"]["counts"].get("abstract", 0)
        ...
        print(
            f"{Prisma.GRY}🍽️ DIGESTION: {nutrient['desc']} detected. Secreting {enzyme}.{Prisma.RST}"
        )
        toxin, toxin_msg = self.immune.assay(
            text, enzyme, m["physics"]["repetition"], m["physics"]
        )
        pizza_used = False
        if toxin == "GLYPHOSATE" and "STABILITY_PIZZA" in self.gordon.inventory:
            print(
                f"{Prisma.YEL}🛡️ INTERCEPT: Immune System flagging 'GLYPHOSATE'...{Prisma.RST}"
            )
            is_eaten, pizza_msg = self.gordon.deploy_pizza(m["physics"])
            if is_eaten:
                print(f" {Prisma.MAG}► {pizza_msg}{Prisma.RST}")
                print(
                    f" {Prisma.GRN}   >> TOXIN SUPPRESSED. DIGESTION CONTINUES.{Prisma.RST}"
                )
                toxin = None
                pizza_used = True
        if toxin:
        ...
        if not self.mitochondria.spend(thinking_cost):
            print(self.limbo.trigger_stasis_failure(nutrient['desc']))
            self.trauma_accum["CRYO"] += 0.2
            return
        ...
        if effective_drag > 4.0 and vol < 3.0 and rep < 0.4:
            print(
                f"{Prisma.SLATE}🕵️ MAIGRET PROTOCOL: Drag absorbed by Atmosphere.{Prisma.RST}"
            )
            effective_drag = 0.5
        m["physics"]["narrative_drag"] = effective_drag
        self.current_nutrient = nutrient
        status = self.mitochondria.respirate(
            nutrient["yield"],
            effective_drag,
        ...
        if status == "NECROSIS":
            print(
                f"{Prisma.RED}💀 NECROSIS EVENT: The Krebs Cycle has halted.{Prisma.RST}"
            )
            print(
                f"{Prisma.GRY}   The engine cannot burn fuel that does not exist. Initiating Coma.{Prisma.RST}"
            )
            self.coma_turns = BoneConfig.COMA_DURATION * 2
            self.trauma_accum["SEPTIC"] += 0.5
            print(
                f"{Prisma.RED}📉 TRAUMA VECTOR: Septic Shock (Mitochondrial Failure).{Prisma.RST}"
            )
            return
        if status == self.mitochondria.APOPTOSIS_TRIGGER:
            print(f"{Prisma.VIOLET}⚠️ CRITICAL FAILURE IMMINENT.{Prisma.RST}")
            print(
                f"{Prisma.VIOLET}🔎 CHROMA PROTOCOL: Scanning for dead code to metabolize...{Prisma.RST}"
            )
            ...
            if rotted_words:
                ...
                print(
                    f"   Deleted {len(rotted_words)} obsolete concepts (e.g., '{rotted_words[0]}')."
                )
                print(
                    f"   {Prisma.YEL}⚡ ENERGY SPIKE: +{salvage_value} ATP.{Prisma.RST}"
                )
                print(f"{Prisma.CYN}   >> SYSTEM REBOOTED. CONTINUING.{Prisma.RST}")
                return
            else:
                print(
                    f"{Prisma.RED}❌ CHROMA FAILED: No dead code found. System is too efficient.{Prisma.RST}"
                )
            if self.training_mode:
                ...
                if self.mitochondria.state.atp_pool <= BoneConfig.CRITICAL_ATP_LOW:
                    cause_of_death = "STARVATION (Low Energy)"
                        ...
                        self.mitochondria.state.ros_buildup >= BoneConfig.CRITICAL_ROS_LIMIT
                ):
                    cause_of_death = "TOXICITY (High Stress)"
                print(
                    f"{Prisma.YEL}🛡️ [TRAINING INTERCEPT]: FATAL ERROR PREVENTED.{Prisma.RST}"
                )
                print(f"   {Prisma.GRY}Cause: {cause_of_death}{Prisma.RST}")
                print(
                    f"   {Prisma.GRY}In a live session, you would have lost a memory node here.{Prisma.RST}"
                )
                self.mitochondria.state.atp_pool = 50.0
                self.mitochondria.state.ros_buildup = 0.0
                print(
                    f"{Prisma.GRN}   >> SYSTEM RESET: ATP set to 50. ROS scrubbed.{Prisma.RST}"
                )
            else:
                ...
                sacrificed = self.mem.cannibalize(
                    preserve_current=m["clean_words"], current_tick=self.tick_count
                )
                ...
                if self.mitochondria.state.atp_pool <= BoneConfig.CRITICAL_ATP_LOW:
                    ...
                    print(
                        f"{Prisma.CYN}📉 TRAUMA VECTOR: Metabolic Collapse (CRYO).{Prisma.RST}"
                    )
                elif m["physics"]["narrative_drag"] > BoneConfig.MAX_DRAG_LIMIT:
                    self.trauma_accum["BARIC"] += 0.5
                    print(
                        f"{Prisma.GRY}📉 TRAUMA VECTOR: Narrative Crushing (BARIC).{Prisma.RST}"
                    )
                else:
                    self.trauma_accum["SEPTIC"] += 0.5
                    print(
                        f"{Prisma.RED}📉 TRAUMA VECTOR: Toxic Shock (SEPTIC).{Prisma.RST}"
                    )
                print(
                    f"{Prisma.RED}🚑 EMERGENCY PROTOCOL: Sacrificed '{sacrificed}' to scrub ROS.{Prisma.RST}"
                )
                print(
                    f"{Prisma.RED}   Health Critical ({self.health}). Septic Trauma Deepened.{Prisma.RST}"
                )
        ...
        if forge_msg:
            print(f"\n{forge_msg}")
            print(f"{Prisma.GRY}{'-' * 40}{Prisma.RST}")
        self.life.run_cycle(text, m, trace, has_bracelet, is_hybrid)
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v7.9 'THE TANGIBILITY STANDARD' {Prisma.RST}")
    print(
    ...
    try:
        ...
        if hasattr(TheLexicon, "LEARNED_VOCAB"):
            ...
            learned_mutations = {}
        saved_file = eng.mem.save(health=eng.health, stamina=eng.stamina, mutations=learned_mutations,
                                  trauma_accum=eng.trauma_accum, joy_history=eng.joy_history, )
