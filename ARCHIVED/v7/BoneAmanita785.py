...
class TheGreyHat:
    ...
    def tip(self, voltage_spike, stability_index):
	    ...
	    if voltage_spike > self.max_tolerance:
	        ...
	        return True, f"🎩 GREY HAT: Circuit Breaker. Reduced {voltage_spike}v by {reduction}v.", reduction
	    elif voltage_spike > 8.0 and stability_index < 0.4:
	            self.charges -= 1
	            return True, f"🎩 THE GREY HAT: Tipped. High Voltage ({voltage_spike}v) on Unstable Ground (γ: {stability_index}). Dampening."
...
class MitochondrialForge:
    ...
    def respirate(self, input_complexity: float, narrative_drag: float, has_bracelet: bool = False, is_hybrid: bool = False) -> str:
        ...
        if TheLexicon.ANTIGEN_REGEX:
            ...
            if antigen_hits:
                ...
                if counts.get("thermal", 0) > 0:
                    return None, f"{Prisma.OCHRE}🔥 ALCHEMY: Thermal words boiled off the '{antigen_hits[0]}' toxin. Safe.{Prisma.RST}"
                if counts.get("heavy", 0) >= len(antigen_hits):
                    return None, f"{Prisma.GRN}🌿 MARIK GAMBIT: You ate the poison ('{antigen_hits[0]}'), but the Herbs (Heavy) neutralized it.{Prisma.RST}"
                if counts.get("cryo", 0) > 0:
                    return "CYANIDE_POWDER", f"❄️ CRYO-CONCENTRATION: Cold words freeze-dried the '{antigen_hits[0]}' into a fatal powder."
                return "GLYPHOSATE", f"☣️ TOXIN DETECTED: '{antigen_hits[0]}' is unprocessed waste. Burn it or bury it."
...
class TheMarmChorus:
    ...
    def _check_narrative_physics(physics, ignition_state):
        ...
        if psi > 0.8 or is_dense:
            bids.append((0.85, "SUBSTRATE_WEAVER", f"Permeability High (Ψ: {psi}). Concepts are bleeding. Analyzing substrate knots."))
...
class GordonKnot:
    ...
    inventory: List[str] = field(default_factory=lambda: ["POCKET_ROCKS", "SILENT_KNIFE"])
...
class MycelialNetwork:
    def __init__(self, seed_file=None):
        ...
        self.seeds = [
            ...
            ParadoxSeed("Does the adventurer exist if the narrator stops speaking?", {"narrator", "voice", "graham", "story", "exist", "speak"}),
            ParadoxSeed("If you meet your echo, who moves out of the way?", {"copy", "echo", "self", "collision", "path", "yield", "double", "same"})
        ]
            ...
            try:
                with open(path, "r") as f:
                    ...
                    for t_val in data.get("trauma_vector", {}).values():
                        if not (0 <= t_val <= 10.0):
                            print(f"{Prisma.RED}[MEMORY]: Spore rejected (Impossible Trauma Value: {t_val}). Burned.{Prisma.RST}")
                            return
                    ...
                    if "joy_vectors" in data and data["joy_vectors"]:
                        ...
                        if best['dominant_flavor'] == "kinetic":
                            ...
                            print(f"  {Prisma.CYN}► Adjustment: Kinetic Gain boosted.{Prisma.RST}")
                        elif best['dominant_flavor'] == "abstract":
                            BoneConfig.SIGNAL_DRAG_MULTIPLIER *= 0.8
                            print(f"  {Prisma.CYN}► Adjustment: Drag sensitivity lowered.{Prisma.RST}")
    ...
    def execute(self, text):
        ...
        if cmd in ["/teach", "/kill", "/flag"]:
            if self.eng.mirror.profile.confidence < 50:
                print(f"{Prisma.YEL}⚠️ COMMAND LOCKED: Requires 50+ turns of trust (Current: {self.eng.mirror.profile.confidence}).{Prisma.RST}")
                return True
        if cmd == "/kill":
            if len(parts) >= 2:
                toxin = parts[1]
            ...
            if len(parts) > 1 and parts[1] == "--hard":
                print(f"{Prisma.RED}🧨 FACTORY RESET INITIATED. DELETING ALL MEMORIES...{Prisma.RST}")
                try:
                    import shutil
                    shutil.rmtree("memories")
                    os.makedirs("memories")
                    print(f"{Prisma.GRY}   Tabula Rasa achieved. Restart required.{Prisma.RST}")
                    exit()
                ...
                print(f"{Prisma.YEL}Usage: /reset --soft (Session) | /reset --hard (Full Wipe){Prisma.RST}")
        elif cmd == "/profile":
            try:
                name = parts[1]
                likes = []
            ...
            print(f"   {Prisma.GRY}Status: Working on Exit bug. Message me if you see this sign.{Prisma.RST}") #
            self.eng.phys.vector_memory.clear()
            self.eng.stamina = BoneConfig.MAX_STAMINA
            print(f"   {Prisma.CYN}► CACHE CLEARED. STAMINA RESTORED. REALITY SHARPENED.{Prisma.RST}")
            return True
        elif cmd == "/_prove":
            if len(parts) < 2:
                ...
                print(f"{color}📐 LOGIC PROBE: Density={truth:.2f} [{verdict}]{Prisma.RST}")
        elif cmd == "/help":
            if len(parts) > 1:
                sub = parts[1]
                ...
                print("/teach, /kill, /seed, /focus, /status, /orbit, /gym, /mirror, /weave, /profile, /debug, /flag, /reset, /prove")
        else:
...
class DreamEngine:
    ...
    def _dream_of_others(cls):
        try:
            ...
            required_keys = ["meta", "trauma_vector", "core_graph"]
            if not all(k in data for k in required_keys):
                return f"{Prisma.RED}⚡ DREAM FRACTURE: Memory '{target_file}' is corrupted.{Prisma.RST}", None, 0.0
...
class LifecycleManager:
    ...
    def run_cycle(self, text, m, trace):
        ...
        if refusal_mode:
            ...
            return
        rupture_msg = None
        hat_success, hat_msg = self.eng.grey_hat.tip(m["physics"]["voltage"])
        if hat_success:
        ...
        if suburban_density > 0.2 and m["physics"]["voltage"] < 3.0:
            ...
            if rotted:
                print(f"   {Prisma.GRY}🌫️ EROSION: Forgot {len(rotted)} concepts (e.g., '{rotted[0]}').{Prisma.RST}")
            print(f"   {Prisma.WHT}► HINT: Light a fire (THERMAL) or break the box (/debug).{Prisma.RST}")
        ...
        status = self.eng.mitochondria.respirate(
            nutrient["yield"],
            effective_drag,
        ...
        if m["physics"]["E"] > 0.8:
            if "TIME_BRACELET" in self.eng.gordon.inventory:
                 lens_data = ("GLASS", f"ANACHRONISM DETECTED ({m['physics']['E']}). Payment processed via Time Bracelet. Immersion sustained.")
            else:
        ...
        self._render(m, meta, cosmic_msg, lens_data, mirror_msg, kintsugi_msg, rupture_msg, crystal_msg, ignition_msg, pareidolia_msg, theremin_msg, folly_msg, grind_msg, hat_msg if hat_success else None, lex_msg)
    ...
    def _calculate_health(self, glass_data):
        ...
        for key in self.eng.trauma_accum:
            self.eng.trauma_accum[key] = max(0.0, self.eng.trauma_accum[key] * decay_rate)
        elf.eng.health = min(BoneConfig.MAX_HEALTH, self.eng.health + health_impact)
        self.mitochondria.state.ros_buildup = min(BoneConfig.MAX_ROS, self.mitochondria.state.ros_buildup)
        m["physics"]["voltage"] = min(BoneConfig.MAX_VOLTAGE, m["physics"]["voltage"])
        if self.eng.health <= 0:
            self.eng.coma_turns = BoneConfig.COMA_DURATION
            self.eng.health = 20
        self.eng.mem.cannibalize(preserve_current=glass_data["clean_words"], current_tick=self.eng.tick_count)
    def _render(self, m, meta, cosmic_msg, lens_data, mirror_msg, kintsugi_msg, rupture_msg=None, crystal_msg=None, ignition_msg=None, pareidolia_msg=None, theremin_msg=None, folly_msg=None, grind_msg=None, hat_msg=None, lex_msg=None):
...
class BoneAmanita:
    ...
    def process(self, text):
        ...
        toxin, toxin_msg = self.immune.assay(text, enzyme, m["physics"]["repetition"], m["physics"])
        pizza_used = False
        ...
        if status == "NECROSIS":
		    print(f"{Prisma.RED}💀 NECROSIS EVENT: The Krebs Cycle has halted.{Prisma.RST}")
		    print(f"{Prisma.GRY}   The engine cannot burn fuel that does not exist. Initiating Coma.{Prisma.RST}")
		    self.coma_turns = BoneConfig.COMA_DURATION * 2
		    self.trauma_accum["SEPTIC"] += 0.5
		    print(f"{Prisma.RED}📉 TRAUMA VECTOR: Septic Shock (Mitochondrial Failure).{Prisma.RST}")
		    return
        if status == self.mitochondria.APOPTOSIS_TRIGGER:
            ...
            if self.training_mode:
                ...
                sacrificed = self.mem.cannibalize(
                	preserve_current=m["clean_words"],
