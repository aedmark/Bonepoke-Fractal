...
class TheGreyHat:
    def __init__(self):
        self.charges = 3
    def tip(self, voltage_spike):
        if self.charges > 0 and voltage_spike > 8.0:
            self.charges -= 1
            return True, f"🎩 THE GREY HAT: Tipped. Voltage ({voltage_spike}v) grounded to 0.5v. The grim answer is shorted out."
        return False, "The Hat stays on. No threat detected."
class LexNode:
...
class GordonKnot:
    ...
    inventory: List[str] = field(default_factory=lambda: ["POCKET_ROCKS", "SILENT_KNIFE", "STABILITY_PIZZA, "TIME_BRACELET"])
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v7.8.3 'LORE DUMP' {Prisma.RST}")
