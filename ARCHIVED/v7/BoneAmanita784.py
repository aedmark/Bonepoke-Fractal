# BONEAMANITA 7.8.4 - "THE RATIONALIST"
...
class TheGreyHat:
    ...
    def tip(self, voltage_spike, stability_index):
        if self.charges <= 0:
            return False, "The Hat is empty. You are on your own."
        if voltage_spike > self.max_tolerance:
            self.charges -= 1
            return True, f"🎩 THE GREY HAT: Circuit Breaker. {voltage_spike}v is unsafe for biologicals. Grounded."
        if voltage_spike > 8.0 and stability_index < 0.4:
            self.charges -= 1
...
class TheLexicon:
    ...
    def taste(word):
        ...
        # [PATCH 11]: Specific Elemental & Tonal Triggers (High Confidence)
        # These must run BEFORE generic suffix checks to catch "burning" as THERMAL, not KINETIC.

        # THERMAL: High energy, destructive heat.
        if any(x in w for x in ["burn", "fire", "flame", "heat", "char", "sear"]):
            return "thermal", 0.9

        # CRYO: Low energy, preservation, stasis.
        if any(x in w for x in ["ice", "cold", "freez", "frost", "snow", "chill"]):
            return "cryo", 0.9

        # SUBURBAN: Low stakes, polite, filler.
        # [SCHUR LENS]: These are words that try to avoid conflict.
        if w in ["nice", "okay", "fine", "lovely", "decent", "property", "lawn", "just"]:
            return "suburban", 0.8

        # PLAY: Whimsical, non-linear movement.
        if w.endswith(("le", "y", "pop", "zz")) and len(w) < 8:
            # Captures: wobble, fizz, dizzy, pop, bubble
            return "play", 0.7

        # --- EXISTING GENERIC HEURISTICS ---

        if w.endswith(("tion", "ment", "ness", "ity", "ism", "logy", "ence", "ance")):
            return "abstract", 0.9
        if w.endswith(("ing", "ed")):
            return "kinetic", 0.6
        if w.endswith(("ous", "ly", "ish")): # Removed 'y' as it is now covered by PLAY
            return "aerobic", 0.6
    ...
    def execute(self, text):
        ...
        if cmd == "/kill":
            ...
            if len(parts) > 1:
                ...
                print("/teach, /kill, /seed, /focus, /status, /orbit, /gym, /mirror, /weave, /profile, /debug, /flag")
        else:
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v7.8.4 'THE RATIONALIST' {Prisma.RST}")
