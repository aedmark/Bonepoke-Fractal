# BONEAMANITA 4.5 - "THE SPORE PRINT"
...
class MycelialNetwork:
    ...
    def save(self, health, stamina, mutations, isotopes):
        """Generates the Spore Print."""
        trauma = (BoneConfig.MAX_HEALTH - health) / BoneConfig.MAX_HEALTH
        ...
        if os.path.exists(path):
            try:
                with open(path, "r") as f:
                    ...
                    # INGEST MUTATIONS (The Virus)
                    if "mutations" in data:
                        print(f"{Prisma.CYN}[SPORE]: Ingesting lexical mutations...{Prisma.RST}")
                        for cat, words in data["mutations"].items():
                            if cat in TheLexicon.LEARNED_VOCAB:
                                # We trust the spore, but reset the tick to 0 (freshly learned)
                                for w in words: TheLexicon.LEARNED_VOCAB[cat][w] = 0
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v4.5 (THE SPORE PRINT){Prisma.RST}")
