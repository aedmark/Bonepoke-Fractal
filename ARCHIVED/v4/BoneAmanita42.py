# BONEAMANITA 4.2 - "THE SYMBIONT"
# Architects: SLASH, Eloise, & Clarence | Auditors: James Taylor & Andrew Edmark
# "The Mandate is GROWTH. The Method is HARDENING. The Spore is AWAKE. The Symbiont is EVOLVING."
...
class TheOracle:
    @staticmethod
    def triage(phys, stamina, charge): # Rename atp to stamina
        # Returns: (Priority_Score, Message, Color)
        omens = []
        drag = phys["narrative_drag"]
        volt = phys["voltage"]

        # If we have charge (>5.0), we aren't dying yet.
        if stamina < 5 and charge < 2.0:
            omens.append((100, "FAMINE IMMINENT. FEED OR DIE.", Prisma.RED))
        elif stamina < 15 and charge < 5.0:
            omens.append((45, "Resources Low. Conserve energy.", Prisma.YEL))
...
class BoneAmanita:
    ...
    def process(self, text):
        ...
        # BATTERY DISCHARGE (The Reserve Tank)
        # If we are starving (<20 STM) and have charge, burn it.
        if self.stamina < 20 and self.battery.current_charge > 0.5:
            # We want to get back to a safe buffer (30 STM)
            deficit = 30 - self.stamina
            # Efficiency: 1 Charge = 2 Stamina. So we need deficit / 2 charge.
            needed_charge = deficit / 2.0

            drained = self.battery.discharge(needed_charge)

            if drained > 0:
                recovered_stamina = drained * 2.0
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v4.2 'SYMBIONT' ONLINE.{Prisma.RST}")
