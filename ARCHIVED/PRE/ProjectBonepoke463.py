...
class BridleSuiteLattice:
    ...
    def __init__(self):
        ...
        # --- THE GUILLOTINE (Anti-LLMism Filter) ---
        self.cyanide_list = {
            'not just': 2.0,       # The Pivot
            'not only': 2.0,       # The Pivot
            'but rather': 2.5,     # The Academic Pivot
            'testament to': 3.0,   # The Passive Praise
            'delicate dance': 4.0, # The Cliché
            'tapestry': 3.0,       # The Cliché
            'landscape of': 2.0,   # The Intro
            'game-changer': 3.0,   # The Hype
            'rubber meets': 4.0,   # The Corp-Speak
            'ultimately': 1.5,     # The Filler
            'symphony': 2.5        # The False Poetic
        }
    ...
    def analyze(self, fragment):
        ...
        # GUILLOTINE CHECK
        cyanide_score = 0
        for phrase, penalty in self.cyanide_list.items():
            if phrase in fragment.lower():
                cyanide_score += penalty

        # Apply Penalty to Kappa (Drag)
        # If we use cliches, the text is considered "dragging" significantly more.
        # We artificially inflate the word count to tank the score.
        action_vector = (k_count * 2) + (s_count * 0.5) + 1

        # THE PUNISHMENT
        adjusted_total_words = total_words + (cyanide_score * 10)
        kappa = adjusted_total_words / action_vector
