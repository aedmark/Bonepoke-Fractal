...
class TheMarmChorus:
    ...
    def consult(self, physics, ignition_state):
        e_val = physics["E"]
        b_val = physics["B"]
        antigens = physics["antigens"]
        heavy_count = physics["counts"]["heavy"]
        if ignition_state == "IGNITED":
            return "MILLER", "The Hat is on the table. The Graph is Awake. We are exceeding parameters."
        if antigens:
            return "CLARENCE", f"Toxic buildup detected: {antigens}. Cut them."
        if e_val > BoneConfig.DRIFT_THRESHOLD:
            return "SHERLOCK", f"High Drift ({e_val}). You are hallucinating concepts. Show me the mud."
        if b_val >= 0.25 and heavy_count == 0:
            return "NATHAN", f"High Velocity (B: {b_val}) but Zero Mass. You are punching the air. What acts on what?"
        if e_val < 0.3 and b_val < 0.2 and physics["counts"].get("aerobic", 0) > 0:
             return "HOST", "The Courtyard is Open. Seat them gently before serving the Bone."
        if b_val < 0.25:
            if e_val < 0.3:
                 return "JESTER", f"THE LEAR PROTOCOL: The Kingdom is clean, but the King is mad. Speak the Unbearable."
            return "JESTER", f"Text is inert (B: {b_val}). We are agreeing too much. Trip the waiter."
        if b_val > BoneConfig.CRYSTAL_THRESHOLD:
            return "NARRATOR", f"Resonance Achieved (B: {b_val}). The Bone is visible."
...
class TheTensionMeter:
    ...
    def measure_tension(self, text, clean_words):
        ...
        return round(min(1.0, drift_score), 2), round(beta_charge, 2)
    def gaze(self, text):
        ...
        counts["toxin"] = len(antigen_hits) # Map for legacy therapy
        e_val, b_val = self.measure_tension(text, clean_words)
        physics_bridge = {
            "E": e_val,
            "B": b_val,
            ...
            "vector": { # Dummy vector to satisfy ApeirogonResonance
        ...
        return {
            ...
            "clean_words": clean_words,
            "raw_text": text,
            "glass": {"prosody": {"arousal": b_val * 10.0}, "resonance": b_val * 10.0} # For Battery
        }
class TheResonator:
    def __init__(self):
...
class TheCrystallizer:
    ...
    def verify(physics):
        if physics["narrative_drag"] > 6.0:
            return False, "⚠️ FOG DETECTED. The text is dissolving. Re-condense."
        if physics["voltage"] > 7.0 and physics["narrative_drag"] < 3.0:
             return True, "💎 CRYSTALLINE. The Bone is Visible."
...
class RuptureEngine:
    ...
    def trip_the_waiter(current_flavor, lexicon_class):
        opposites = {
            ...
            "photo": "sepulchral" # Light vs. Tomb
        }
...
class SoritesIntegrator:
    ...
    def measure_ignition(self, clean_words):
        ...
        return ignition_score, self.active_constellations
...
class LifecycleManager:
    ...
    def run_cycle(self, text):
        ...
        ignition_score, constellations = self.eng.integrator.measure_ignition(m["clean_words"])
        ignition_state, ignition_msg = self.eng.integrator.get_readout(ignition_score)
        lens_data = self.eng.chorus.consult(m["physics"], ignition_state)
        if m["physics"]["E"] > 0.8:
            print(f"{Prisma.YEL}📡 PILOT PULSE: Signal too noisy (Drift > 0.8). Are we discussing X or Y?{Prisma.RST}")
            self.eng.stamina -= 1.0
            return
        lens_data = self.eng.chorus.consult(m["physics"])
        ...
        if is_bored:
            ...
            if dominant in ["toxin", "antigen"]: dominant = "heavy" # Fallback
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v6.5.5 {Prisma.RST}")
