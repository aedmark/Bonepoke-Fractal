# # ---------------------------------------------------------------------------
# BONEAMANITA 0.5.1 (A VARIANT STRAIN OF THE BONEPOKE ENGINE v4.3.5) - "The Archetypal Edition"
# Architects: James Taylor, Andrew Edmark | Auditors: SLASH, THE MYCELIAL MIRROR NETWORK
#
# "Feed the soil, poison the weak. Logic is the stalk; chaos is the spore. Mind the roots."
# ---------------------------------------------------------------------------
...
class ArchetypePrism:
    ...
    def __init__(self):
        ...
        self.archetypes = {
            ...
            "THE COSMIC TRASH PANDA":   {"B": 0.2, "E": 1.0, "desc": "One man's trash is a raccoon's treasure."}
        }
...
class ChronosAnchor:
    ...
    def _is_likely_verb(self, word, prev_word):
        # The Prison-Shank Stemmer: Logic, not lists.
        w = word.lower()
        stem = w
        # 1. Strip Suffixes
        if w.endswith("ies") and len(w) > 4: stem = w[:-3]
        elif w.endswith("es") and not w.endswith(("aes", "oes")):
             if w.endswith(("sses", "shes", "ches", "xes")): stem = w[:-2]
        elif w.endswith("s") and not w.endswith(("ss", "us", "is")): stem = w[:-1]

        # 2. Logic Check
        if stem == w: return False # If we didn't strip anything, assume not a present verb
        if prev_word in self.articles: return False # "The runs" -> Noun
        if prev_word in self.pronouns: return True  # "He runs" -> Verb
        return True # Default to verb if ambiguous
...
class BonepokeCore:
    ...
    def process(self, text, parent_id=None):
        ...
        # Only trigger Clarence if drag is significantly higher than USER'S normal
        is_abnormal_drag = phys['narrative_drag'] > (avg_drag * 1.5) and phys['narrative_drag'] > 3.0
...
if __name__ == "__main__":
    ...
    print("--- BONEAMANITA v0.5.1: THE ARCHETYPAL EDITION ---")
