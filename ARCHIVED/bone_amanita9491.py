# BONEAMANITA 9.4.9.1 "THE CURTAIN CALL (BLACKOUT)"
...
from bone_shared import (
    MycelialNetwork, DreamEngine, MirrorGraph, ApeirogonResonance, 
    ViralTracer, SoritesIntegrator, LimboLayer, TheLexicon)
...
from bone_biology import (
    ...
    LichenSymbiont, NeuroPlasticity, MetabolicGovernor, ParasiticSymbiont
)
...
class BoneAmanita:
    def __init__(self, memory_layer=None, lexicon_layer=None):
        ...
        _lex = lexicon_layer if lexicon_layer else TheLexicon
        
        # Construct the strongly-typed container
        ...
        # 5. THE LOOPS (Updated to use dot notation)
    ...
    def process_turn(self, user_message: str) -> Dict[str, Any]:
        ...
        toxin_type, toxin_msg = self.bio.immune.assay(
            user_message, "NARRATIVE", phys["repetition"], phys, self.phys.pulse.get_status()
        )
        if toxin_type:
        ...
        # SomaticLoop is already fixed to handle objects
        ...
        # --- 5. WORLD DYNAMICS ---
        is_face, p_msg = self.check_pareidolia(context["clean_words"])
