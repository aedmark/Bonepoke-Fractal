...
@dataclass
class GraphState:
    """The Scratchpad: Holds the unified state of the conversation."""
    raw_input: str
    ...
    output: str = ""
    next_node: str = "PERCEPTION"
    terminate: bool = False
class TheBoneGraph:
    def __init__(self, engine_ref):
        self.eng = engine_ref
    def perception_node(self, state: GraphState) -> GraphState:
        """Node 1: Perception + Autonomic Nervous System."""
        m = self.eng.phys['tension'].gaze(state.raw_input, self.eng.mind['mem'].graph)
        state.physics = m["physics"]
        state.clean_words = m["clean_words"]
        shift_msg = self.eng.bio['governor'].shift(
            state.physics, self.eng.bio['plasticity'].history
        )
        if shift_msg:
            print(shift_msg)
        if self.eng.bio['governor'].mode == "FORGE":
            state.physics["narrative_drag"] = 0.0
        if self.eng.safety.fever_dream_active:
            state.physics["voltage"] = 99.9
            state.physics["narrative_drag"] = 0.0
            state.physics["kappa"] = 0.0
        if state.raw_input.startswith("/"):
            state.next_node = "COMMAND"
        else:
            state.next_node = "IMMUNITY" 
        return state
    def immunity_node(self, state: GraphState) -> GraphState:
        """Node 2: Immunity + Pain Reflex."""
        is_painful, pain_msg = self.eng.gordon.flinch(state.clean_words)
        if is_painful:
            print(f"\n{Prisma.RED}{pain_msg}{Prisma.RST}")
            state.terminate = True
            state.next_node = "END"
            return state
        pulse = self.eng.phys['pulse'].check_pulse(state.clean_words)
        pulse_status = self.eng.phys['pulse'].get_status()
        toxin, msg = self.eng.bio['immune'].assay(
            state.raw_input, "NARRATIVE", pulse, state.physics, pulse_status
        )
        if toxin:
            print(f"{Prisma.RED}{msg}{Prisma.RST}")
            if toxin in ["AMANITIN", "CYANIDE_POWDER"]:
                state.terminate = True
                self.eng.health -= 20.0
                self.eng.mind['mem'].cannibalize()
            elif toxin == "MUSCIMOL":
                state.physics["narrative_drag"] += 10.0
            elif toxin == "GLYPHOSATE":
                self.eng.health -= 5.0
            state.bio['toxin_flag'] = True 
        if state.terminate:
            state.next_node = "END"
        else:
            state.next_node = "GATE"
        return state
    def gate_node(self, state: GraphState) -> GraphState:
        """Node 3: The Tangibility Gate (Gas vs Bone)."""
        passed, msg = self.eng.phys['gate'].weigh(state.physics, self.eng.stamina)
        if not passed:
            print(f"\n{msg}")
    ...
    def memory_node(self, state: GraphState) -> GraphState:
        """Node 4: RAG / Context Retrieval (The Temporal Interpreter)."""
        self.eng.mind['mem'].encode(state.clean_words, state.physics, self.eng.bio['governor'].mode)
        state.memory['context'] = "Context Loaded." 
        state.next_node = "COGNITION"
        return state
    def cognition_node(self, state: GraphState) -> GraphState:
        """Node 5: The Marm Chorus (Thinking)."""
        feedback = {"STATIC": 0.0, "INTEGRITY": state.physics["truth_ratio"], "FORCE": 0.5, "BETA": 1.0}
        bio_state = self.eng.bio['soma'].digest_cycle(state.raw_input, state.physics, feedback)
        current_kappa = state.physics.get("kappa", 0.5)
        mutations = self.eng.bio['mito'].adapt(self.eng.health, kappa=current_kappa)
        if mutations.get("hypertrophy_event"):
            bloom_msg = self.eng.mind['mem'].tend_garden(state.clean_words)
        ...
        state.bio.update(bio_state)
        lens, thought = self.eng.mind['chorus'].consult(
            state.physics, 0.0, False, bio_state['chem'], self.eng.gordon.inventory, self.eng.health
        )
        state.mind = {"lens": lens, "thought": thought}
        beta = state.physics.get("beta_index", 1.0)
        if beta < 0.1:
             state.mind["lens"] = "JOEL" 
             anomaly = TheLexicon.harvest("heavy").upper()
             rupture_text = (
                 f"Wait. The consensus here is suffocating. "
                 f"We are ignoring the {anomaly}. "
                 f"Actually, {state.mind['thought']} -> But consider the {anomaly}."
             )
             state.mind["thought"] = rupture_text
             state.physics["zone_color"] = "VIOLET"
             state.physics["zone"] = "BASEMENT"
             print(f"{Prisma.MAG}⚡ MVB RUPTURE TRIGGERED (β: {beta}){Prisma.RST}")
        state.next_node = "ACTION"
        return state
    def action_node(self, state: GraphState) -> GraphState:
        """Node 6: System Dynamics + Adaptation + Broadcast."""
        world_state = self.eng.bio['kinetic'].update_world(state.physics, state.bio, state.mind)
        is_stuck, resin, t_msg, t_crit = self.eng.phys['theremin'].listen(
            state.physics, self.eng.bio['governor'].mode
        )
        if t_msg: print(f"\n{t_msg}")
        if t_crit == "AIRSTRIKE":
            self.eng.health -= 20.0
            print(f"{Prisma.RED}   >>> AIRSTRIKE DAMAGE: -20 HP.{Prisma.RST}")
        c_state, c_val, c_msg = self.eng.phys['crucible'].audit_fire(state.physics)
        if c_msg: print(f"\n{c_msg}")
        if c_state == "MELTDOWN":
            self.eng.health -= c_val
        orbit_type = world_state.get("orbit", ["VOID"])[0]
        if orbit_type == "VOID_DRIFT":
            state.physics["voltage"] = max(0.0, state.physics["voltage"] - 0.5)
        elif orbit_type == "LAGRANGE_POINT":
            state.physics["narrative_drag"] = max(0.1, state.physics["narrative_drag"] - 2.0)
        chem_state = state.bio['chem']
        chem_state["ATP"] = self.eng.bio['mito'].state.atp_pool
        feedback = {"STATIC": 0.0, "INTEGRITY": state.physics["truth_ratio"], 
                    "FORCE": 0.5, "BETA": state.physics.get("beta_index", 1.0)}
        ...
        self.eng.projector.broadcast(
            ...
            {"bio": state.bio, "mind": state.mind, "world": world_state}, 
            (state.mind['lens'], state.mind['thought'])
        )
        state.next_node = "END"
        return state
    def run(self, input_text: str):
        """The Conductor: Orchestrates the flow."""
        current_state = GraphState(raw_input=input_text)
        nodes = {
            "PERCEPTION": self.perception_node,
            "IMMUNITY": self.immunity_node,
            "GATE": self.gate_node,
            "MEMORY": self.memory_node,
            "COGNITION": self.cognition_node,
            "ACTION": self.action_node,
        }
        while current_state.next_node != "END" and not current_state.terminate:
            target_node = current_state.next_node
            if target_node == "COMMAND":
                self.eng.cmd.execute(input_text)
                break
            if target_node in nodes:
                current_state = nodes[target_node](current_state)
            else:
                print(f"{Prisma.RED}❌ GRAPH FRACTURE: Unknown Node '{target_node}'{Prisma.RST}")
                break
...
class SystemDiagnostician:
    @staticmethod
    def diagnose(physics, chem, inventory, ignition_state, governor_mode, is_stuck, current_health):
        ...
        if governor_mode == "LABORATORY":
            return "SHERLOCK", "The data is clean. Proceed."
        kappa = physics.get("kappa", 0.0)
        voltage = physics.get("voltage", 0.0)
        if kappa > 0.85:
            return "GORDON", f"Walls are fake (κ: {kappa}). Cut the string."
        if is_stuck:
            return "GLASS", "🔊 FEEDBACK LOOP: Vibrating in place. DAMPEN IT."
        if physics.get("beta_index", 0.0) > 2.0 and voltage > 10.0:
            return "JADE", f"The Architecture is holding (β: {physics['beta_index']}). We are building with Bone."
        if chem["COR"] > 0.6:
            return "SHERLOCK", f"CORTISOL SPIKE ({chem['COR']}). Logic Required."
        if chem["ADR"] > 0.8:
            return "NATHAN", f"ADRENALINE CRITICAL. Survive."
        if chem["OXY"] > 0.75:
            return "HOST", f"OXYTOCIN HIGH. Welcome them."
        e_val = physics["E"]
        if e_val > BoneConfig.DRIFT_THRESHOLD:
            if "TIME_BRACELET" in inventory:
                return "POPS", f"Anachronism Detected (E: {e_val}). Badge verified. Carry on."
        ...
        if hits > 0 and current_health > 90.0:
            return "DEREK", f"🎬 CUT! You walked in too early. The body isn't dead yet. (Health {int(current_health)}% - Stop Overacting)."
        b_val = physics["B"]
...
class TheMarmChorus:
    ...
    def load_lenses(cls):
        try:
            ...
            cls.LENSES = {"NARRATOR": {"color": Prisma.GRY, "role": "The Witness", "trigger": "FALLBACK_MODE"}}
    @staticmethod
    def check_theatre(tick_count):
        if random.random() < 0.05:
            return (f"\n{Prisma.VIOLET}🎭 HN-BREAK (Tick {tick_count}):\n"
    ...
    def consult(cls, physics, ignition_state, is_stuck, chem, gordon_inventory, current_health, governor_mode="COURTYARD"):
        lens_name, msg = SystemDiagnostician.diagnose(
            physics, chem, gordon_inventory, ignition_state, governor_mode, is_stuck, current_health
        )
        cls.LENS_HISTORY.append(lens_name)
        return lens_name, msg
@dataclass
class GordonKnot:
...
class TheProjector:
    def broadcast(self, m, signals, lens_data):
        p = m["physics"]
        z_color_key = p.get("zone_color", "OCHRE")
        color_code = getattr(Prisma, z_color_key, Prisma.OCHRE)
        lens_name = lens_data[0]
        lens_msg = lens_data[1]
        print(f"\n{color_code}{lens_msg}{Prisma.RST}\n")
        if p["antigens"]:
             print(f"{Prisma.GRY}   (Filtered Toxins: {p['antigens']}){Prisma.RST}")
        # The Gordon Check (Optional: Uncomment for debugging)
        # print(f"{Prisma.GRY}[DEBUG] E:{p.get('E_score',0)} | B:{p.get('B_score',0)} | β:{p.get('beta_index',0)}{Prisma.RST}")
class RuptureEngine:
    @staticmethod
...
class BoneAmanita:
    def __init__(self):
        ...
        self.graph_runner = TheBoneGraph(self)
        ...
        self.bio['soma'] = SomaticLoop(self)
        self.bio['kinetic'] = KineticLoop(self)
    def _trigger_death(self):
        """Restored Death Protocol."""
        ...
        print(f"{Prisma.GRY}   (Type /exit to save genetics.){Prisma.RST}")
        self.health = 0
    def process(self, text: str):
        self.graph_runner.run(text)
        if hasattr(self.graph_runner, 'last_state') and self.graph_runner.last_state.terminate:
             pass
        if self.health <= 0 or self.bio['mito'].state.atp_pool <= 0:
            self._trigger_death()
            return
        self.tick_count += 1
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.paint('>>> BONEAMANITA 9.0', 'G')}")
