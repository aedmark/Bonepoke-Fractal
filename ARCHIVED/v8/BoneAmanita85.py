...
class SomaticLoop:
    ...
    def metabolize(self, text, physics_packet, trace_data):
        ...
        chem_state = self.bio['endo'].metabolize(trace_data, ...)
...
class KineticLoop:
    def __init__(self, phys_layer, gordon):
        self.phys = phys_layer
        self.gordon = gordon
    def update_world(self, physics_packet, bio_state, mind_state):
        orbit_state, _, _ = self.eng.cosmic.analyze_orbit(...)
...
class LifecycleManager:
    def __init__(self, engine):
        ...
        self.kinetic = KineticLoop(engine.phys, engine.gordon)
    def run_cycle(self, text, m, trace):
        bio_state = self.soma.metabolize(text, m["physics"], trace)
        ...
        if mind_state["mode"] == "REFUSAL":
            print("🚫 REFUSAL.")
            return
        world_state = self.kinetic.update_world(
            m["physics"],
            bio_state,
            mind_state
        )
