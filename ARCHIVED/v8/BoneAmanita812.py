# BONEAMANITA 8.1.2 - "THE LUCID DREAM"
...
class RefusalEngine:
    ...
    def execute_fractal(self, query, kappa=0.5):
        ...
        return (
            ...
            f"{self.execute_fractal(sub_query, kappa)}" # Pass kappa down
        )
    @classmethod
...
class TheCartographer:
    ...
    def survey(cls, text, memory_graph, bio_metrics, limbo=None):
        ...
        if not knots:
            if config["max_anchors"] == 2: # Low Energy check
        ...
        anchor_strength = sum(memory_graph[a]["edges"][t] for a in anchors for t in memory_graph[a]["edges"]) # Total mass of anchors
    ...
    def execute(self, text):
        ...
        if cmd == "/kill":
            ...
            if len(parts) > 1:
                ...
                print(
                    "/teach, /lineage, /strata, /kill, /seed, /focus, /status, /orbit, /gym, /mirror, /weave, /profile, /debug, /flag, /reset, /prove"
                )
        else:
...
class LifecycleManager:
    ...
    def run_cycle(self, text, m, trace, has_bracelet, is_hybrid, enzyme):
        ...
        print(f"\n{Prisma.paint('--- [ BONEAMANITA 8.1.2 ] ---', '0')}")
...
class MetabolicGovernor:
    ...
    def shift(self, voltage, abstract_density, heavy_density):
        self.mode = "COURTYARD"
        self.psi_mod = 0.2
        self.kappa_target = 0.1
        self.drag_floor = 2.0
        if abstract_density > 0.3 or (voltage > 3.0 and voltage < 7.0):
            self.mode = "LABORATORY"
            self.psi_mod = 0.8
            self.kappa_target = 0.5
            self.drag_floor = 0.5
            return f"{Prisma.CYN}🔬 STATE SHIFT: Entering LABORATORY (Ψ: {self.psi_mod}).{Prisma.RST}"
        if voltage >= 7.0 and heavy_density > 0.2:
            self.mode = "FORGE"
            self.psi_mod = 0.95
            self.kappa_target = 0.9
            self.drag_floor = 0.0
            return f"{Prisma.RED}🔥 STATE SHIFT: THE FORGE IS LIT (Drag: 0.0).{Prisma.RST}"
        return None
class BoneAmanita:
    def __init__(self):
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.paint('>>> BONEAMANITA 8.1.2', 'G')}")
