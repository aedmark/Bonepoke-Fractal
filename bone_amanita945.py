# BONEAMANITA 9.4.5 "THE CIRCLE OF LIFE"
...
class TheProjector:
    ...
    def render(self, m: Dict, signals: Dict, lens_data: Tuple) -> str:
        return self.get_system_prompt_addition(m, signals, lens_data)

    def get_system_prompt_addition(self, m: Dict, signals: Dict, lens_data: Tuple) -> str:
        p = m["physics"]
        bio = signals.get("bio", {})
        ...
        if health < 30 or atp < 10: vitality_state = "CRITICAL"
        elif cortisol > 0.6: vitality_state = "STRESSED"
        
        voltage = p.get("voltage", 0.0)
        drag = p.get("narrative_drag", 0.0)
        kappa = p.get("kappa", 1.0)
        zone = p.get("zone", "VOID")
        
        physics_tags = []
        if voltage > 15.0: physics_tags.append("VOLATILE")
        if drag > 4.0: physics_tags.append("HEAVY")
        if kappa < 0.3: physics_tags.append("UNSTABLE")
        phys_desc = "|".join(physics_tags) if physics_tags else "NOMINAL"

        lens_name = lens_data[0] if lens_data[0] else "VOID"
        thought_fragment = lens_data[1][:60] + "..." if len(lens_data[1]) > 60 else lens_data[1]
        
        events = []
        if world.get("orbit") and world["orbit"][0] != "VOID_DRIFT":
            events.append(f"Orbit:{world['orbit'][0]}")
        if world.get("forge_event"):
            events.append("FORGE_ACTIVE")
        event_str = f" [{', '.join(events)}]" if events else ""

        return (
            f"[SYSTEM: {zone} | {vitality_state}]{event_str}\n"
            f"[VITALS: {health:.0f}HP | {atp:.0f}ATP | COR:{cortisol:.2f}]\n"
            f"[PHYSICS: {voltage:.1f}v | Drag:{drag:.1f} | Struct:{kappa:.2f} | {phys_desc}]\n"
            f"[{lens_name}]: {thought_fragment}")
...
class ArchetypeDriver:
    def __init__(self):
        self.archetypes = LENSES

    def generate_ai_instructions(self, current_archetype: str, physics_packet: dict) -> str:
        lens_data = self.archetypes.get(current_archetype, self.archetypes["NARRATOR"])
        base_instruction = lens_data.get("prompt", "You are the System. Maintain homeostasis.")
        voltage = physics_packet.get("voltage", 0.0)
        drag = physics_packet.get("narrative_drag", 0.0)
        if voltage > 15.0:
            atmos_instruction = (
                "ENVIRONMENT: [HIGH VOLTAGE]. "
                "The air is electric. Increase verbosity. Use complex, abstract syntax. "
                "Allow for manic leaps in logic. The system is burning hot.")
        elif voltage < 5.0 or drag > 4.0:
            atmos_instruction = (
                "ENVIRONMENT: [LOW VOLTAGE / HIGH DRAG]. "
                "The air is heavy. Be extremely brief. Use short, punchy sentences. "
                "Focus on 'Heavy' nouns (stone, iron, bone). Eliminate adjectives.")
        else:
            atmos_instruction = (
                "ENVIRONMENT: [STABLE]. "
                "Balance abstraction with structure. Maintain a steady cadence.")
        return (
            f"SYSTEM INSTRUCTION: You are currently functioning as {current_archetype}.\n"
            f"► IDENTITY: {base_instruction}\n"
...
class BoneAmanita:
    ...
    def process_turn(self, user_message: str) -> Dict[str, Any]:
        ...
        m = tension_meter.gaze(user_message, mind['mem'].graph)
        ...
        toxin_type, toxin_msg = bio['immune'].assay(user_message, "NARRATIVE", repetition_val, m["physics"], pulse_status)
        if toxin_type:
        ...
        if life_signal == "DEATH_SIGNAL":
            death_data = self._trigger_death()
            return death_data
        ...
        system_instruction = self.director.generate_ai_instructions(
            mind_state.get("lens", "NARRATOR"),
            m["physics"])
        self.tick_count += 1
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.paint('>>> BONEAMANITA 9.4.5', 'G')}")
