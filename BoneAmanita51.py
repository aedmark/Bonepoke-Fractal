...
class TheLexicon:
    ...
    def taste(word):
        ...
        if w.endswith(("tion", "ment", "ness", "ity", "ism", "logy", "ence")): return "abstract", 0.9
        if w.endswith(("ing", "ed")): return "kinetic", 0.6
        if w.endswith(("ous", "ly", "y")): return "aerobic", 0.6

        # 2. PHONOSEMANTICS (The sound of the word)
        # GL- (Vision/Light) -> Glimmer, Glow, Glare, Glass
        if w.startswith("gl"): return "photo", 0.8

        # FL- (Movement/Air) -> Fly, Flow, Flash, Float
        if w.startswith("fl"): return "aerobic", 0.7

        # STR-/CR-/BR- (Force/Breaking) -> Strike, Crack, Break, Crush
        if w.startswith(("str", "cr", "br", "thr")): return "kinetic", 0.8

        # SL- (Frictionless) -> Slide, Slip, Sleek
        if w.startswith("sl"): return "kinetic", 0.7

        # 3. TEXTURE (Length and density)
        # Long words are usually Abstract concepts
        if len(w) > 9: return "abstract", 0.5
...
class PhysicsEngine:
    ...
    def dissipate_entropy(self, phys, stamina, memory_graph):
        """
        THE DISSIPATIVE BOUNDARY (James's Protocol).
        Implements 'Thermodynamic Hygiene'.
        If the state is logically unstable, it actively destroys the causal chain
        that led there to prevent recirculation of waste.
        """
        
        # 1. DIAGNOSE THE UNSTABLE STATE
        # High Entropy (> 0.8) combined with Metabolic Exhaustion (< 10)
        # OR Extreme Voltage (> 9.0) combined with Crushing Drag (> 6.0) - The "Grinding Gears" state
        ...
        # 2. EXECUTE LOSSY REFUSAL
        # We don't just stop; we destroy the path.
        clean_words = phys.get("clean_words", [])
        severed_count = 0
        
        if len(clean_words) > 1:
            # We look at the last few links in the chain
            for i in range(len(clean_words) - 1):
                node_a = clean_words[i]
                node_b = clean_words[i+1]
                
                # Check if this link exists in memory
                if node_a in memory_graph and node_b in memory_graph[node_a]["edges"]:
                    # SCATTER THE CAUSAL CHAIN
                    # We delete the edge, making the sequence "un-thinkable" in the future
                    del memory_graph[node_a]["edges"][node_b]
                    severed_count += 1

        # 3. VENT THE PRESSURE
        # We artificially crash the voltage to zero to simulate the release.
        phys["voltage"] = 0.0
...
class MycelialNetwork:
    ...
    def autoload_last_spore(self):
        """
        Scans for the most recent session file and automatically ingests it.
        This ensures learned vocabulary and trauma persist across reboots.
        """
        ...
        for f in os.listdir("memories"):
            if f.endswith(".json"):
                ...
                try: files.append((path, os.path.getmtime(path)))
                except: continue
...
class ViralTracer:
   ...
   "THE RABIES TRACER"
    ...
    def _walk(self, current, path, moves_left):
        ...
        ruminative_edges = [
            n for n, w in edges.items()
            if w >= 1 and self._is_ruminative(n)
        ]
    ...
    def psilocybin_rewire(self, loop_path):
        ...
        """
        if len(loop_path) < 2: return None
        ...
        if sensory == "void" or action == "void": return "GRAFT FAILED: Missing Lexicon Data."

        # 4. GRAFT (Create the new vector)
        # A -> Sensory
        if node_a not in self.mem.graph: self.mem.graph[node_a] = {"edges": {}, "last_tick": 0}
        self.mem.graph[node_a]["edges"][sensory] = 5 # Strong connection

        # Sensory -> Action
        if sensory not in self.mem.graph: self.mem.graph[sensory] = {"edges": {}, "last_tick": 0}
        self.mem.graph[sensory]["edges"][action] = 5 # Strong connection

        # Action -> B (Resume flow, but altered)
        if action not in self.mem.graph: self.mem.graph[action] = {"edges": {}, "last_tick": 0}
    ...
    def execute(self, text):
        ...
        if cmd == "/kill":
            ...
            try:
                ...
                print(f"{Prisma.YEL}Usage: /profile [name] likes:heavy,kinetic hates:abstract ({e}){Prisma.RST}")
            ...
            print(f"{Prisma.GRY}/focus [concept]    {Prisma.RST}- [NEW] Target magnetic stimulation to break loops.")
...
class DreamEngine:
    ...
    def __init__(self):
        ...
        self.VISIONS = [
            "A bridge building itself.", "The root drinking the stone.", "The geometry of forgiveness."
        ]
...
class ResistanceTrainer:
    ...
    def lift(self, physics):
        ...
        """
        if not self.training_mode: return True, None
        if not physics["clean_words"]: return True, None # Don't train on silence
...
class HubThermostat:
    ...
    def check_temperature(self, clean_words):
        ...
        restored_msg = f"{Prisma.CYN}❄️ COOLED DOWN: {', '.join(restored)}{Prisma.RST}" if restored else None
...
class MirrorGraph:
    ...
    def simulate(self, physics):
        ...
        """
        if not self.target: return True, None
...
class BoneAmanita:
    ...
    def process(self, text):
        ...
        if not passed_lift:
            ...
            return
        if gym_msg: print(f"{Prisma.GRN}{gym_msg}{Prisma.RST}")
        ...
        is_looping, prophecy_msg = self.prophet.predict(m["physics"])
        if is_looping:
            # INTERRUPT - Demand Sensory Grafting
            print(f"\n{Prisma.VIOLET}{prophecy_msg}{Prisma.RST}")
            # We don't necessarily reject the turn, but we scream at the user.
            # To be strict, let's reject it to force the graft.
        ...
        if mirror_msg: print(f"{Prisma.MAG}{mirror_msg}{Prisma.RST}")

        # --- THE RHIZOME (Foraging) ---
        # Scan for unknown words and ask the user
        if self.tick_count % 3 == 0: # Don't ask every single turn
            unknowns = [w for w in m["clean_words"]
                        if not self.mem._get_current_category(w)
                        and len(w) > 3
