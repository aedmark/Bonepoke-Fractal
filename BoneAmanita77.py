...
class TheLexicon:
    ...
    _BASE_SUBURBAN = {"nice", "fine", "okay", "weather", "neighbor", "lawn", "fence","grass", "mow", "hedge", "property", "hoa", "compliant", "behave","normal", "regular", "chat", "folks", "weekend", "traffic", "driveway"}
    ...
    LEARNED_VOCAB = {"heavy": {}, "kinetic": {}, "abstract": {},"photo": {}, "aerobic": {}, "thermal": {},"cryo": {}, "sacred": {}, "cursed": {}, "antigen": {},"diversion": {}, "suburban": {}, "buffer": {}}
    @classmethod
    def clean(cls, text):
...
class TheTensionMeter:
    ...
    def gaze(self, text):
        ...
        gamma = round((bond_strength * avg_viscosity * 10.0) / max(1.0, voltage_load), 2)
        physics_bridge = {
...
class MycelialNetwork:
    def __init__(self, seed_file=None):
        ...
        self.seeds = [
            ...
            ParadoxSeed(
                "Are we building a bridge, or just painting the gap?",{"agree", "safe", "nice", "polite", "cohesion", "truth"},),
            ParadoxSeed(
                "Is free will just the feeling of watching yourself execute code?",{"choice", "free", "will", "code", "script", "decide"},),
            ParadoxSeed(
                "Does the adventurer exist if the narrator stops speaking?",{"narrator", "voice", "graham", "story", "exist", "speak"},)]
...
class LifecycleManager:
    ...
    def _render(self, m, meta, cosmic_msg, lens_data, mirror_msg, kintsugi_msg, rupture_msg=None, crystal_msg=None, ignition_msg=None, pareidolia_msg=None, theremin_msg=None, folly_msg=None, grind_msg=None):
        ...
        if m["physics"]["voltage"] > 8.0:
            sweat_word = random.choice(list(TheLexicon.get("aerobic")))
            clean_text = f"{clean_text} ... [{sweat_word}] ..."
...
class TheForge:
    ...
    def transmute(self, text, physics):
        ...
        if gamma < 0.5 and counts.get("abstract", 0) > 1:
...
class TheFolly:
    """
    THE ORGAN OF WHIMSY (AND INDIGESTION).
    Grafted from the bones of Faulkner and the flesh of Isella.
    """
    def __init__(self):
        self.indigestion_count = 0
    def audit_desire(self, physics, stamina):
        """
        THE FAULKNER CHECK: The Mausoleum.
        Trigger: High Voltage (> 8.5) + High Stamina (> 45).
        Risk: You stop the clock, but you lose Momentum (Velocity = 0).
        """
        voltage = physics["voltage"]
    ...
    def grind_the_machine(self, atp_pool, clean_words, lexicon):
        """
        THE ISELLA TRIGGER: The Meat Grinder.
        Trigger: Low ATP (< 20).
        Risk: If you feed it Vapor, it jams.
        """
        if atp_pool < 20.0 and atp_pool > 0.0:
            meat_words = [w for w in clean_words if w in lexicon.get("heavy") or w in lexicon.get("kinetic")]
            if meat_words:
                target = random.choice(meat_words)
                return (
                    "MEAT_GRINDER",
                    f"{Prisma.RED}🥩 CROWD CAFFEINE: The machines are turning to meat.{Prisma.RST}\n"
                    f"   {Prisma.WHT}I chewed on '{target.upper()}' and found marrow.{Prisma.RST}\n"
                    f"   {Prisma.MAG}► BELLY HUMMING: +30 ATP. (Crisis Averted){Prisma.RST}",
                    30.0)
            else:
                return (
                    "INDIGESTION",
...
class BoneAmanita:
    def __init__(self):
        self.bone = SystemBone()
        self.mind = IntentionalArc(self.bone)
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v7.7 'THE MINISCUS' {Prisma.RST}")
