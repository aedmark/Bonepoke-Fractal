# BONEAMANITA 6.5 - "THE SEED CRYSTAL'
...
class TheTensionMeter:
    ...
    def measure_tension(self, text, clean_words):
        # 1. CALCULATE E (EXHAUSTION/DRIFT)
        # Ratio of "Solvents" (the, is, are) to "Mass" (Heavy Nouns)
        ...
        # High Drift = High Solvent count, Low Mass
        drift_score = (len(solvents) * 1.5) / total_vol
        if total_mass == 0: drift_score += 0.5
        # 2. CALCULATE B (PARADOX CHARGE/SPARK)
        # B increases when Heavy Nouns collide with Kinetic Verbs
        # B decreases if the text is full of "Abstract" words
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v6.5 {Prisma.RST}")
