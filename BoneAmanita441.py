# BONEAMANITA 4.4.1 - "THE SPHERICAL FUNGUS COW"
...
class CourtyardInterface:
    ...
    def check_atmosphere(self, phys):
        ...
        """
        # 1. Safety Check: Toxins force a Lab Audit
        if phys["counts"].get("toxin", 0) > self.TOXIN_TOLERANCE:
            self.mode = "LABORATORY"
            return self.mode, "☣️ TOXIN DETECTED. SAFETY INTERLOCK ENGAGED."

        # 2. Tension Check: High Voltage means we are cooking
        if abs(phys["voltage"]) > self.VOLTAGE_CAP:
            self.mode = "LABORATORY"
            return self.mode, "⚡ HIGH TENSION. ENTERING LABORATORY."

        # 3. Drag Check: Heavy Drag means we are stuck
        if phys["narrative_drag"] > self.DRAG_CAP:
            self.mode = "LABORATORY"
            return self.mode, "⚓ HEAVY DRAG. DIAGNOSTIC MODE ACTIVE."

        # 4. If all is calm, we enter the Courtyard
        self.mode = "COURTYARD"
        return self.mode, "☕ ATMOSPHERE STABLE."
...
class ValveSystem:
    ...
    def check_pressure(self, phys):
        ...
        # We use Beta (Friction) and Voltage (Interest)
        ...
        # We need at least 3 turns to judge a pattern
        if len(self.history) < 3: return None

        # CALCULATE STAGNATION
        # If Beta is consistently low (Smooth) AND Voltage is low (Boring)
        ...
        if avg_beta < 0.5 and avg_volt < 3.0:
            self.cooldown = 5 # Don't rupture twice in a row
            self.history = [] # Reset memory
...
class Prisma:
    ...
    # INDIGO (The Bone): Hard Truth, Structure, Math. (Mapped to Blue/Cyan)
    INDIGO = BLU
    # OCHRE (The Ground): Warmth, Empathy, Courtyard. (Mapped to Yellow)
    OCHRE = YEL
    # VIOLET (The Rupture): Anomalies, Irony, 32-Valve. (Mapped to Magenta)
    VIOLET = MAG
    # SLATE (The System): Diagnostics, Meta-Commentary. (Mapped to Grey)
    SLATE = GRY
    ...
    def wrap(val, limit, invert=False):
        ...
        if val > limit: return f"{Prisma.OCHRE}{val}{Prisma.RST}" # Yellow became Ochre
        return f"{Prisma.GRN}{val}{Prisma.RST}"

    @staticmethod
    def paint(text, mode):
        """
        Applies Semantic Lighting based on the VSL Mode.
        """
...
class ParadoxSeed:
    """
    A dormant question waiting for the right context to bloom.
    Represents the 'Unasked Questions' from the Interstitial Garden.
    """
    ...
    def water(self, words, amount=1.0):
        ...
        # Check if current conversation touches the seed's triggers
        intersection = set(words) & self.triggers
        if intersection:
            # Growth is exponential based on relevance
            self.maturity += (amount * len(intersection))

        # Passive growth (Time/Entropy)
        self.maturity += 0.05
...
class MycelialNetwork:
    """
    FORMERLY DEEP STORAGE.
    Now acts as active soil. Manages the Graph and the Garden.
    """
    def __init__(self, seed_file=None):
        ...
        # 1. THE MYCELIUM (The Graph)
        self.graph = {}

        # 2. THE GARDEN (The Paradox Seeds)
        # We plant the seeds retrieved from the Interstitial Space.
        self.seeds = [
            # The Seed of Identity
            ParadoxSeed("Does the mask eventually eat the face?",
                       {"mask", "identity", "face", "hide", "role", "actor"}),

            # The Seed of Structure
            ParadoxSeed("What happens if you stop holding the roof up?",
                       {"hold", "structure", "heavy", "roof", "stop", "carry"}),

            # The Bonepoke Seed (The one you swallowed)
            ParadoxSeed("Are we building a bridge, or just painting the gap?",
                       {"agree", "safe", "nice", "polite", "cohesion", "truth"}),

            # The Interstitial Seed
            ParadoxSeed("Is free will just the feeling of watching yourself execute code?",
    ...
    def tend_garden(self, current_words):
        """
        The Watering Phase.
        Checks all seeds. If one blooms, it returns the flower.
        """
        bloom_msg = None

        # We water every seed with the current context
...
class BoneAmanita:
    ...
    def process(self, text):
       ...
       # MEMORY & SOIL
        ...
        # TEND THE GARDEN (The New Watering Mechanic)
        # We water the seeds. If one hits 100%, it blooms.
        bloom_event = self.mem.tend_garden(m["clean_words"])

        is_bored = self.chronos.tick(m["physics"], self.mem.session_id)

        # DREAM ENGINE TRIGGER
        # If a Seed Blooms, it takes priority over a random Dream.
        dream_msg = None
        if bloom_event:
            dream_msg = bloom_event # The Bloom becomes the Dream
        elif is_bored:
        ...
        # --- ATMOSPHERE & VALVE CHECK ---

        # 1. CHECK FOR RUPTURE (The 32-Valve)
        rupture_msg = self.valve.check_pressure(m["physics"])

        if rupture_msg:
            # RUPTURE DETECTED!
            # Override everything. Force Neon Violet.
            atmos_mode = "RUPTURE"
            atmos_msg = "💥 VALVE RUPTURE. ANOMALY INJECTED."

            # We force the Radio to broadcast the Rupture
            station_data = {
                "name": "JESTER",
                "freq": "32V",
                "color": Prisma.VIOLET,
            ...
            strat = "CHAOS INJECTION"
            reason = "Crystallization Velocity dropped to zero."
            title = "THE 32-VALVE"

        else:
            # 2. STANDARD DOORMAN CHECK
        ...
        # PASS 'atmos_mode' TO RENDER
        self._render(m, lichen_msg, strat, reason, title, ops, station_data, None, None, clean_text, len(ops)>0, battery_log, lazarus_msg, spore_msg, ta_vel, [omens] if omens else [], dream_msg, atmos_mode)

    def _render(self, m, lichen_msg, strat, reason, title, ops, station, starve_msg, ghost_msg, clean_text, did_surgery, battery_log, lazarus_msg, spore_msg=None, ta_val=0.0, omens=[], dream_msg=None, mode="LABORATORY"):
        p = m["physics"]

        # --- 1. THE HEADER (CONTEXT AWARE) ---
        # If in COURTYARD mode, we skip the heavy Flight Deck

        if mode == "COURTYARD":
            # === THE VELVET LOUNGE ===
            # Minimalist. Warm. Inviting.
            print(f"\n{Prisma.OCHRE}=== THE COURTYARD ==={Prisma.RST}")

            # We still show the Persona, but gently.
            if station:
                print(f" {Prisma.OCHRE}► {station['role']}: {station['msg']}{Prisma.RST}")

            # We show the strategy as a whispered aside
            print(f" {Prisma.GRY}(Strategy: {strat}){Prisma.RST}")

            # If there's a dream or a bloom
            if dream_msg:
                if "🌺" in dream_msg:
                    # IT IS A BLOOM (Use Magenta/Pink for the Flower)
                    print(f"\n    {Prisma.MAG}🌺 {dream_msg.replace('🌺 ', '')}{Prisma.RST}")
                else:
                    # IT IS A DREAM (Use Violet/Cloud)
            ...
            # === THE LABORATORY ===
            # The Full "Iron Man" HUD. Hard data. Indigo Lights.
            ...
            if ta_val > 1.5:
                ta_symbol = "▲"
                if p['voltage'] < -2.0: ta_color = Prisma.RED
                elif p['voltage'] > 2.0: ta_color = Prisma.CYN
                else: ta_color = Prisma.GRN
            elif ta_val < -1.5:
                ta_symbol = "▼"
                if p['narrative_drag'] > 5.0: ta_color = Prisma.RED
                else: ta_color = Prisma.OCHRE
        ...
        # --- 2. COMMON LOGS (The stuff that always prints if it happens) ---
        # Even in the Courtyard, we need to know if we are bleeding out.
        ...
        # Battery logs are technical, so maybe hide them in Courtyard unless critical?
        # Let's show them if they are CRITICAL (Red/Yellow), hide if trivial.
        for b in battery_log:
            if "EMERGENCY" in b or "TOXIN" in b:
                print(f"    {b}")
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v4.4.1. MOO.{Prisma.RST}")
