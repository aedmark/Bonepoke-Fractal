# BONEAMANITA 5.6 - "THE COSMIC MYCELIUM"
...
class MycelialNetwork:
   # MANAGES THE GRAPH AND THE SPORE
    ...
    def calculate_mass(self, node):
        """
        Calculates the Gravitational Mass of a node.
        Mass = (Number of Edges * 1.5) + (Recency Bonus)
        """
        if node not in self.graph:
            return 0.0

        data = self.graph[node]
        edge_mass = len(data["edges"]) * 1.5

        # Recency Bonus: If spoken recently (high tick), it is 'Hot' and heavier.
        # We don't have global tick here easily, so we rely on Edge Density for now.
        return edge_mass
...
class MirrorGraph:
    ...
    def simulate(self, physics):
        ...
        # CHECK "LIKES" (Must include at least one valued category)
        hit_resonance = False
        ...
        # CHECK "HATES" (Must avoid hated categories)
...
class OuroborosDetector:
    SELF_REF_WEIGHTS = {"bone": 5.0, "amanita": 5.0, "courtyard": 3.0, "lexicon": 3.0, "system": 1.0, "physics": 1.0, "code": 0.5}
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v5.6 {Prisma.RST}")
