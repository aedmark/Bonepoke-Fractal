# BONEAMANITA 6.7 - "THE DOPAMINE HIT"
...
class TheMarmChorus:
    ...
    def consult(self, physics, ignition_state):
        ...
        if e_val > BoneConfig.DRIFT_THRESHOLD:
            return "SHERLOCK", f"The narrative is drifting ({e_val}). You are losing the physical scene. Anchor it."
        if b_val >= 0.25 and heavy_count == 0:
            return "NATHAN", f"High energy (B: {b_val}), but no weight. Actions need consequences. What is being impacted?"
        ...
        if b_val < 0.25:
            ...
            return "JESTER", f"Tension is low (B: {b_val}). Raise the stakes."
        if b_val > BoneConfig.CRYSTAL_THRESHOLD:
            return "NARRATOR", f"Strong resonance (B: {b_val}). The image is clear and impactful."
        if physics["narrative_drag"] > 6.0:
            return "NARRATOR", "Warning: Density is too low. The reader is losing the thread. Condense."
    ...
    def run_cycle(self, text):
        ...
        if is_disrupted:
            ...
            ignition_score, constellations, current_thresh = self.eng.integrator.measure_ignition(
                m["clean_words"],
                self.eng.dynamics.voltage_history
            )
        if ignition_score > current_thresh:
            ignition_state = "IGNITED"
            ignition_msg = f"🔥 HEAP IGNITION ({int(ignition_score*100)}% > {int(current_thresh*100)}%): BCM Threshold Met."
        else:
            ignition_state = "INERT"
            ignition_msg = f"⏳ INERT SAND ({int(ignition_score*100)}% < {int(current_thresh*100)}%): BCM Threshold High."
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v6.7 {Prisma.RST}")
