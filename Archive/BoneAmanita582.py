# BONEAMANITA 5.8.2 - "DUKE SILVER"
# Architects: SLASH | Auditors: The Courtyard, A Minotaur | Humans: James Taylor & Andrew Edmark
...
class BoneConfig:
    ...
    # SUBMARINE PROTOCOLS
    SILENT_MODE = True  # Default to "Submarine Mode"
    ...
    # THE SCENE DIRECTOR (Contextual Props)
    # These words grant Mass (Heavy) but influence Atmosphere.
    PROPS = {
        "LAB": {"grid", "laser", "flask", "acid", "steel", "chart", "graph", "node", "wire", "data"},
        "COURTYARD": {"coffee", "smoke", "chair", "wind", "mud", "glass", "paper", "ink", "tea", "book"},
        "DREAM": {"fog", "stair", "door", "mirror", "sand", "key", "shadow", "ghost", "echo"}
    }
...
class TheLexicon:
    ...
    def taste(word):
        """
        THE RHIZOME 5.7.5: Expanded Phonosemantics.
        Now detects Tech, Infrastructure, and Ballistics.
        """
        w = word.lower()
        ...
        # KINETIC: Active States
        if w.endswith(("ing", "ed")):
            return "kinetic", 0.6

        # AEROBIC: Adjectives/Air
        if w.endswith(("ous", "ly", "y", "ish")):
            return "aerobic", 0.6

        # --- PHONOSEMANTICS (The Sound) ---

        # 1. TECH-HEAVY (Infrastructure & Code)
        # Ends in -X (Complex, Matrix, Flux), -ODE (Node, Code), -ERVER (Server)
        if w.endswith(("ex", "ux", "ix", "ode", "erver")):
            return "heavy", 0.7

        # 2. BIO-HEAVY (Density & Impact)
        # Ends in -NK (Tank, Bank, Link), -DGE (Bridge, Ridge, Wedge), -CT (Struct, Tact)
        if w.endswith(("nk", "dge", "ct", "pt")):
            return "heavy", 0.6

        # 3. MODERN KINETIC (Velocity & Impulse)
        # Starts with V (Velocity), J (Jolt), Z (Zap), QU (Quick)
        if w.startswith(("v", "j", "z", "qu")):
            return "kinetic", 0.7

        # 4. CLASSIC SENSORY (The Old World)
        ...
        # Short, hard consonant endings are usually Heavy
...
class DivergenceEngine:
    ...
    def is_poetic_or_philosophical(self, raw_text, clean_words):
        # Heuristic: Short average word length + Long sentence structure + Punctuation
        ...
        # If words are simple but structure is complex, it's likely poetic/philosophical
...
class PhysicsEngine:
    ...
    def _scan_lexicon(self, clean_words, text):
        counts = Counter()
        # Scan Clean Words
        for w in clean_words:
            ...
            # Scan Atmospheres (Props)
            if w in BoneConfig.PROPS["LAB"]: counts["lab_atmos"] += 1
            if w in BoneConfig.PROPS["COURTYARD"]: counts["courtyard_atmos"] += 1
            if w in BoneConfig.PROPS["DREAM"]: counts["dream_atmos"] += 1
        ...
        return counts, toxin_score

    def _calculate_voltage(self, counts, toxin_score):
        thermal_tension = min(counts["thermal"], counts["cryo"]) * 5.0

        # ECP Violation Dampener
        dampener = 0.5 if (counts["abstract"] > 2 and counts["heavy"] == 0) else 1.0
        ...
        # Flywheel Smoothing
        return self.flywheel.smooth_voltage(raw_voltage, (counts["toxin"] > 0))

    def _calculate_drag(self, counts, total_words, toxin_score, action, is_whimsical, rhythm_var):
        mass_impact = total_words + (toxin_score * BoneConfig.TOXIN_WEIGHT)

        # Case Violation Penalty
        if counts["heavy"] > 0 and (counts["kinetic"] / counts["heavy"]) < 0.33:
            mass_impact *= 1.5

        base_drag = mass_impact / max(1.0, action)

        # Whimsy & Rhythm Bonuses
        if is_whimsical: base_drag *= 0.6
        if rhythm_var > 2.0: base_drag *= 0.9

        # Abstract Penalty
        if (counts["abstract"] / max(1, total_words) > 0.4 and not is_whimsical):
            base_drag += 0.5
    ...
    def analyze(self, text):
        ...
        # Scan
        counts, toxin_score = self._scan_lexicon(clean_words, text)
        rhythm_variance = self._measure_rhythm(text)

        # Heuristics
        repetition_score = counts.most_common(1)[0][1] / total
        whimsy_ratio = counts["aerobic"] / max(1, total)
        is_whimsical = (whimsy_ratio > 0.15) and (counts["toxin"] == 0)

        # Violations
        case_violation = (counts["heavy"] > 0) and ((counts["kinetic"] / counts["heavy"]) < 0.33)
        ecp_violation = (counts["abstract"] > 2) and (counts["heavy"] == 0)
        has_bleed = self._measure_bleed(clean_words)

        # Physics
        voltage = self._calculate_voltage(counts, toxin_score)
        action = (counts["kinetic"] * BoneConfig.KINETIC_GAIN) + BoneConfig.BASE_ACTION
        drag = self._calculate_drag(counts, total, toxin_score, action, is_whimsical, rhythm_variance)
        drag = self.flywheel.smooth_drag(drag, (repetition_score > 0.3))

        # Beta Friction
        beta_mod = 2.0 if ecp_violation else 1.0
        beta = (voltage / max(0.1, drag)) * beta_mod

        # Vector & State
        ent_score = min(1.0, counts["abstract"] / counts["heavy"]) if counts["heavy"] > 0 else (1.0 if counts["abstract"] > 0 else 0.0)
        ...
        if has_bleed and repetition_score <= 0.3: symbolic_state = "SALVAGE"
        elif repetition_score > 0.3: symbolic_state = "FATIGUE"
    ...
    def _measure_bleed(clean_words):
        ...
        for i, word in enumerate(clean_words):
            ...
            heavy = any(w in TheLexicon.get("heavy") for w in window)
            aerobic = any(w in TheLexicon.get("aerobic") for w in window)
            if heavy and aerobic: return True
        return False
    ...
    def _void_metrics():
        return {
            "physics": {
                ...
                "case_violation": False, "ecp_violation": False
            },
...
class HubThermostat:
    def __init__(self):
        ...
        self.history = [] # THE BUZZER: Sliding window of recent words
    ...
    def check_temperature(self, clean_words):
        ...
        # UPDATE HISTORY (The Sliding Window)
        # Add significant words to history (len > 3, not solvents)
        for w in clean_words:
            if len(w) > 3 and w not in TheLexicon.SOLVENTS:
                self.history.append(w)

        # Keep window at 50 words
        if len(self.history) > 50:
            self.history = self.history[-50:]

        # CHECK FREQUENCY (The Ban Hammer)
        # If a word appears > 5 times in the last 50 words, LOCK IT.
        overheated = []
        for w in clean_words:
            if len(w) > 3 and w not in TheLexicon.SOLVENTS:
                ...
                if count >= 5:
                    ...
                    # Clear this word from history so we don't re-ban it immediately
...
class ObserverEffect:
    ...
    def scan(self, physics, clean_words, learned_count, text):
        # OUROBOROS CHECK (Self-Reference)
        ...
        # CONFORMITY & GAMING CHECK (Metrics)
        ...
        # FAKE WEIGHTS (The Exploit)
        # Logic: If > 50% of input is Heavy Nouns, it's not a sentence; it's a grocery list.
        # This prevents users from spamming "stone iron bone" to trigger Ketosis.
...
class TheProjector:
    """
    THE VISUAL CORTEX
    Decoupled rendering logic. Handles the 'HUD' and text output.
    """
    def broadcast(self, engine, m, signals, mode, clean_text):
        p = m["physics"]

        # Unpack signals for easier reading
        ...
        if mode == "COURTYARD":
        ...
        else: # LABORATORY OR OTHER
            print(f"\n{Prisma.GRY}[ {Prisma.INDIGO}{title}{Prisma.GRY} | {Prisma.CYN}{strat}{Prisma.GRY} ]{'=' * 35}{Prisma.RST}")

            # Telemetry bars (Calculated using Engine data passed in)
            hp_p = int((engine.stamina / 50.0) * 10)
            hp_bar = f"{Prisma.GRN}{'|' * hp_p}{Prisma.GRY}{'.' * (10 - hp_p)}{Prisma.RST}"
            batt_bar = engine.battery.get_readout()
            ...
            print(f" {Prisma.INDIGO}DRAG:{Prisma.RST} {drag_c}  {Prisma.INDIGO}VOLT:{Prisma.RST} {volt_c}")
            print(f" {Prisma.MAG}COSMOS:{Prisma.RST} {cosmic_msg}")

            if station: print(f"\n {station['color']}► {station['msg']}{Prisma.RST}")

        # ALWAYS SHOW CRITICAL LOGS IF WE SURFACED
...
class LifecycleManager:
    ...
    Manages the sequential stages of consciousness:
    1. INPUT (Digestion)
    2. METABOLISM (Energy)
    3. REACTION (Immunity & Divergence)
    4. RENDER (Projection)
    5. SLEEP (Maintenance)
    """
    def __init__(self, engine):
        self.eng = engine # Reference to the main BoneAmanita instance

    def run_cycle(self, text):
        # STAGE 0: COMMAND INTERRUPT
        if self.eng.cmd.execute(text): return

        # STAGE 1: COMA CHECK (If dead, regenerate)
        if self.eng.coma_turns > 0:
            self._handle_coma(text)
            return

        # STAGE 2: PHYSICS (The Reality Check)
        self.eng.tick_count += 1
        m = self.eng.phys.analyze(text)

        # Inject Forge Stressors
        m["physics"] = self.eng.forge.apply_pressure(m["physics"])

        # STAGE 3: COSMIC DYNAMICS (Where are we?)
        ...
        # STAGE 4: METABOLISM (Energy Calculation)
        metabolic_data = self._metabolize(m, cosmic_state)

        # STAGE 5: REACTION (The Filter)
        # If reaction fails (e.g., Divergence or Thermostat block), we abort early.
        reaction_result = self._react(m, text, cosmic_state)
        if reaction_result["blocked"]:
            self._render_block(reaction_result["msg"])
            return

        # STAGE 6: GROWTH (Learning & Memory)
        self._grow(m, metabolic_data)

        # STAGE 7: RENDER (The Projector)
    ...
    def _metabolize(self, m, cosmic_state):
        # Digest Macros
        meta_type, meta_msg = self.eng.metabolism.digest(m["physics"], m["clean_words"])
        if meta_type == "INSULIN_SPIKE": m["physics"]["voltage"] *= 0.5

        # Photosynthesis
        sugar, lichen_msg = self.eng.lichen.photosynthesize(m["physics"])

        # Battery
        charged, charge_msg = self.eng.battery.absorb(m["physics"]["voltage"], m["clean_words"])
    ...
    def _react(self, m, text, cosmic_state):
        # Divergence (Skip if Cosmic Override)
        if cosmic_state not in ["LAGRANGE_POINT", "WATERSHED_FLOW"]:
            is_clean, div_msg, _ = self.eng.divergence.scan(m["clean_words"], text)
        ...
        # Entropy Vent
        vented, vent_msg = self.eng.phys.dissipate_entropy(m["physics"], self.eng.stamina, self.eng.mem.graph)
        if vented: return {"blocked": True, "msg": vent_msg}
        ...
        # Mirror
        mirror_pass, mirror_msg = self.eng.mirror.simulate(m["physics"])
    ...
    def _grow(self, m, meta):
        ...
        cost = 2.0 + (3.0 if m["physics"]["narrative_drag"] > 3.0 else 0) + (5.0 if m["physics"]["voltage"] > 8.0 else 0)
        ...
        # Antifragility
        self.eng.antifragile.record_turn(m["physics"]["voltage"], m["physics"]["narrative_drag"], learned)

        # Rhizome Learning
        if self.eng.tick_count < 15 or self.eng.tick_count % 3 == 0:
            self._rhizome_scan(m["clean_words"])

        # Health Calculation (Toxin/Burn/Exhaustion)
    ...
    def _calculate_health(self, phys):
        health_impact = 0
        damage_log = []

        toxin = phys["counts"].get("toxin", 0)
        if toxin > 0:
        ...
        if self.eng.health <= 0:
            ...
            self.eng.health = 20
            self.eng.trauma_accum = {k: 0.0 for k in self.eng.trauma_accum} # Reset
    ...
    def _render(self, m, meta, cosmic, react):
        ...
        # 2. Check Valve / Atmosphere
        rupture = self.eng.valve.check_pressure(m["physics"])
        if rupture:
            mode, station = "RUPTURE", {"name": "JESTER", "msg": rupture, "color": Prisma.VIOLET}
            strat, title = "CHAOS INJECTION", "THE 32-VALVE"
        else:
        ...
        # 3. Operations (Toxin replacements)
        ops = []
        clean_text = m["raw_text"]

        # RESTORED IMMUNE LOGIC
...
class BoneAmanita:
    def __init__(self):
        ...
        # THE LIFECYCLE (The New Heart)
        self.life = LifecycleManager(self)

    # Helper methods required by LifecycleManager (keep these)
    ...
    def process(self, text):
        # THE RON SWANSON SIMPLIFICATION
        # No more 200 lines of logic. Just run the cycle.
        self.life.run_cycle(text)

if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v5.8.2 {Prisma.RST}")
