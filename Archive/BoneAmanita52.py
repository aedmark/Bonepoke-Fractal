# BONEAMANITA 5.2 - "THE SELF-EATING SNAKE"
    ...
    def dissipate_entropy(phys, stamina, memory_graph):
        ...
        if len(clean_words) > 1:
            for i in range(len(clean_words) - 1):
                node_a = clean_words[i]
                node_b = clean_words[i+1]
...
class OuroborosDetector:
    SELF_REF_WORDS = {"system", "bone", "amanita", "courtyard", "lexicon", "physics", "code"}

    def detect(self, text, clean_words):
        # Count how many times we reference our own internal mechanics
        self_ref_count = sum(1 for w in clean_words if w in self.SELF_REF_WORDS)

        # The Ratio of Navel-Gazing
        total = max(1, len(clean_words))
        navel_ratio = self_ref_count / total

        if self_ref_count > 2 and navel_ratio > 0.3:
            return True, "🌀 OUROBOROS DETECTED: We are eating our own tail. Reference an external object immediately."
...
class AntifragilityMetric:
    ...
    def record_turn(self, voltage, drag, new_words_learned):
        # Stress is the magnitude of the chaos
        stress = abs(voltage) + drag

        # Growth is the tangible utility gained (Neuroplasticity)
        self.stress_history.append(stress)
        self.growth_history.append(new_words_learned)
    ...
    def calculate_convexity(self):
        if sum(self.stress_history) == 0: return 0.0

        # Simple heuristic: Are we learning MORE when stress is HIGH?
        # We want Positive Correlation between Stress and Growth.
        # Ideally: High Stress -> High Growth.

        high_stress_growth = 0
        low_stress_growth = 0
        avg_stress = sum(self.stress_history) / len(self.stress_history)

        for s, g in zip(self.stress_history, self.growth_history):
            if s > avg_stress:
                high_stress_growth += g
            else:
                low_stress_growth += g

        # The Convexity Ratio
        if low_stress_growth == 0: return 1.0 # Infinite gain
        return round(high_stress_growth / low_stress_growth, 2)
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v5.2 (GULP!){Prisma.RST}")
    print(
