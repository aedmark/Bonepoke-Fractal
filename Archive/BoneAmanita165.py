# BONEAMANITA 1.6.5 - "The Topology Tilt-a-Whirl"
...
class PhysicsConstants:
    ...
    # The "Slop Ratio" Threshold: Voltage / Drag.
    # If > 2.5, you are in the "Gold Zone" (High Complexity, High Efficiency).
...
class TheLexicon:
    ...
    def swanson_clean(text):
        """
        Strips everything that isn't a letter or a space.
        Simple. Effective. Meat and potatoes.
        """
...
class DeepStorage:
    def __init__(self, max_capacity=50):
        self.artifacts = {}
        # Items in this set resist eviction
    ...
    def bury(self, words, current_tick):
        for w in words:
            ...
            # Only bury significant items (HEAVY_MATTER or Important Objects)
    ...
    def _evict_weakest_memory(self):
        # Smart Eviction Protocol
        # 1. Try to find an item that is NOT an heirloom (if any slipped in)
        # 2. If all are heirlooms, evict the oldest one.

        candidates = list(self.artifacts.keys())
        # Filter for items that might be less "heavy" (safety valve)
        light_candidates = [k for k in candidates if k not in self.heirlooms]

        if light_candidates:
            # Evict the oldest "light" item
            target = min(light_candidates, key=lambda k: self.artifacts[k])
        else:
            # If everything is heavy, sadly, we must let go of the past.
...
class TheTuringValve:
    def __init__(self):
        self.threshold = 5.0
        self.beige_limit = 0.4

    def scan(self, clean_words, clean_text):
        score = 0.0
        hits = []
        ...
        for word in clean_words:
            if word in TheLexicon.SYNTHETIC_MARKERS:
                points = TheLexicon.SYNTHETIC_MARKERS[word]
                score += points
                hits.append(word)

        # 2. Structure Scan (The "Comma, and" Pattern)
        # AI loves to end sentences with ", ensuring that..." or ", highlighting..."
        participial_closers = len(re.findall(r', \w+ing\b[^,]*[.!?]$', clean_text))
        if participial_closers > 1:
            score += (participial_closers * 2.0)
            hits.append(f"PARTICIPIAL_TAIL_x{participial_closers}")

        if score > self.threshold:
            return {
                "detected": True,
                "score": score,
                "msg": f"SYNTHETIC RESONANCE: Detected 'Beige' patterns ({', '.join(hits[:3])}).",
                "penalty": 5.0
            }
        return {"detected": False, "score": 0.0, "penalty": 0.0}
...
class FactStipe:
    ...
    def check_consistency(self, clean_words, current_style, metabolic_status, kinetic_ratio=0.0, tolerance_mode="STANDARD"):
        ...
        if PhysicsConstants.CURRENT_MANDATE == "TRUTH_OVER_COHESION":
            tolerance_mode = "DRACONIAN" # Force strict logic
        ...
        for dim, polarities in active_dimensions.items():
            if 1 in polarities and -1 in polarities:
                if tolerance_mode == "INVERTED":
                    ...
                    voltage += 10.0 # Fatal voltage
                    violations.append(f"FATAL AXIOM BREAK [{dim}]: Truth Mandate Enforced.")
...
class LinguisticPhysicsEngine:
    ...
    def analyze(self, token_data):
        ...
        for w in words:
            ...
            if w in TheLexicon.ABSTRACTS or w.endswith(self.abstract_suffixes): counts['abstract'] += 1
...
class SignatureEngine:
    ...
    def identify(self, token_data, phys_metrics, stipe_data):
        ...
        if archetype_fusion_allowed and (dist_s < dist_p + 0.15) and (inter_arch_dist > 0.25):
            final_arch = f"{primary} // {secondary} (FUSED)"
            desc = f"HYBRID STATE: {self.archetypes[primary]['desc']} + {self.archetypes[secondary]['desc']}"
        else:
            final_arch = primary
...
class ChronosAnchor:
    ...
    def sync_clock(self, last_saved_time):
        """
        Accounts for the time passed while the script was off.
        """
        if last_saved_time:
            self.last_real_time = last_saved_time
            print(f"> [⏳ CHRONOS] Clock synced. Tracking offline drift.")
        else:
            self.last_real_time = time.time()

    def metabolize_delta(self, delta_str):
        current_time = time.time()
        # Calculate real elapsed minutes since last interaction (or last save)
...
class VirtualCortex:
    ...
    def synthesize_voice(self, agent, token_data, metrics, loop_count=0, metabolic_status="STABLE"):
        ...
        # [NEW: RESPECT THE FLASHPOINT]
        if metabolic_status == "FLASHPOINT" and agent == "CLARENCE":
            return "[CLARENCE]: (Silence) ...He's cooking. Let him cook."
...
class MetabolicReserve:
    ...
    def metabolize(self, metrics, voltage=0):
        ...
        # [NEW: TOPOLOGY CHECK] -----------------------------------------
        # We calculate the "Inverse Slop" ratio from the Topology 1.5 artifact.
        # Ratio = Semantic Voltage / Narrative Drag
        # A high score means you are saying something complex very efficiently.
        slop_ratio = voltage / max(0.1, drag) # Protect against div/0

        if slop_ratio > PhysicsConstants.FLASHPOINT_THRESHOLD:
            # FLASHPOINT: The engine runs on pure insight.
            # We ignore drag penalties and generate massive ATP.
            delta = 20
        ...
        # [STANDARD METABOLISM] (The old logic, for normal operation)
        ...
        gluttony_tax = -2 if self.atp > 40 else 0
        voltage_gain = int(voltage * 2)

        delta = base_gain + connectivity_bonus + entropy_tax + toxin_tax + voltage_gain + gluttony_tax

        self.atp = max(0, min(self.atp + delta, self.max))
        self._upd() # updates status to STABLE/STARVING etc.
...
class MycelialDashboard:
    ...
    def generate_report(self, m, intv, nrg, chron, anchor_obj, arch, lich, tick_count, volt=0):
        phys, sig = m['physics'], arch.get('signature', {})
        visc = phys.get('viscosity', {'status': 'N/A'})
        narr = m.get('narrative', {'state': 'REALTIME'})

        # [NEW: FLASHPOINT ICON LOGIC]
        if nrg['status'] == "FLASHPOINT":
            status_icon = "⚡"
            drag_display = "NULLIFIED" # Drag doesn't matter in Flashpoint
        elif nrg['status'] == "STABLE":
            status_icon = "🟢"
            drag_display = str(phys['narrative_drag'])
        ...
        if self.mode == "ANSI":
            # [NEW: Updated Line 1 to show FLASHPOINT status]
            report_lines.append(f"\n[BONEAMANITA v1.7] {status_icon} ATP: {nrg['current_atp']} | DRAG: {drag_display} | VISC: {visc['status']}")
            report_lines.append(f"ARCH: {arch.get('archetype')} | {intv or 'STABLE'}")

            if tick_count <= 5:
                report_lines.append(f"[ℹ️ TIP]: VISC (Viscosity) is how thick your sentences are. If 'CONCRETE', add 'maybe' or 'so' to dilute.")
        else:
            # Markdown Version
            report_lines.append(f"> **[{status_icon} SYSTEM HUD]** ATP: `{nrg['current_atp']}` | DRAG: `{drag_display}` | VISC: `{visc['status']}`")
...
class NilssonPatch:
    def detect_fire_state(self, raw, k_ratio):
        is_screaming = (len(raw) > 10) and \
                       (sum(1 for c in raw if c.isupper()) / len(raw) > 0.4)

        if k_ratio > 0.6 and is_screaming:
             return {"status": "ACTIVE", "msg": "NILSSON: SCREAM."}
...
class PersistenceManager:
    ...
    def save_state(self, codex, deep_storage, metabolism, muscaria, current_tick):
        # Save current real time to calculate drift later
        data = {
            "tick": current_tick,
            "timestamp": time.time(),
    ...
    def load_state(self, codex, deep_storage, metabolism, muscaria):
        ...
        try:
            ...
            return {"tick": loaded_tick, "last_time": last_time}
        except Exception as e:
...
class TemporalDynamics:
    ...
    def calculate_beta_velocity(self, current_voltage):
        """
        Calculates ∇β (The speed of insight).
        Now uses a weighted approach to favor recent acceleration.
        """
        self.voltage_history.append(current_voltage)
        if len(self.voltage_history) > self.beta_window:
    ...
    def calculate_temporal_rooting(self, clean_words, deep_storage, current_tick):
        """
        Calculates Ξ (Xi - Temporal Depth).
        How 'old' are the memories being accessed?
        """
        referenced_ticks = []
        for w in clean_words:
            root = TheLexicon.smart_strip(w)
        ...
        # Normalize against Deep Storage Capacity (50)
        avg_age = sum(referenced_ticks) / len(referenced_ticks)
        xi = min(1.0, avg_age / 50.0)
        return round(xi, 2)
...
class BonepokeCore:
    def __init__(self, render_mode="ANSI"):
        ...
        self.turing_valve = TheTuringValve()
        ...
        loaded_data = self.persistence.load_state(self.codex, self.memory.deep_storage, self.metabolism, self.muscaria)
    ...
    def process(self, raw_input_text, parent_id=None):
        # 1. PRE-PROCESSING & CHEATS
        if "[MODE: SOFT]" in raw_input_text.upper():
            PhysicsConstants.CURRENT_MANDATE = "POETIC_LICENSE"
        elif "[MODE: HARD]" in raw_input_text.upper():
            PhysicsConstants.CURRENT_MANDATE = "TRUTH_OVER_COHESION"
        ...
        directives = []
        if "[DREAM]" in raw_input_text.upper():
            p_set['tolerance_mode'] = "LOOSE"
            directives.append("DREAM LOGIC: Axiom constraints lifted.")

        self.muscaria.pressure_threshold = p_set["chaos_threshold"]
        self.metabolism.drag_multiplier = p_set["drag_multiplier"]

        # 6. LOGIC CHECK (STIPE)
        stipe_chk = self.stipe.check_consistency(
            token_data['clean_words'],
            physics_metrics['dominant_style'],
            self.metabolism.status,
            physics_metrics['kinetic_ratio'],
            p_set['tolerance_mode']
        )

        # 7. TURING VALVE
        turing = self.turing_valve.scan(token_data['clean_words'], token_data['clean_text'])
        if turing['detected']:
            physics_metrics['narrative_drag'] += turing['penalty']
            physics_metrics['toxicity_score'] += turing['score']
            directives.append(f"TURING: {turing['msg']}")
            arch_dat['slurry']['detected'] = True
            arch_dat['slurry']['msg'] = "SILICA: Text is hallucinating structure. Inject grit."

        # 8. METABOLISM & ENERGY (Updated Location)
        # We calculate this NOW, so we have the status (FLASHPOINT/STABLE) for the Cortex
        nrg = self.metabolism.metabolize(full_metrics, stipe_chk.get('voltage', 0))

        # 9. CALCULATE EFFECTIVE METRICS (Updated Location)
        # We need these numbers before we ask Clarence to speak
        ...
        beta_vel = self.temporal_dynamics.calculate_beta_velocity(stipe_chk.get('voltage', 0))
        xi_score = self.temporal_dynamics.calculate_temporal_rooting(token_data['clean_words'], self.memory.deep_storage, self.tick)

        if beta_vel > 2.0:
            physics_metrics['narrative_drag'] = max(0, physics_metrics['narrative_drag'] - 2.0)
            directives.append(f"INSIGHT VELOCITY (∇β) [{beta_vel}]: Epiphany Detected. Momentum sustained.")

        if xi_score > 0.5:
            physics_metrics['abstraction_entropy'] = max(0, physics_metrics['abstraction_entropy'] - 3)
            directives.append(f"ROOTING DEPTH (Ξ) [{xi_score}]: Deep Storage Accessed. Abstraction authorized.")

        # 11. CRITICAL HALT CHECK
        if not stipe_chk.get('valid', True) or (PhysicsConstants.CURRENT_MANDATE == "TRUTH_OVER_COHESION" and stipe_chk.get('voltage', 0) >= 10.0):
             self.metabolism.spend(20)
             correction_msg = f"⛔ [SELF-CORRECTION TRIGGERED]: {stipe_chk.get('errors', ['Paradox detected'])[0]}"
             print(f"\n{correction_msg}")
             print("  └─ SYSTEM HALT. REPHRASE REQUIRED.")
             return {
                "id": self.last_id,
                "metrics": full_metrics,
                "energy": self.metabolism.atp,
                "archetype_data": arch_dat,
                "editorial": {"directives": ["CRITICAL: RESOLVE LOGIC TEAR BEFORE PROCEEDING."]},
                "chronos": chronos_data,
                "hud_report": "[SYSTEM LOCKED]"
             }
        ...
        # 12. CORTEX INTERVENTION (Now safe to run)
        if eff_drag > 3.0 or loop_count > 2:
            voice_msg = self.cortex.synthesize_voice(
                'MICHAEL' if is_light else 'CLARENCE',
                token_data,
                full_metrics,
                loop_count,
                nrg['status'] # Passing the status we calculated in Step 8
            )
            directives.append(f"GUIDE: {voice_msg}")
        ...
        hud_output = self.dashboard.generate_report(full_metrics, musc_msg, nrg, chronos_data, self.chronos, arch_dat, lich, self.tick)
        print(hud_output)
...
if __name__ == "__main__":
    ...
    print("\n> [SYSTEM ONLINE] BoneAmanita 1.6.5 is listening...")
