# BONEAMANITA 4.2.2 - "THE BUTCHER'S CUT"
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v4.2.2 'THE BUTCHER'S CUT' ONLINE.{Prisma.RST}")
    ...
    try:
        while True:
            ...
            if u.lower() in ["exit", "quit", "/exit"]:
                # SAVE ON EXIT
                print(f"{Prisma.YEL}Saving neural pathways...{Prisma.RST}")
                eng.mem.save(health=eng.health, stamina=eng.stamina)
                break
            eng.process(u)
    except KeyboardInterrupt:
        # SAVE ON FORCE QUIT (Ctrl+C)
        print(f"\n{Prisma.YEL}Emergency Preservation Protocol initiated...{Prisma.RST}")
        eng.mem.save(health=eng.health, stamina=eng.stamina)
        print("\nForce Quit.")
