# ---------------------------------------------------------------------------
# BONEAMANITA 0.1 (A VARIANT STRAIN OF THE BONEPOKE ENGINE v4.3.5) - "The Red Cap"
    ...
    def check_consistency(self, current_text):
        ...
        for fact_key in self.facts:
            ...
            if core_concept in self.conflict_map:
                ...
                if conflicts:
                    violations.append(f"Stem Snap: '{core_concept}' clashes with {conflicts}.")
...
class BonepokeCore:
    ...
    def process(self, text, parent_id=None): # Allow manual parent override
        ...
        if not stipe_check['valid']:
            print(f"[WARNING] {stipe_check['errors']}")
