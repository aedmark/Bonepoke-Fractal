# # ---------------------------------------------------------------------------
# BONEAMANITA 0.3 (A VARIANT STRAIN OF THE BONEPOKE ENGINE v4.3.5) - "The Virtual Cortex"
...
class VirtualCortex:
    ...
    It uses 'Weighted Randomness' and 'Context Echoing' to fake consciousness.
    """
    def __init__(self):
        # --- PERSONALITY DATABANKS ---

        # CLARENCE: The Architect (Trigger: High Narrative Drag)
        self.clarence_templates = [
            "I am looking at a Drag Score of {drag}. It is offensive. Cut the word '{target_word}'.",
            "This sentence is a swamp. {drag} score? You are wading through molasses. Find the verb.",
            "You are building with marshmallows. The structure cannot hold. Tighten the struts.",
            "Efficiency failure. You used {word_count} words to say what could be said in {half_count}. Fix it.",
            "The verb '{target_word}' is weak. It has no spine. Replace it with a kinetic action."
        ]

        # ELOISE: The Gardener (Trigger: High Abstraction Entropy)
        self.eloise_templates = [
            "I can't touch this. It's all smoke. The Entropy is {entropy}. Give me dirt.",
            "You say '{target_word}', but I see nothing. Show me the rust. Show me the light.",
            "This is too clean. It feels like a hospital. Mess it up with some sensory detail.",
            "You are stuck in your head. 'Context', 'System', 'Framework'. Boring. What does the room smell like?",
            "Ground this. Anchor the thought to a physical object, or it will float away."
        ]

        # THE YAGA: The Witch (Trigger: Sycophancy / Weak Intent)
        self.yaga_templates = [
            "You offer me sugar ('{target_word}'). The Witch Ring demands meat.",
            "Do not apologize. Do not ask. simply Speak. The forest is listening.",
        ...
        # MUSCARIA: The Chaos Jester (Trigger: Boredom)
        self.muscaria_templates = [
            "BOREDOM ALERT. The text is gray. Suddenly, a bird flies into the window. Write that.",
    ...
    def _find_trigger_word(self, text, category):
        """
        Scans the text for a specific word to 'Echo' back to the user.
        This makes the bot feel like it actually read the sentence.
        """
        words = text.lower().replace('.', '').split()

        if category == 'stative':
            targets = {'is', 'was', 'seem', 'appear', 'have'}
        elif category == 'abstract':
            targets = {'context', 'system', 'framework', 'process', 'layer', 'logic'}
        elif category == 'sugar':
            targets = {'please', 'sorry', 'maybe', 'think', 'hope'}
        else:
            return "it" # Fallback

        # Return the first match found in the user's text
        for w in words:
            if w in targets:
                return w
        return "that" # Fallback if no specific target found

    def synthesize_voice(self, agent, text, metrics):
        """
        Assembles a 'Neural' response using the templates and metrics.
        """
        import random
        ...
        if agent == "CLARENCE":
            ...
            response = template.format(
                entropy=metrics['physics']['abstraction_entropy'],
                target_word=target
            )
...
if __name__ == "__main__":
    ...
    print("--- BONEAMANITA v0.3: THE VIRTUAL CORTEX ---")
