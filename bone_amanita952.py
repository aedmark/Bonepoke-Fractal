...
class TheCycleController:
    ...
    def run_turn(self, user_message: str) -> Dict[str, Any]:
        ...
        security_package = self.security.audit_input(user_message, context, cycle_logs)
        if security_package:
    ...
    def _phase_physics_and_nav(self, user_message):
        cycle_logs = []
        # Accessing systems via the engine reference
        context = self.eng.phys.tension.gaze(user_message, self.eng.mind.mem.graph)
        phys = context["physics"]
        self.eng.phys.tension.last_physics_packet = phys

        current_manifold = self.eng.navigator.locate(phys)

        if self.eng.navigator.check_anomaly(user_message):
            ...
            cycle_logs.append(f"{Prisma.GRY}[NAV]: Orbiting {current_manifold} | Shimmer: {self.eng.bio.shimmer.current:.1f}{Prisma.RST}")
    ...
    def _phase_memory_encoding(self, m):
        ...
        shift_msg = self.eng.bio.governor.shift(m["physics"], self.eng.bio.plasticity.history)
        if shift_msg:
            self.eng.events.log(shift_msg, "GOV")

    def _phase_apply_overrides(self, m):
        m["physics"]["psi"] = self.eng.bio.governor.psi_mod
    ...
    def _phase_simulation_loop(self, user_message, context, cycle_logs):
        phys = context["physics"]
        phys["tick_count"] = self.eng.tick_count # Inject time
        folly_clamp, folly_msg, voltage_mod, _ = self.eng.folly.audit_desire(phys, self.eng.stamina)
        if folly_clamp:
            phys["voltage"] = voltage_mod
            cycle_logs.append(f"\n{folly_msg}")
        is_kintsugi, k_msg = self.eng.kintsugi.attempt_repair(phys, self.eng.trauma_accum)
        if is_kintsugi:
            cycle_logs.append(f"\n{k_msg}")
            phys["narrative_drag"] = 0.0
        feedback_signal = {
        ...
        self.eng.safety.audit_cycle({"err": phys["repetition"]})
        drift_delta, grav_msg = self.eng.gordon.check_gravity(phys["narrative_drag"], phys["psi"])
        if drift_delta != phys["narrative_drag"]:
            phys["narrative_drag"] = drift_delta
            cycle_logs.append(f"\n{grav_msg}")
        tool_logs = self.eng.gordon.audit_tools(phys)
        if tool_logs:
            cycle_logs.extend(tool_logs)
            if "pain_signal" in phys:
                self.eng.health -= phys["pain_signal"]
                del phys["pain_signal"]
        is_pain, pain_msg, panic_physics = self.eng.gordon.flinch(context["clean_words"])
        if is_pain:
            if panic_physics:
                for k, v in panic_physics.items():
                    if k == "narrative_drag": phys["narrative_drag"] += v
                    elif k == "voltage": phys["voltage"] = v
            cycle_logs.append(f"\n{pain_msg}")
        g_active, g_log = self.eng.gordon.emergency_reflex(phys)
        ...
        if not bio_result["is_alive"]:
            return self.eng._trigger_death(phys)
        vel = self.eng.phys.dynamics.get_velocity()
        if vel > 4.0:
            burn = vel * 1.5
            self.eng.stamina -= burn
            cycle_logs.append(f"VELOCITY SPIKE ({vel:.2f}): -{burn:.1f} Stamina.")
        transmute_msg = self.eng.phys.forge.transmute(phys)
        if transmute_msg: cycle_logs.append(f"\n{transmute_msg}")
        is_pareidolia, p_msg = self.eng.check_pareidolia(context["clean_words"])
        if is_pareidolia:
            cycle_logs.append(f"\n{p_msg}")
            phys["psi"] = min(1.0, phys["psi"] + 0.3)
        is_stuck, _, t_msg, t_crit = self.eng.phys.theremin.listen(phys, self.eng.bio.governor.mode)
        if t_msg: cycle_logs.append(f"\n{t_msg}")
        if t_crit == "AIRSTRIKE": self.eng.health -= 25.0
        rupture, rupture_msg = RuptureEngine.check_for_disruption(phys, self.eng.mind.lex, self.eng.trauma_accum)
        if rupture:
            cycle_logs.append(f"\n{rupture_msg}")
            phys["voltage"] += 5.0
            phys["beta_index"] = 0.0
        if self.eng.cassandra.check_trigger(phys):
            c_msg = self.eng.cassandra.seize()
            if c_msg:
                cycle_logs.append(c_msg)
                phys["voltage"] = max(5.0, phys["voltage"] - 8.0)
                cycle_logs.append(f"{Prisma.GRY}   (DISCHARGE: Voltage dropped by 8.0 to silence the voice.){Prisma.RST}")
        pulse_status = self.eng.phys.pulse.get_status()
        if pulse_status == "ZOMBIE_KNOCK" or phys["narrative_drag"] > 5.0:
            ...
            if healed_trauma and heal_amt > 0:
                ...
                cycle_logs.append(f"{Prisma.GRN}   TRAUMA PROCESSED: {healed_trauma} -{heal_amt:.2f}{Prisma.RST}")
        mind_state = self.eng.noetic.think(
            phys, bio_result, self.eng.gordon.inventory,
            self.eng.phys.dynamics.voltage_history)
        if mind_state["mode"] == "REFUSAL":
            trig = mind_state.get("trigger")
            if trig == "GURU_TRAP": cycle_logs.append(f"\n{self.eng.refusal.execute_guru_refusal()}")
            elif trig == "SILENT": cycle_logs.append(f"\n{self.eng.refusal.execute_silent()}")
            elif trig == "MIRROR": cycle_logs.append(f"\n{self.eng.refusal.execute_mirror(user_message)}")
            elif trig == "FRACTAL": cycle_logs.append(f"\n{self.eng.refusal.execute_fractal(user_message)}")
            else: cycle_logs.append(f"\n{self.eng.refusal.manifest_glitch(user_message, self.eng.mind.mem)}")
            return self.eng._package_turn("REFUSAL", cycle_logs, context)
        world_state = self.eng.kinetic.update_world(phys, bio_result, mind_state)
        self.eng._apply_cosmic_physics(phys, world_state["orbit"][0], 0.0)
        ...
        ui_output = self.eng.projector.render(context, {
            "bio": bio_result, "mind": mind_state, "world": world_state,
            "title": title_data, "health": self.eng.health
        }, (mind_state.get("lens"), self.eng.limbo.haunt(mind_state["thought"])))
        sys_instr, _ = self.eng.director.generate_chorus_instruction(phys)
        cycle_logs.extend([e['text'] for e in self.eng.events.flush()])
        return {
            "type": "CYCLE_COMPLETE",
            "ui": ui_output,
            "logs": cycle_logs,
