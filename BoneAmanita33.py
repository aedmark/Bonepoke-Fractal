# BONEAMANITA 3.3 - "THE RESONANCE"
# Base: v3.2.1 | Restored: Memory, Radio, Time from 2.2 | Auditors: James Taylor & Andrew Edmark
...
# --- 5. FREQUENCY MODULATOR (THE RADIO RESTORED) ---
class FrequencyModulator:
    ...
    STATIONS = {
        ...
        # THE GHOST STATION (Interference Pattern)
    ...
    def tune_in(self, phys, vector, is_rag_slop):
        ...
        # 3. STANDARD TUNING (Loudest Signal)
...
class WisdomNode:
    ...
    def _resolve_title(self, vec):
        # 1. Sort dimensions by intensity to find Primary and Secondary traits
    ...
    def architect(self, metrics, station, is_bored):
        ...
        # 3. THE APEIROGON (Infinite Title Generation)
...
class BoneAmanita:
    ...
    def _invoke_ghost(self, current_phys):
        ...
        # 1. CLARENCE'S GHOST (Did you cut?)
    ...
    def process(self, text):
        ...
        ops = ["Toxins neutralized"] if clean_text != text else []
        ...
        # 6. ARCHITECTURE
        # Note: We now unpack 3 values because of Phase 1 (strategy, reason, title)
        strat, reason, title = self.wise.architect(m, station_data, is_bored)

        # --- [THE GHOST INVOCATION] ---
        ...
        # Pass ghost_msg to render
        self._render(m, lichen_msg, strat, reason, title, ops, station_data, starvation_msg, ghost_msg)
        self.mem.save()

    def _render(self, m, lichen_msg, strat, reason, title, ops, station, starve_msg, ghost_msg):
        p = m['physics']

        print(f"\n{Prisma.GRY}--------------------------------------------------{Prisma.RST}")
        print(f"{Prisma.WHT}[BONEAMANITA v3.4]{Prisma.RST} {Prisma.CYN}{title}{Prisma.RST}")
        ...
        if ops: print(f"{Prisma.RED}🔪 SURGERY:{Prisma.RST} {ops[0]}")
...
if __name__ == "__main__":
    eng = BoneAmanita()
    print(f"{Prisma.GRN}>>> BONEAMANITA v3.3 ONLINE (RESONANCE ENGINE).{Prisma.RST}")
    while True:
