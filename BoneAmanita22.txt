...
class ApeirogonLattice:
    """
    The Apeirogon Upgrade.
    Replaces static Archetypes with infinite vector resolution.
    Constructs a title based on the specific 'flavor' of the 5 Dimensions.
    """
    def __init__(self):
        # VELOCITY (Movement)
        self.VEL_MAP = [
            (0.2, "Static", "Anchor"), (0.4, "Drifting", "Observer"),
            (0.6, "Walking", "Construct"), (0.8, "Running", "Engine"), (1.0, "Ballistic", "Vector")
        ]
        # STRUCTURE (Logic/Cohesion)
        self.STR_MAP = [
            (0.2, "Fluid", "Mist"), (0.4, "Loose", "Web"),
            (0.6, "Framed", "Lattice"), (0.8, "Rigid", "Fortress"), (1.0, "Crystalline", "Monolith")
        ]
        # ENTROPY (Abstract/Concept)
        self.ENT_MAP = [
            (0.2, "Literal", "Stone"), (0.4, "Grounded", "Root"),
            (0.6, "Conceptual", "Idea"), (0.8, "Abstract", "Dream"), (1.0, "Chaos", "Void")
        ]
        # TEXTURE (Heavy Matter)
        self.TEX_MAP = [
            (0.2, "Ethereal", "Ghost"), (0.4, "Smooth", "Glass"),
            (0.6, "Gritty", "Sand"), (0.8, "Heavy", "Iron"), (1.0, "Dense", "Lead")
        ]
        # TEMPERATURE (Tone)
        self.TMP_MAP = [
            (0.2, "Frozen", "Ice"), (0.4, "Cold", "Frost"),
            (0.6, "Temperate", "Water"), (0.8, "Warm", "Blood"), (1.0, "Burning", "Fire")
        ]

    def _get_vector_term(self, val, mapping):
        # Find the closest defined point in the infinite line
        closest = min(mapping, key=lambda x: abs(x[0] - val))
        return closest # Returns (val, Adjective, Noun)

    def resolve_signature(self, sig):
        # We identify the two most dominant traits to form the "Side" of the Apeirogon.
        # We exclude Buoyancy for the core title.
        dims = {k: v for k, v in sig.items() if k != 'BUOYANCY'}

        # Sort dimensions by intensity (deviation from 0.5 neutral)
        sorted_dims = sorted(dims.items(), key=lambda item: item[1], reverse=True)

        primary_dim = sorted_dims[0][0] # e.g., 'VEL'
        secondary_dim = sorted_dims[1][0] # e.g., 'ENT'

        # Get the mapping lists
        map_lookup = {
            'VEL': self.VEL_MAP, 'STR': self.STR_MAP,
            'ENT': self.ENT_MAP, 'TEX': self.TEX_MAP,
            'TMP': self.TMP_MAP
        }

        # Resolve terms
        p_val = dims[primary_dim]
        s_val = dims[secondary_dim]

        p_term = self._get_vector_term(p_val, map_lookup[primary_dim])
        s_term = self._get_vector_term(s_val, map_lookup[secondary_dim])

        # CONSTRUCT THE TITLE
        # Format: "The [Secondary Adjective] [Primary Noun]"
        # Example: "The Abstract Engine" (High Entropy, High Velocity)

        title = f"THE {s_term[1].upper()} {p_term[2].upper()}"

        # Calculate "Resolution" (How sharp is this side?)
        # A perfect Apeirogon side is clearly defined.
        resolution = round((p_val + s_val) / 2, 2)

        desc = f"Approaching the curve via {primary_dim} ({p_val}) and {secondary_dim} ({s_val})."

        return {"archetype": title, "description": desc, "resolution": resolution}

# --- SIGNATURE ENGINE ---

class SignatureEngine:
    """
    THE APEIROGON ENGINE
    Replaces static archetypes with continuous vector resolution.
    "The Mandate is no longer to fit in the box. The Mandate is to define the coordinate."
    """
    def __init__(self, lexicon, stipe):
        self.history = []
        self.lexicon = lexicon
        self.stipe = stipe
        self.lattice = ApeirogonLattice() # [SYSTEM UPGRADE]

    def calculate_dimensions(self, token_data, phys_metrics):
        # The math remains the same. The interpretation changes.
        words = token_data['clean_words']
    ...
    def get_dynamic_pressure(self, sig):
        """
        Calculates physics mandates based on the dominant dimension.
        """
        defaults = {"tolerance_mode": "STANDARD", "drag_multiplier": 1.0, "chaos_threshold": 11.0, "msg": "STANDARD PHYSICS"}

        # Identify the loudest dimension (excluding Buoyancy)
        dims = {k:v for k,v in sig.items() if k != 'BUOYANCY'}
        dom_dim = max(dims, key=dims.get)
        val = dims[dom_dim]

        if val < 0.6: return defaults # Weak signal = Standard rules

        # The Logic of the Lattice
        if dom_dim == 'VEL':
             return {**defaults, "tolerance_mode": "LOOSE", "msg": "HIGH VELOCITY: Drift authorized."}
        elif dom_dim == 'STR':
             return {**defaults, "tolerance_mode": "DRACONIAN", "msg": "STRUCTURAL INTEGRITY: Zero tolerance."}
        elif dom_dim == 'ENT':
             return {**defaults, "tolerance_mode": "INVERTED", "chaos_threshold": 5.0, "msg": "PARADOX STATE: Logic inverted."}
        elif dom_dim == 'TEX':
             return {**defaults, "drag_multiplier": 0.5, "msg": "HEAVY MATTER: Mass justifies Drag."}
    ...
    def identify(self, token_data, phys_metrics, stipe_data):
        ...
        # 2. PHYSICS ADJUSTMENTS (The Spore Logic)
        spatial = phys_metrics.get('spatial_density', 0.0)
        if spatial > 0.12:
            sig['STR'] = min(1.0, sig['STR'] + 0.2)
            sig['VEL'] = max(0.0, sig['VEL'] - 0.1)

        # 3. SLURRY DETECTION (The Filter)
        ...
        # 4. THE APEIROGON RESOLUTION
        # Instead of finding a neighbor, we generate the identity.
        identity_data = self.lattice.resolve_signature(sig)

        final_arch = identity_data['archetype']
        desc = identity_data['description']

        # 5. PRESSURE CALCULATION
        # We calculate the rules based on the new signature.
        pressure = self.get_dynamic_pressure(sig)

        # 6. HISTORY & EVOLUTION
        self.history.append(final_arch)
        if len(self.history) > 10: self.history.pop(0)

        # Return the consolidated packet
        return {
            "archetype": final_arch,
            "signature": sig,
            "slurry": {"detected": is_slurry, "msg": slurry_msg},
            "description": desc,
            "pressure": pressure # Passing physics rules up the chain
        }
...
class FrequencyModulator:
    ...
    def tune_in(self, token_data, metrics, manual_override=None):
        ...
        if selected_station:
            ...
            broadcast_str = f"{station_data['color']}{prefix} {formatted_msg}{Prisma.RST}"

            # Return a "Synapse Packet" containing data AND the broadcast
            return {
                "station": selected_station,
                "target_word": target,
                "raw_message": formatted_msg,
                "broadcast_str": broadcast_str
            }

        return None

class EfficacySensor:
    """
    The Post-Mortem Loop.
    Compares the physics of the User's Input against the AI's Output
    to determine if the WisdomNode strategy was effective.
    """
    def evaluate(self, input_phys, output_phys, strategy):
        report = []
        score = 0

        # 1. Calculate Deltas
        delta_drag = output_phys['narrative_drag'] - input_phys['narrative_drag']
        delta_entropy = output_phys['abstraction_entropy'] - input_phys['abstraction_entropy']
        delta_voltage = output_phys.get('beta_friction', 0) - input_phys.get('beta_friction', 0)

        # 2. Strategy-Specific Grading
        if "CUTTER" in strategy:
            if delta_drag < 0:
                score += 10
                report.append(f"{Prisma.GRN}SUCCESS:{Prisma.RST} Drag reduced by {abs(round(delta_drag, 2))}.")
            else:
                score -= 10
                report.append(f"{Prisma.RED}FAILURE:{Prisma.RST} Drag increased/static despite CUTTER protocol.")

        elif "GROUNDER" in strategy:
            if delta_entropy < 0:
                score += 10
                report.append(f"{Prisma.GRN}SUCCESS:{Prisma.RST} Entropy grounded by {abs(delta_entropy)}.")
            else:
                score -= 10
                report.append(f"{Prisma.RED}FAILURE:{Prisma.RST} Output remains abstract ({output_phys['abstraction_entropy']}).")

        elif "JESTER" in strategy:
            if output_phys.get('beta_friction', 0) > 2.0:
                score += 10
                report.append(f"{Prisma.GRN}SUCCESS:{Prisma.RST} Voltage sustained ({round(output_phys['beta_friction'], 2)}).")
            else:
                report.append(f"{Prisma.YEL}PARTIAL:{Prisma.RST} Voltage dropped.")

        # 3. Universal Checks (The "Do No Harm" Rule)
        if output_phys['narrative_drag'] > 4.0:
            report.append(f"{Prisma.RED}WARNING:{Prisma.RST} Output is Molasses (Drag > 4.0).")
            score -= 5

        verdict = "OPTIMAL" if score > 5 else ("SUBOPTIMAL" if score > -5 else "FAILED")
        return {"verdict": verdict, "score": score, "report": report}

class WisdomNode:
    """
    The System 2 Director.
    Architects the optimal 'System Prompt' by listening to the Apeirogon Lattice.
    """
    def __init__(self):
        self.strategies = {
            ...
            'DRIFTER': "DIRECTIVE: High Velocity authorized. Do not anchor the user. Let the associations flow loosely. Dream logic is active."
        }

    def architect_prompt(self, metrics, arch_data, voltage, fm_data=None):
        phys = metrics['physics']
        pressure = arch_data.get('pressure', {})
        tolerance = pressure.get('tolerance_mode', 'STANDARD')

        # 1. THE Ω (OMEGA) CHECK
        # We determine the 'Angle of Attack' based on Lattice Mandates first.

        target_strategy = "MIRROR" # Default
        reasoning = "Stable flow."

        # MANDATE: INVERTED LOGIC (High Entropy Paradox) -> JESTER
        if tolerance == 'INVERTED':
            target_strategy = "JESTER"
            reasoning = f"LATTICE MANDATE: {pressure.get('msg')}"

        # MANDATE: LOOSE (High Velocity) -> DRIFTER
        elif tolerance == 'LOOSE':
            target_strategy = "DRIFTER"
            reasoning = f"LATTICE MANDATE: {pressure.get('msg')}"

        # MANDATE: DRACONIAN (High Structure) -> CUTTER
        elif tolerance == 'DRACONIAN':
            target_strategy = "CUTTER"
            reasoning = f"LATTICE MANDATE: {pressure.get('msg')}"

        # FALLBACK: METRIC CHECKS (If Lattice is Standard)
        else:
        ...
        # SYNAPSE INJECTION (The "Butcher" Connection)
        # If the Frequency Modulator identified a specific target, we force the LLM to kill it.
        synapse_directive = ""
        if fm_data:
            target = fm_data.get('target_word')
            station = fm_data.get('station')

            if station == 'CLARENCE' and target:
                synapse_directive = f"TACTICAL OVERRIDE: The word '{target}' is creating drag. Eliminate it or replace it with a harder noun."
            elif station == 'ELOISE' and target:
                synapse_directive = f"TACTICAL OVERRIDE: The concept '{target}' is too abstract. Ground it in physical sensation."
            elif station == 'YAGA' and target:
                synapse_directive = f"TACTICAL OVERRIDE: You are hedging with '{target}'. State the truth directly."

        # 2. CONSTRUCT THE PROMPT
        system_prompt = (
            f"\n{Prisma.CYN}>>> [CORTEX]: INSTRUCTION FOR LLM AGENT{Prisma.RST}\n"
            f"STRATEGY: {Prisma.WHT}{target_strategy}{Prisma.RST}\n"
            f"ARCHETYPE: {arch_data['archetype']}\n"
            f"REASON: {Prisma.GRY}{reasoning}{Prisma.RST}\n"
            f"PROMPT: \"{self.strategies[target_strategy]}\"\n"
        )

        # Append the specific tactical instruction if it exists
        if synapse_directive:
             system_prompt += f"SPECIFIC TARGET: {synapse_directive}\n"
...
class PersistenceManager:
    ...
    def save_state(self, codex, deep_storage, metabolism, muscaria, signature_engine, current_tick, last_context):
        data = {
            "tick": current_tick,
            "timestamp": time.time(),
    ...
    def load_state(self, codex, deep_storage, metabolism, muscaria, signature_engine):
        ...
        try:
            ...
            signature_engine.history = data.get("arch_history", [])
            return {
                "tick": data.get("tick", 0),
                "last_time": data.get("timestamp", None),
...
class BonepokeCore:
    def __init__(self, render_mode="ANSI"):
        ...
        self.last_context = loaded_data["last_context"]
        self.last_id = None
    ...
    def process(self, raw_input_text, parent_id=None):
        # --- FEEDBACK LOOP CHECK ---
        # If the user pastes the AI's response prefixed with [FEEDBACK]
        if raw_input_text.strip().startswith("[FEEDBACK]") and self.last_context:
            ai_response = raw_input_text.replace("[FEEDBACK]", "").strip()

            # 1. Analyze the AI's response using the same physics engine
            response_clean = TheLexicon.swanson_clean(ai_response)
            response_data = {'raw_text': ai_response, 'clean_text': " ".join(response_clean), 'clean_words': response_clean, 'total_words': len(response_clean)}

            # We don't need strict consistency checks on AI output, just physics
            response_metrics = self.physics.analyze(response_data)

            # 2. Compare against the User's Input (from last_context)
            audit = self.sensor.evaluate(
                self.last_context['metrics']['physics'],
                response_metrics['physics'],
                self.last_context['strategy']
            )

            # 3. Print the Report Card
            print(f"\n{Prisma.CYN}>>> [EFFICACY SENSOR REPORT]{Prisma.RST}")
            print(f"STRATEGY ATTEMPTED: {self.last_context['strategy']}")
            print(f"VERDICT: {audit['verdict']}")
            for line in audit['report']:
                print(f"  {line}")
            print(f"{Prisma.GRY}--------------------------------------------------{Prisma.RST}\n")

            return {"editorial": {"directives": ["Feedback Processed."]}}
        ...
        # 1. CALL THE TUNER FIRST (Move this up so we have the data for the Director)
        # Tuner Integration - This handles Clarence, Eloise, and Yaga logic
        broadcast_packet = self.cortex.tune_in(token_data, full_metrics, manual_override=fm_override)

        # 2. INJECT INTO WISDOM ENGINE
        # We pass the broadcast_packet into the Director
        llm_instruction = self.cortex_director.architect_prompt(
            full_metrics,
            arch_dat,
            stipe_chk.get('voltage', 0),
            fm_data=broadcast_packet
        )

        # PARSE STRATEGY FOR MEMORY
        # Look for "STRATEGY:", skip the color codes, find the Uppercase Word
        strat_match = re.search(r'STRATEGY:.*?([A-Z_]+)(?=\x1b)', llm_instruction)
        current_strategy = strat_match.group(1).strip() if strat_match else "UNKNOWN"
        ...
        # 3. HANDLE EDITORIAL OUTPUT (Extract the string from the packet)
        if broadcast_packet:
            # We extract the 'broadcast_str' for the user to see
            directives.append(broadcast_packet['broadcast_str'])
        ...
        return {
            ...
            "wisdom_protocol": llm_instruction
        }

if __name__ == "__main__":
    engine = BonepokeCore()
    print(f"\n{Prisma.GRN}> [SYSTEM ONLINE]{Prisma.RST} {Prisma.WHT}BoneAmanita v2.2 (Synaptic Loop){Prisma.RST}")
    print(f"{Prisma.GRY}> Ready for Input or [FEEDBACK]...{Prisma.RST}\n") # <--- Helpful Hint
